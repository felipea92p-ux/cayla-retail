...
/**
 * El vocabulario cerrado de colores — y el primer lugar donde de verdad se
 * puede agregar uno (hueco documentado en docs/datos/modulos/02-catalogo-y-vocabulario.md:
 * hasta hoy, `0048_conteos.sql` y `0051_conteo_color_vacio.sql` le dicen a la persona
 * "La Líder puede agregarlo en Catálogo → Colores" y esa pantalla no existía).
 *
 * La policy de escritura (`colores_insert_lider`, 0046) ya exige Líder — `puedeEditar`
 * solo decide si se MUESTRA el botón; el candado real vive en la base, no acá.
 */

type Color = { codigo: string; nombre: string; familiaColor: string; hex: string | null };
...
export function ColoresLista({ coloresIniciales, puedeEditar }: { coloresIniciales: Color[]; puedeEditar: boolean }) {
  ...
  async function guardar() {
    ...
    setAviso(null);
    try {
      const res = await fetch("/api/inventario/colores", {
        method: "POST",
      ...
      setColores((actual) =>
        [...actual, { codigo: datos.color.codigo, nombre: datos.color.nombre, familiaColor: datos.color.familia_color, hex: datos.color.hex }].sort((a, b) => a.nombre.localeCompare(b.nombre))
      );
      setAgregando(false);
    } catch {
