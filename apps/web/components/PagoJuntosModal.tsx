...
export function PagoJuntosModal({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    avisar.exito(`Pago de ${soles(total)} registrado`, {
      detalle: `${lote.length === 1 ? "1 comprobante" : `${lote.length} comprobantes`} de ${proveedorNombre}, como un solo pago.`,
    });
    router.refresh();
    onPagado();
  }
        ...
        <form onSubmit={onSubmit} className="space-y-5">
          ...
          <div className="border-t border-tinta/10 pt-4">
            <p className="text-sm text-tinta">
              Pagarás <span className="font-display text-xl tabular-nums">{soles(total)}</span> · quedarán en cero{" "}
              <b className="font-semibold">{enCero === 1 ? "1 comprobante" : `${enCero} comprobantes`}</b>
            </p>
            <p className="mt-1 text-xs leading-relaxed text-tinta/65">
              Se registra como <b className="font-semibold">un solo pago</b>: en el estado de cuenta del banco verás una línea de {soles(total)}. Cada comprobante conserva su propio historial de pagos.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 border-t border-tinta/10 pt-4">
            ...
            <button type="submit" className={BTN_PRIMARIO} disabled={!puedeRegistrar}>
              {loading ? "Registrando…" : `Registrar pago de ${soles(total)}`}
            </button>
          </div>
        </form>
      )}
