...
export type CompraResumen = {
  ...
  /**
   * Tiendas a las que está repartido el comprobante (ADR-0132). Antes era UN destino (`ubicacionDestinoId`): una
  ...
  /**
   * Lo que le toca a la tienda DESDE LA QUE SE MIRA (ADR-0132): solo viene cuando la lista se pide con una tienda
...
export type LineaCompra = {
  ...
  cerrado: number;
  pendiente: number;
  // ---- Reparto por tienda (ADR-0132). Con una tienda de por medio, `cantidad`, `recibido`, `cerrado` y `pendiente`
...
export type FilaOperativa = {
  ...
  /** Las tiendas del reparto (ADR-0132); para un colaborador, solo la suya. */
...
export function comprobanteDeFilaOperativa(f: FilaOperativa): CompraResumen {
  // Con una tienda de por medio, las cifras del comprobante pasan a ser las de ESA tienda (ADR-0132).
  const aqui =
