...
function deps(esLider: boolean | null | "lanza", admin = adminDoble()) {
  ...
  const d: Dependencias = {
    ...
    personaActual: async () => "persona-felipe",
    admin: abrirAdmin,
    azar: (n) => randomBytes(n),
  };
  return { d, abrirAdmin, admin };
}
...
describe("crear (líder)", () => {
  it("crea la cuenta y la fila, con creada_por = el líder, y devuelve correo y clave UNA vez", async () => {
