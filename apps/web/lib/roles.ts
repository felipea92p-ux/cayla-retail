...
// Lectura de «Roles y accesos» (ADR-0161 B, migración 20260923030000_roles_por_modulo.sql). Solo el líder: las tablas
// `roles` y `rol_modulos` se leen por RLS (`fn_es_lider()`) y las cuentas por `fn_cuentas_con_rol()`, que lo vuelve a
// exigir. Las escrituras van por RPC desde el cliente (`lib/roles-acciones.ts`).
