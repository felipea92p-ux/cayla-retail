...
//
// Quien no es líder no recibe montos: los costos se recortan ACÁ, en el servidor, antes de llegar a la pantalla. (La
// base todavía deja leerlos por la API directa; ese candado es F4e.)
...
export async function getInsumosDelTaller(tallerId: string, opciones: { conCostos: boolean; hoy: string }): Promise<InsumosDelTaller> {
  ...
  const [insumos, lotes, movs] = await Promise.all([
    supabase
      .from("insumos")
    ...
    supabase
      .from("insumo_lotes")
      .select("id, insumo_id, codigo_lote, fecha_ingreso, created_at, cantidad_ingresada, costo_unitario, documento, origen")
      .eq("ubicacion_id", tallerId),
    supabase
      .from("movimientos_insumo")
      .select("id, insumo_id, insumo_lote_id, tipo, cantidad, costo_unitario, produccion_id, motivo, created_at, produccion:producciones ( producto:productos ( referencia ) )")
      .eq("ubicacion_id", tallerId)
  ...
  const lotesBase = filasLotes.map((l) => ({
    ...
    costoUnitario: opciones.conCostos ? Number(l.costo_unitario) : null,
