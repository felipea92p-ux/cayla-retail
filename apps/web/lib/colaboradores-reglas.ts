...
/** Cómo se lee una cuenta terminal en la tabla (ADR-0159). */
 ...
 *  de PERSONAS y una cuenta compartida no es una persona (ADR-0159) — se resta antes de repartir entre las dos. */
...
export function accionesDeFila(c: Pick<Colaborador, "rol" | "es_yo" | "terminal">): AccionFila[] {
  if (c.es_yo) return [];
  // Una terminal no se muda: está fija a UNA tienda por diseño (ADR-0159) y `agregar_terminal` solo acepta tiendas. Para
