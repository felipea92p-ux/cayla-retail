...
export type AccionesRoles = {
  ...
  guardarModulos: (rolId: string, modulos: readonly ClaveModulo[], firma: Firma | null) => Promise<ResultadoRol>;
...
export const accionesRolesSupabase: AccionesRoles = {
  ...
  guardarModulos: async (rolId, modulos, firma) => {
    const { error } = await firmar(createClient().rpc("guardar_modulos_rol", { p_rol_id: rolId, p_modulos: [...modulos] }), firma);
    return { error };
  },
