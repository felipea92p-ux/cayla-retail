...
// y todo archivo "use server"— y lo recorre a lo ancho. En cada archivo "use client" se detiene:
...
//  - `import * as ns` y sus llamadas `ns.f()` no se rastrean.
 ...
 *  también para `f!()`, `(f)()` y `f?.()`. Devuelve null si lo llamado no cuelga de un nombre
 *  (`(() => 1)()`, `a().b()` — el `a()` de adentro se revisa aparte, al recorrer el árbol). */
...
describe("Arquitectura — el servidor no llama funciones de archivos «use client»", () => {
  ...
  it("parte de las entradas reales del servidor (que el candado no mire el vacío)", () => {
    ...
    expect(accionesDeServidor.length).toBeGreaterThanOrEqual(3);
  });
  ...
  it("ningún módulo del servidor llama como función a algo que vive en un archivo «use client»", () => {
    expect(real.violaciones.map(mensajeDe).join("\n\n")).toBe("");
  });
});
