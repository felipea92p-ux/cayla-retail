...
import { conStockReleido } from "@/lib/vender-stock-local";
...
export async function leerStockDeSede(ubicacionId: string, conocidos: string[], ids?: string[]): Promise<Map<string, number> | null> {
  ...
  return conStockReleido(new Map(), pedidas, sumarCantidades(data));
}
 ...
 * pantalla decide dónde aterriza (su propio `ajustesStock`)—; una lectura que falla no llama a `alLeer`, la
 ...
 */
export function useStockEnVivo(ubicacionId: string, conocidos: string[], activo: boolean, alLeer: (releido: Map<string, number>) => void, cadaMs = 10_000) {
  ...
  useEffect(() => {
    ...
    const sondear = async () => {
      ...
      if (releido && !cancelado) alLeerRef.current(releido);
    };
    void sondear();
