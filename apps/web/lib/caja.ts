...
export type CierreCaja = {
  ...
  esPrueba: boolean;
  /** Lo que quedó en el cajón al cerrar (ADR-0183). `null` en cierres anteriores a ADR-0183. */
  montoFondo: number | null;
  /** A dónde fue el resto del efectivo contado (ADR-0183). Vacío si todo quedó en el cajón. */
  traslados: TrasladoCaja[];
  /** Con qué monto debió abrir según el cierre anterior, y por qué abrió con otro (ADR-0183). */
...
/** Columnas y tabla de ADR-0183: si la migración aún no está pegada, la pantalla sigue sin ellas. */
const TABLA_INEXISTENTE = "42P01";

/**
 * Lo de ADR-0183 de cada caja (fondo, apertura esperada, motivo y traslados), pedido aparte y tolerante: si la
...
/**
 * Aperturas que no coincidieron con el último cierre y que ningún líder marcó todavía como revisadas (ADR-0183).
