  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="label-cayla text-[11px] text-tinta/65">Finanzas · Estados financieros</p>
          <h1 className="font-display mt-1 text-2xl text-tinta">{MESES[mes - 1]} {anio}</h1>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/finanzas/balances?m=${mesPrevio}`} className="label-cayla rounded-md border border-tinta/20 px-3 py-2 text-[11px] text-tinta/75 transition-colors hover:border-rojo hover:text-rojo">
            ← {MESES[(mes + 10) % 12]}
          </Link>
          {!esMesActual && (
            <Link href={`/finanzas/balances?m=${mesSiguiente}`} className="label-cayla rounded-md border border-tinta/20 px-3 py-2 text-[11px] text-tinta/75 transition-colors hover:border-rojo hover:text-rojo">
              {MESES[mes % 12]} →
            </Link>
          )}
        </div>
      ...
      <section>
        ...
        <p className="mb-3 text-xs text-tinta/65">Lo que ganó o perdió el negocio en {MESES[mes - 1]}.</p>
        <div className="card-cayla">
          <div className={fila}>
            <span className="text-tinta/75">Ventas netas
          ...
          <div className={fila}>
            <span className="text-tinta/65">(−) Costo de ventas
            ...
            <span className="text-tinta/75">{money(-e.eerr.costoVentas)}</span>
          </div>
          <div className={fila}>
            <span className="text-tinta/65">(−) Fletes de compra
            ...
            <span className="text-tinta/75">{money(-e.eerr.fletes)}</span>
          </div>
          <div className={fila}>
            <span className="text-tinta/65">(−) Mermas
              <Ayuda titulo="Mermas">
            ...
            <span className="text-tinta/75">{money(-e.eerr.mermas)}</span>
          </div>
          <div className={filaTotal}>
            <span className="text-tinta">Margen bruto
              ...
              {e.eerr.margenBrutoPct != null && <span className="ml-2 text-tinta/65">{e.eerr.margenBrutoPct}%</span>}
            </span>
          ...
          {e.eerr.gastosOperativos.map((g) => (
            <div key={g.categoria} className={fila}>
              <span className="text-tinta/65">(−) {ETIQUETA_GASTO[g.categoria] ?? g.categoria}</span>
              <span className="text-tinta/75">{money(-g.monto)}</span>
            </div>
          ))}
          <div className={`${filaTotal} bg-crema`}>
            <span className="label-cayla text-[11px] text-tinta">Utilidad operativa
      ...
      <section>
        <div className="flex items-baseline justify-between">
          ...
          <span className={`label-cayla text-[11px] ${e.balance.cuadra ? "text-tinta/65" : "text-rojo"}`}>
        ...
        <p className="mb-3 text-xs text-tinta/65">Cuánto vale CAYLA hoy. Activo = Pasivo + Patrimonio.</p>
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="card-cayla">
            <p className="label-cayla border-b border-tinta/10 px-4 py-2 text-[11px] text-tinta/65">Activo — lo que tiene</p>
            <div className={fila}><span className="text-tinta/75">Caja (efectivo en tiendas)</span><span className="text-tinta">{money(e.balance.caja)}</span></div>
            <div className={fila}><span className="text-tinta/75">Banco y medios electrónicos</span><span className="text-tinta">{money(e.balance.banco)}</span></div>
            <div className={fila}>
              <span className="text-tinta/75">Inventario (a costo)
            ...
            <div className={fila}><span className="text-tinta/75">Muebles y equipos</span><span className="text-tinta">{money(e.balance.activosFijos)}</span></div>
            <div className={`${filaTotal} bg-crema`}><span className="label-cayla text-[11px] text-tinta">Total activo</span><span className="font-display text-base text-tinta">{money(e.balance.totalActivo)}</span></div>
          </div>
          <div className="card-cayla">
            <p className="label-cayla border-b border-tinta/10 px-4 py-2 text-[11px] text-tinta/65">Pasivo y Patrimonio — cómo se financia</p>
            <div className={fila}>
              <span className="text-tinta/75">IGV por pagar (SUNAT)
            ...
            <div className={fila}><span className="text-tinta/75">Otras deudas</span><span className="text-tinta">{money(e.balance.pasivosManuales)}</span></div>
            <div className="flex items-center justify-between px-4 py-2 text-sm text-tinta/65"><span>Total pasivo</span><span>{money(e.balance.totalPasivo)}</span></div>
            <div className={fila}>
              <span className="text-tinta/75">Capital y aportes
            ...
            <div className={fila}>
              <span className="text-tinta/75">Utilidades acumuladas
            ...
            <div className={`${filaTotal} bg-crema`}><span className="label-cayla text-[11px] text-tinta">Total pasivo + patrimonio</span><span className="font-display text-base text-tinta">{money(e.balance.totalPasivo + e.balance.totalPatrimonio)}</span></div>
          </div>
        </div>
      ...
      <section>
        ...
        <p className="mb-3 text-xs text-tinta/65">De dónde vino y a dónde se fue el dinero en {MESES[mes - 1]}.</p>
        <div className="card-cayla">
          <div className={fila}><span className="text-tinta/75">Efectivo al inicio del mes</span><span className="text-tinta">{money(e.flujo.efectivoInicial)}</span></div>
          <div className={fila}><span className="text-tinta/65">(+) Cobros de ventas</span><span className="text-tinta/75">{money(e.flujo.cobrosVentas)}</span></div>
          <div className={fila}><span className="text-tinta/65">(−) Pagos de gastos</span><span className="text-tinta/75">{money(-e.flujo.pagosGastos)}</span></div>
          {e.flujo.ajustes !== 0 && <div className={fila}><span className="text-tinta/65">(±) Ajustes de efectivo</span><span className="text-tinta/75">{money(e.flujo.ajustes)}</span></div>}
          ...
          <div className={`${filaTotal} bg-crema`}><span className="label-cayla text-[11px] text-tinta">Efectivo al final del mes</span><span className="font-display text-xl text-tinta">{money(e.flujo.efectivoFinal)}</span></div>
        </div>
      </section>
      ...
      <p className="border-t border-tinta/10 pt-4 text-xs text-tinta/65">
