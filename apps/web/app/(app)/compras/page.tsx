...
// Emisión (+ condición/vencimiento) · Proveedor (+ RUC) · N.° documento ·
// Recepción · Pago · Total · (flecha). La última columna es solo la flecha
// "›": la fila entera es un enlace y sin una señal visible nadie sabía que
// se podía abrir. En celular no hace falta (la fila apilada ya se ve como
// tarjeta) y se oculta.
...
const PLANTILLA = "sm:grid-cols-[8rem_1fr_7rem_9rem_7.5rem_8.5rem_1rem]";
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Indicador
          ...
          tono={resumen.deuda > 0 ? "ambar" : "verde"}
        ...
        <Indicador
          ...
          tono={resumen.porRecibir > 0 ? "ambar" : "verde"}
          ...
          {compras.map((c) => (
            <Link key={c.id} href={`/compras/factura/${c.id}`} className={fila(PLANTILLA, "group transition-colors hover:bg-tinta/[0.03] focus-visible:bg-tinta/[0.03] focus-visible:outline-none")}>
              <span className={celda("izq", "whitespace-normal text-xs tabular-nums text-tinta/65")}>
                <span className="block">{fechaCorta(c.fechaEmision)}</span>
                {c.condicion === "credito" && c.fechaVencimiento ? (
                  <span className={`block ${c.vencida ? "text-rojo" : "text-tinta/45"}`}>
                    {c.vencida ? "Venció" : "Vence"} {fechaCorta(c.fechaVencimiento)}
                  </span>
                ) : (
                  <span className="block text-tinta/45">Contado</span>
                )}
              </span>
...
// El punto de color adelanta el estado de la tarjeta antes de leer la
// cifra — mismo vocabulario semántico que los Chip de la tabla (verde =
// va bien, ámbar = a medias/pendiente, rojo = hay que actuar), nunca un
// color nuevo: el brandbook es tinta/crema/rojo + verde/ámbar, no más.
const PUNTO_TONO = { neutro: "bg-tinta/25", ambar: "bg-ambar", verde: "bg-verde" } as const;
...
function Indicador({
  ...
  detalle: string;
  alerta?: boolean;
  tono?: keyof typeof PUNTO_TONO;
  href?: string;
}) {
  const contenido = (
    <>
      <div className="flex items-center gap-1.5">
        <span aria-hidden className={`h-1.5 w-1.5 shrink-0 rounded-full ${alerta ? "bg-rojo" : PUNTO_TONO[tono]}`} />
