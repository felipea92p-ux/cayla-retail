...
export default async function NuevaCompraPage({ searchParams }: { searchParams: Promise<{ prov?: string }> }) {
  ...
  // ADR-0151 (F5): un comprador de tienda registra con SU tienda como gestora (el backend lo exige: la gestora tiene
  // que tener parte en el reparto). `persona.ubicacionId` es la sede fija de la persona (`colaboradores`), que puede
  // no coincidir con dónde compra (`tiendasCompra`) — por eso el formulario necesita las dos por separado.
  ...
  return (
    <CompraFormV2
      ...
      // ADR-0151: solo para un comprador — el líder gestiona con cualquier tienda (`undefined` es «sin restricción»,
