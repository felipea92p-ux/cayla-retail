...
export default async function AlmacenPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <Link href="/" className="text-xs text-neutral-400 hover:underline">← Volver</Link>
        <h1 className="mt-1 text-lg font-semibold text-neutral-900">Almacén {almacen.codigo}</h1>
        <p className="text-xs text-neutral-400">{items.length} referencias con stock</p>
      </div>

      <Link href="/almacen/recibir" className="inline-block text-sm text-neutral-500 hover:text-neutral-900 hover:underline">
        + Recibir mercadería
      </Link>
