...
export default async function InicioPage() {
  ...
  const [productos, variantes, stock, movimientosRecientes] = await Promise.all([
    ...
    getMovimientos(persona.ubicacionId, 8),
  ]);
            ...
            {movimientosRecientes.map((m) => (
              <div key={m.id} className="flex items-baseline gap-3 px-5 py-2.5">
                <span className="w-16 shrink-0 text-xs tabular-nums text-tinta/65">
                  {new Date(m.creadoEn).toLocaleDateString("es-PE", { day: "2-digit", month: "2-digit" })}
                </span>
                <span className="min-w-0 flex-1 text-sm text-tinta">
                  <span className="label-cayla text-[11px] text-tinta/65">{m.tipo}</span> {m.referencia}{" "}
                  <span className="text-tinta/65">{m.sku} × {m.cantidad}</span>
                </span>
              </div>
            ))}
          </div>
