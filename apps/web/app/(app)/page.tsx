...
function SeccionHoy({ hoy, esLider }: { hoy: HoyDeLaSede; esLider: boolean }) {
  ...
  return (
    <div>
      ...
      <div className={`grid grid-cols-1 gap-3 ${esLider ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}>
        <Tarjeta etiqueta={esLider ? "Ventas" : "Tus ventas"} valor={formatoSoles(r.importe)}>
        ...
        {esLider ? (
          <>
            <Tarjeta etiqueta="Valor medio de venta" valor={formatoSoles(r.valorMedio)}>
              {r.ventas} {r.ventas === 1 ? "venta" : "ventas"}
            </Tarjeta>
            {r.meta && (
          ...
          <Tarjeta etiqueta="Caja" valor={hoy.cajaAbierta === null ? "—" : hoy.cajaAbierta ? "Abierta" : "Cerrada"}>
            {hoy.cajaAbierta === null ? "No pude leer esto" : hoy.cajaAbierta ? "Puedes vender" : "Ábrela en Caja para vender"}
          </Tarjeta>
        )}
      </div>
    </div>
...
function Tarjeta({ etiqueta, valor, children }: { etiqueta: string; valor: string; children?: React.ReactNode }) {
  return (
