...
| **D-E** | Cotización de maquila externa (mitad de D-31) | ✅ **Revivida por Felipe (2026-09-21, en la ronda de 60 preguntas — ver `docs/datos/DECISIONES-2026-09-21-menu-comercial.md`, D-82), sobre la enmienda que la había descartado más temprano ese mismo día.** Construida: tabla `retail.cotizaciones_maquila` (por **tipo de prenda** vía `categoria_id`, no por modelo — `fn_cotizacion_maquila_vigente`) en el PR #269 (fusionado). **Sigue sin usarse**: F7/Eficiencia no la llama todavía — cuando se conecte, el Taller se compara contra el mercado ADEMÁS de contra sí mismo, no en vez de | Dos sesiones el mismo día decidieron cosas contrarias; la más reciente manda. Costo pendiente: conectar `fn_cotizacion_maquila_vigente` a la vista de Eficiencia | esquema: hecho, sin conectar |
