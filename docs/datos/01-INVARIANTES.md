...
> (no hay índice único a propósito: un mismo dueño puede tener dos fichas legítimas). Ver la sección 2 y ADR-0134.
...
| **Editar una caja ya cerrada** | Que la política de `update` mire el estado | ~~`cajas_update` en producción permite editar cualquier caja de tu sede~~ **Cerrado** (verificado 2026-09-23, solo lectura): en producción `retail.cajas` solo tiene `cajas_select`; sin política de UPDATE nadie reescribe un cierre por la API. En la base local tampoco existe (solo `cajas_select`); la vieja era `cajas_update_propia_sede`, `0007_finanzas.sql:66` | **Las dos** | El cierre de caja es el tercero de los tres números de **D-52**, y hoy se puede reescribir después de contado |
