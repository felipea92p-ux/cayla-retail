...
| Dos variantes iguales del mismo producto (misma talla, mismo color) | Índice único sobre la identidad: talla normalizada ("M" = "m "), color por código, y "sin talla"/"sin color" cuentan como un valor más | `variantes_identidad_unica` — `(producto_id, fn_clave_texto(talla), color_codigo) nulls not distinct` (V2: `20260916190000_variantes_identidad_unica.sql`, ADR-0069; reemplaza `variantes_producto_id_talla_color_codigo_key`, que comparaba la talla como texto exacto) | **Local**; producción al pegar `SQL-PENDIENTE-PRODUCCION-2026-09-16-loro.sql` | "Blusa Aurora M vino" existiendo dos veces, con el stock repartido. La clienta la ve agotada mientras hay 4 en la otra fila |
