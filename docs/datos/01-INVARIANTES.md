...
| Correr dos veces la misma importación de catálogo | Índice único parcial | `importaciones_token_unico` (solo cuando `token` no es nulo) | **Ninguna** (borrado el 2026-09-12 con el corte a V2; Golondrina se reconstruye después del censo) | 900 prendas duplicadas de una sola pasada, y cada una con su stock propio |
| Dos versiones de la taxonomía activas | Índice único parcial | `taxonomia_una_sola_activa` | **Ninguna** (borrado el 2026-09-12 con el corte a V2; Tucán se reconstruye después del censo) | Dos traducciones distintas del mismo catálogo conviviendo — **ADR-0030** |
