...
| Dos variantes iguales del mismo producto (misma talla, mismo color) | Índice único sobre la identidad, tratando el vacío como vacío | `variantes_identidad_unica` — `(producto_id, coalesce(talla,''), coalesce(color,''))` | **Las dos** | "Blusa Aurora M vino" existiendo dos veces, con el stock repartido. La clienta la ve agotada mientras hay 4 en la otra fila |
