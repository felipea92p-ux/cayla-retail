-- ============================================================================
-- SQL PENDIENTE DE PRODUCCIÓN · 2026-09-16 · Loro (módulo 02) · editar sin SKU
--
-- QUÉ ARREGLA EN TIENDA
--   Una prenda cargada por /productos/nuevo (censo) nace sin SKU, y la ficha de
--   edición la rechazaba con "Cada variante necesita un SKU": no se le podía
--   corregir el precio, subir la foto, cambiar la categoría ni descontinuarla.
--   Migración 20260916193000_catalogo_sku_opcional (mismo SQL, copiado tal cual).
--
-- Medido contra producción (proyecto cayla-dynamic, schema retail) el 2026-09-16,
-- solo lectura:
--   · pg_get_functiondef de catalogo_crear_producto y catalogo_actualizar_producto
--     tiene el mismo código que el repo (20260915224500) — misma firma, misma regla
--     "Cada variante necesita un SKU"; solo faltan comentarios internos.
--   · variantes.sku ya es nullable y único (variantes_sku_key); 0 variantes con sku
--     NULL y 0 con sku '' todavía.
--
-- Cómo pegar: en el SQL Editor de producción, UN bloque por vez y en orden.
-- Todo es repetible: pegar dos veces no falla ni duplica nada. No borra datos ni
-- toca filas: solo reemplaza las dos funciones (mismos permisos, misma firma).
-- Independiente de SQL-PENDIENTE-PRODUCCION-2026-09-16-loro.sql: puede ir antes o
-- después.
-- ============================================================================


-- ---------- 0. PRE-FLIGHT — lo esperado está a la derecha de cada columna ----------
-- Si total_firmas no es 2, NO sigas: hay una sobrecarga vieja viva (ADR-0026) y
-- `create or replace` crearía una tercera en vez de reemplazar.
select
  to_regprocedure('retail.catalogo_crear_producto(text,jsonb,uuid,text,integer,text,boolean,jsonb)') is not null                as firma_crear,       -- true
  to_regprocedure('retail.catalogo_actualizar_producto(uuid,text,text,jsonb,uuid,text,integer,text,boolean,jsonb)') is not null as firma_actualizar,  -- true
  (select count(*) from pg_proc
     where pronamespace = 'retail'::regnamespace
       and proname in ('catalogo_crear_producto', 'catalogo_actualizar_producto'))                                                   as total_firmas;      -- 2


-- ---------- 1. Migración 20260916193000_catalogo_sku_opcional ----------
begin;
...
values ('20260916193000', 'catalogo_sku_opcional')
on conflict (version) do nothing;
commit;


-- ---------- 2. COMPROBACIÓN — lo esperado está a la derecha de cada columna ----------
select
  (select count(*) from pg_proc
     where pronamespace = 'retail'::regnamespace
       and proname in ('catalogo_crear_producto', 'catalogo_actualizar_producto')
       and prosrc like '%necesita un SKU%')                                          as funciones_que_exigen_sku, -- 0
  (select count(*) from pg_proc
     where pronamespace = 'retail'::regnamespace
       and proname in ('catalogo_crear_producto', 'catalogo_actualizar_producto')) as total_firmas,             -- 2
  (select count(*) from supabase_migrations.schema_migrations
     where version = '20260916193000')                                             as migracion_registrada,     -- 1
  (select count(*) from retail.variantes where sku = '')                           as variantes_con_sku_vacio;  -- 0
