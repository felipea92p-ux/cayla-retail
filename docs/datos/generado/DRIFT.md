...
> Comparadas 69 llamadas de `apps/web` contra 120 funciones del schema `retail` en producción.
...
- `emitir_comprobante` · `apps/web/components/ComprobantesPanel.tsx:344` — no manda `p_venta_id`, `p_items` (normal si tienen valor por defecto)
- `registrar_serie_comprobante` · `apps/web/components/ComprobantesPanel.tsx:369` — no manda `p_siguiente_numero` (normal si tienen valor por defecto)
...
- `registrar_venta` · `apps/web/components/PuntoDeVenta.tsx:659` — los parámetros no van escritos ahí mismo
...
- `fn_prioridad_conteo` · `apps/web/lib/conteos.ts:185` — el objeto se arma con «...», no se puede leer entero
