...
function cajaDe(u){
  ...
  const factor = navidad ? 1.5 : 1;                                  // un viernes de diciembre vende más
...
VISTAS.caja = () => {
  ...
  const demoFecha = `<label class="ver"><span class="etq">Demo · día</span><select class="control" style="min-width:min(270px, 62vw)" data-cambia="fechaCaja">
      <option value="${HOY}"${E.fechaCaja===HOY?' selected':''}>Hoy, jueves 24 sep (lo normal)</option>
      <option value="2026-12-11"${E.fechaCaja!==HOY?' selected':''}>Viernes 11 dic (Campaña de Navidad)</option></select></label>`;
  return `
  ...
  <section class="superficie pad anim-sube meta-caja">
    <div class="prioridades-cab"><div><h2>Meta de hoy · ${S(p.meta)}</h2><p>${p.temporada?`${esc(p.temporada.n)}: lo normal de un ${DIAS_L[diaSemana(E.fechaCaja)]} es ${S(p.metaBase)}, esta temporada ${p.pct>0?'+':''}${p.pct} %.`:`Lo normal de un ${DIAS_L[diaSemana(E.fechaCaja)]} en ${nombreUnidad(u)}.`}</p></div>
      ${p.temporada?`<span class="badge" data-tono="ambar">${esc(p.temporada.n)}</span>`:''}</div>
    ...
    ${F('existe','ubicaciones.meta_venta_diaria (barra de hoy)')} ${F('nuevo','fn_parametros_caja: día de la semana + temporada')}
  </section>
  <section class="dos-col par">
    ...
    <div class="superficie pad anim-sube">
      ...
      <p class="sub" style="font-size:12.5px;margin:12px 0 0">${p.temporada?`El fondo sube a ${S(p.fondo)} por la ${esc(p.temporada.n)}: hay más ventas en efectivo y se necesita más sencillo.`:'El fondo lo pone el líder en Configuración ▸ Tiendas y caja, y cambia por temporada.'}</p>
    </div>
  </section>`;
};
...
function modalCerrarCaja(u){
  ...
  ventana(`<h3>Cerrar caja de ${nombreUnidad(u)}</h3><p class="bajada">Cuenta el efectivo del cajón. Lo que no se traslada se queda para el próximo turno.</p>
    ...
    <div class="fondo-pide"><b>Deja ${S(p.fondo)} para el próximo turno</b><span>${p.temporada?esc(p.temporada.n):'Lo normal de '+nombreUnidad(u)}</span></div>
  ...
  $('#okCC').onclick = () => {
    ...
    conf.innerHTML = `<b>Vas a dejar ${S(q)} y ${p.temporada?'esta temporada':'lo normal'} pide ${S(p.fondo)}.</b><p>El próximo turno puede quedarse sin sencillo. Si cierras igual, queda anotado en el cierre.</p>
