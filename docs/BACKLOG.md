...
## 🎯 «Quién vendió» en el ticket del Punto de venta (2026-09-22, ADR-0161) — rehecho sobre la asistencia de Dynamic; falta pegar 1 migración
Un solo equipo de caja y varias colaboradoras por tienda. La fila «Atendió» ofrece a quienes marcaron entrada hoy en Dynamic (`fn_asesoras_de_turno`) y la venta se guarda en `ventas.asesora_id` — ambas ya en producción desde la 20260922150000 (ADR-0153). Decisión en [docs/adr/0161-vendedora-en-el-ticket.md](adr/0161-vendedora-en-el-ticket.md).
...
- [ ] **Pegar `20260922213700_ventas_del_dia_firma_con_quien_atendio.sql` en producción** (con OK de Felipe; `create or replace`, misma firma). Sin ella la venta se guarda bien pero «Ventas de hoy» sigue firmando con la sesión.
- [ ] **Verificar en el navegador contra producción** (local no tiene asistencia de Dynamic, la fila no aparece): en TRU con gente presente, chips; en AQP sin marcas, el aviso «Nadie marcó asistencia hoy».
- [ ] Base local desfasada: todavía tiene `vendedora_id`/`atiende_en_caja` de la versión vieja y le faltan la 140000 y la 150000 de main, así que `pnpm pruebas:vendedora-en-venta` no corre hasta sincronizarla (retirar lo viejo es destructivo sobre la base compartida: pedir OK).
