...
## 🎯 «Quién vendió» en el ticket del Punto de venta (2026-09-21, ADR-0161) — hecho en local, falta pegar en producción
Un solo equipo de caja y varias colaboradoras por tienda (TRU: 11 activas, 6 venden). Spec `docs/superpowers/specs/2026-09-21-vendedora-en-el-ticket-design.md`, plan `docs/superpowers/plans/2026-09-21-vendedora-en-el-ticket.md`, decisión en [docs/adr/0161-vendedora-en-el-ticket.md](adr/0161-vendedora-en-el-ticket.md).
...
- [x] **Verificado en navegador (2026-09-22)**, con dos colaboradoras de prueba en Trujillo: bloqueo con 2+ marcadas sin elegir, «Atiende X» sin bloquear con 1 sola, cobro real firma la venta con la vendedora (no la sesión) en Vender/Caja/Historial, y el ticket térmico + la boleta A4 imprimen «Atendió». Encontró (y quedó anotado aparte, sin arreglarlo aquí) que Lima y Trujillo comparten código de serie de comprobante con contadores independientes.
- [ ] **Pegar `20260922143700_vendedora_en_la_venta.sql` en producción ANTES de fusionar la web** (con OK explícito de Felipe; entera y en una transacción). Antes: la huella de `registrar_venta` y `fn_ventas_del_dia` en producción debe ser la del repo. Después: una sola sobrecarga de `registrar_venta` y `proacl = {postgres=X/postgres,authenticated=X/postgres}` en las cuatro funciones. Luego, un líder marca a las 6 de TRU y se refresca el volcado de producción.
