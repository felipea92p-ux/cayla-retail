...
## 🎯 «Quién vendió» en el ticket del Punto de venta (2026-09-21, ADR-0152) — hecho en local, falta pegar en producción
Un solo equipo de caja y varias colaboradoras por tienda (TRU: 11 activas, 6 venden). Spec `docs/superpowers/specs/2026-09-21-vendedora-en-el-ticket-design.md`, plan `docs/superpowers/plans/2026-09-21-vendedora-en-el-ticket.md`, decisión en [docs/adr/0152-vendedora-en-el-ticket.md](adr/0152-vendedora-en-el-ticket.md).
- [x] Código: `ventas.vendedora_id` junto a `usuario_id`, `colaboradores.atiende_en_caja`, `registrar_venta` con `p_vendedora_id`, fila «Atendió» sin preselección, modal del líder, «Atendió:» en el ticket térmico y los reportes (historial, Cambios/Devoluciones, detalle de caja). 13 pruebas contra Postgres local (`pnpm pruebas:vendedora-en-venta`) + pruebas puras.
- [ ] **Pegar `20260922143700_vendedora_en_la_venta.sql` en producción ANTES de fusionar la web** (con OK explícito de Felipe; entera y en una transacción). Antes: la huella de `registrar_venta` y `fn_ventas_del_dia` en producción debe ser la del repo. Después: una sola sobrecarga de `registrar_venta` y `proacl = {postgres=X/postgres,authenticated=X/postgres}` en las cuatro funciones.
- [ ] Un líder marca a las 6 de TRU en «Elegir quiénes atienden» (Vender). Mientras nadie esté marcada, la fila no aparece y se vende como siempre.
- [ ] Refrescar el volcado de producción (`generado/COMO-REFRESCAR.md`) para que `vendedora_id`, `atiende_en_caja` y las 3 funciones entren al diccionario.
- [ ] Mover el interruptor a `/colaboradores` cuando esa pantalla se asiente (hoy vive en el Punto de venta). Decidir si los líderes que atienden en mostrador deben poder figurar como vendedora (hoy no: no tienen sede asignada).
