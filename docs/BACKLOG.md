...
- 🔒 **El costo de una prenda solo para quien ve el dinero (ADR-0183, Felipe: opción C + regla A):** la base cerraba mal `variantes.costo` (cualquier sesión lo leía; Conteo mandaba los 1.295 costos al navegador; `fn_productos`, `fn_resumen_comparacion`, `fn_costo_historial` y `censo_crear_variante` lo devolvían sin preguntar). Migración `20260923193700` + web por la puerta `fn_costos_variantes_json`. Ensayada en producción como líder, colaboradora y terminal (tabla en el ADR). **Falta: OK de Felipe para pegarla; ORDEN: primero desplegar la web, después pegar** (la web vieja pide `costo` a la tabla y tumbaría la caja). Pendiente también: resultados de ADR-0181 en producción (copia del catálogo: 0 lecturas tras 5 visitas, se renueva sola).
