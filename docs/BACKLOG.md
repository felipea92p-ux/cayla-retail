...
Rama `claude/billing-design-analysis-ee464b`. Spec aprobado por Felipe el 2026-09-19: `docs/superpowers/specs/2026-09-18-facturacion-cuatro-vistas-design.md`. Plan: `docs/superpowers/plans/2026-09-19-facturacion-cuatro-vistas-r0-r1.md` (R0 y R1 al detalle; R2 a R4 se hicieron directo, por rebanadas). **Estado al 2026-09-21: R1 y las cuatro vistas con su look (R2 rebanadas A a C, R3 y R4) construidas y verificadas en el navegador contra la base local, sin push.** `main` está fusionada (Atelier entró a `main` el 2026-09-19, ADR-0123). Falta la franja de proformas del Resumen, el OK de Felipe a la migración de `fn_ventas_del_dia` y el cierre: ver «Falta para cerrar Facturación» más abajo.
...
- [ ] **R2 — Resumen** (no se cierra hasta la migración de `fn_ventas_del_dia` y la franja de proformas): tarjetas de vidrio, «Actividad de hoy», comparativo «mismo día de la semana pasada, a esta hora», línea viva y buscador.
  ...
  - [ ] **Pegarla en producción** (con el OK explícito de Felipe): un solo objeto y una sola firma; el archivo ya trae su `set search_path`. Hoy es idéntica en salida (14 ventas, todas `completada`, ninguna con dos comprobantes al 2026-09-21). Huella del cuerpo (md5 sin espacios) en producción antes: `9f564c0f0a7f93cee69e7ff389972de4` (igual a la del repo); después debe ser la de local, `7a29927aeea6a81a0b85d8fe890a7c9d`. Reversible: el cuerpo anterior está en `20260917100800_talla_texto_en_lecturas.sql`. Después, refrescar el volcado (`pnpm datos:generar:produccion`). El frontend no depende del orden: `ventasUnicas` sigue defendiendo las tarjetas si la base es anterior, y la referencia de la semana pasada ya filtraba `completada`. **Es lo único que falta para cerrar R2 desde la base.**
...
- [ ] **OK para pegar en producción la migración de `fn_ventas_del_dia`** (arriba: escrita, probada y aplicada en local; producción no se toca sin tu OK explícito).
- [ ] **`anular_venta` deja el comprobante `pendiente` (hallado al probar la migración, en local y en producción).** Anular una venta con boleta pendiente no toca esa boleta: sigue `pendiente`, cuenta en «Por enviar a SUNAT» y en Comprobantes tiene su botón *Transmitir*; nada en `/api/lucode/emitir` ni en `actualizar_transmision_comprobante` mira que la venta esté anulada, o sea que se puede declarar a SUNAT una venta que ya se devolvió. `anular_venta` solo se niega si el comprobante ya está `enviado` o `aceptado`. La salida natural es que anular una venta libere su comprobante `pendiente` (`no_emitido`, como «Liberar sin espera», ADR-0093) en la misma transacción; falta decidir qué hacer con uno `rechazado`. Es cambio de `anular_venta` en producción: se propone, no se hace.
