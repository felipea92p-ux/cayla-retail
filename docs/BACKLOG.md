...
- [ ] **`emitir_comprobante` no mira si la venta está anulada** (hallado el 2026-09-21 leyendo su definición en producción, solo lectura): acepta cualquier `p_venta_id`, también el de una venta anulada, y ni siquiera comprueba que la venta sea de la misma `p_ubicacion_id`; lo mismo `convertir_proforma_a_comprobante`, que le pasa su `p_venta_id` opcional sin mirar `ventas` (un solo guard en `emitir_comprobante` cubre a las dos). La lista de Facturación ya no ofrece ventas anuladas (`fn_ventas_del_dia`, `20260921103000`) y `/api/lucode/emitir` se niega a transmitir su comprobante, así que hoy solo se llega llamando la RPC a mano o por una carrera con la anulación (el `for update` de `anular_venta` sobre la venta hace esperar al `insert`, que entra apenas se anula y deja un `pendiente` sobre una venta anulada). Arreglo pequeño: un guard `for share` sobre la venta al principio de `emitir_comprobante` (venta no anulada y de esa ubicación), con su prueba con las RPC reales. Es otra migración a producción: necesita el OK de Felipe.
