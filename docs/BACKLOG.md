...
## 🐛 Bug real: `notas_credito_tablero()` y `compras_nota_pendiente()` duplican un cierre ya resuelto (hallado 2026-09-22, por la siembra de 90 días — ver ADR-0150)
- [ ] Un cierre de línea de compra (`compra_item_cierres`) ya resuelto con una nota de crédito de motivo `devolucion`/`descuento`/`otro` sigue apareciendo como «pendiente, falta la nota» para siempre. Las dos funciones filtran «¿esta COMPRA ya tiene alguna nota con motivo `faltante`?» en vez de «¿este CIERRE concreto ya tiene alguna nota (cualquier motivo)?» — comparan al nivel equivocado. `fn_insertar_nota_credito_compra` sí permite legítimamente resolver un cierre con esos otros motivos sin exigir la compra resuelta (solo `faltante` lo exige), así que cualquier líder real puede producir hoy este estado.
- [ ] Consecuencia: la pantalla de Notas de Crédito y `compras_nota_pendiente()` (que alimenta «Por pagar»/listas de Compras) muestran el mismo dinero dos veces: una vez como nota ya aplicada, otra como «esperando nota» — sobrestima cuánto crédito falta reclamar a proveedores.
- [ ] Arreglo mínimo (no aplicado — necesita el OK de Felipe, es una migración de producción): en ambas funciones, cambiar `not exists (select 1 from compra_notas_credito n where n.compra_id = c.id and n.motivo = 'faltante')` por comparar contra `n.cierre_id = k.id` (o el `cierre_id` concreto de esa fila), sin filtrar por motivo.
- [ ] No confundir con un bug de la siembra de 90 días: el generador construye A5 correctamente (motivo `devolucion`, porque la compra no está resuelta); es la primera vez que hay compras reales suficientes para que este bug de la app se note.
