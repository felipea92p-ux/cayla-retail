...
- [x] **Por pagar: ↓ dentro de «Registrar pago» ya no cambia el cajón de comprobante** (arreglado el 2026-09-26, ADR-0128 act.). Las flechas ↑/↓ del cajón escuchaban en todo su `Dialog.Content`, y React hace subir los eventos de un portal hasta sus ancestros: ↓ en el monto, en una ficha o en un combo del pago pasaba al comprobante siguiente y el pago desaparecía con lo escrito. Ahora las cuatro vistas rápidas usan `useFlechasDelCajon` (`lib/vista-rapida-reglas.ts`, con pruebas y un candado que falla si un cajón vuelve a leer `ArrowDown` a mano): la flecha es del cajón solo si nació en su DOM, ningún control la usó, no viene de un campo y va sin Ctrl/⌘/Alt. Verificado en la pantalla real, en escritorio y a 375 px: sin el arreglo, ↓ con «100» escrito para Textiles Andina dejaba el cajón en la factura de Confecciones del Sur, con su «Pagar» listo.
- [x] **Por pagar: tocar el velo (o un texto) de «Registrar pago» ya no abre la vista rápida de la fila** (encontrado y arreglado el 2026-09-26, misma causa con clics). El «Pagar» de cada fila abre su modal dentro del árbol de React de la fila, y el `onClick` de la fila lo recibía: cancelar un pago tocando afuera abría el cajón de ese comprobante, y tocar un texto del pago lo abría encima del pago. La fila ahora ignora el clic cuyo destino no está en su DOM (`PorPagarLista.tsx`). Verificado en escritorio y a 375 px; las otras listas con vista rápida abren sus modales a nivel de pantalla y no lo tienen.
