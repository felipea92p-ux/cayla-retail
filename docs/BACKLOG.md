...
- [ ] **F1 construida, POR PEGAR (en TRES ejecuciones, PARTE 1 → 2 → 3; el primer intento, pegada entera, dio deadlock con el Asesor de seguridad y no aplicó nada — ADR-0195 «Corrección al pegar F1»):** migración `20260924210000_configuracion_meta_y_fondo_por_campana.sql` en producción **antes** de publicar la web (Configuración y la meta de Caja/Inicio llaman funciones nuevas; `cerrar_caja` no cambia). Después: cargar las metas por día y el fondo de cada tienda en Configuración, y refrescar el diccionario (`pnpm datos:generar:produccion`).
- [ ] **Siguen:** F2 (plan de cuentas + categorías de gasto + gastos y activos con `compras.naturaleza`), F3 (cuentas de dinero sellando la cuenta en las 22 situaciones), F4…F10.
