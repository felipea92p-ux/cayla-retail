...
## 🏋️ Prueba de carga: 15 a 50 a la vez sobre un año de datos (2026-09-24, ADR-0194) — migración `20260924180000` EN PRODUCCIÓN (Felipe la pegó el 2026-09-24; verificada en solo lectura); web sin cambios, PR #387
...
- [x] Pegada en producción (2026-09-24). Verificado: una firma por función, `registrar_cambio` con pre-bloqueo, `fn_ventas_del_dia` y `fn_productos` nuevas (con `force_custom_plan`), 2 índices, 0 políticas por fila, permisos intactos (anon sin acceso), y las dos lecturas responden sin error.
- [ ] Refrescar el diccionario (`pnpm datos:generar:produccion`) con el próximo volcado: 2 índices nuevos.
