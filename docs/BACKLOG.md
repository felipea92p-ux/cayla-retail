...
- [x] **Búsqueda única, atajos y KPI del encabezado** (2026-09-22, pedido de Felipe tras ver la maqueta Dynamic × Retail en `docs/maquetas/historial-ventas-2026-09/`): una sola barra «boleta, clienta o prenda» (`?q=`, resuelta en `resolverBusqueda` de `ventas-historial.ts` reutilizando `clasificarBusqueda` y `literalParaIlike` de `ventas-v2.ts` — mismo patrón de Cambios/Devoluciones, sin `.or()` entre tablas embebidas); atajos «Hoy», «Ayer» y «Pendientes de comprobante · N» (nuevo valor `comprobante = "pendiente"`, mismo embed `comp_filtro` que «con»); franja de 4 cifras en el encabezado (vendido, ventas, por día vendido — `porDiaVendido`, H5 de la auditoría — y pendientes de comprobante, único rojo de la pantalla). 51 pruebas de reglas (6 nuevas), typecheck y lint limpios. **Falta verlo con clics reales**: esta sesión no tuvo Supabase local disponible (el puerto de Docker lo tenía tomado otra sesión) — nadie ha probado el debounce de la barra, el toggle del atajo ni la cifra de pendientes contra datos de verdad.
- [ ] Enlaces «Ver historial →» desde «Ventas de hoy» (Caja, Punto de Venta, Facturación) y desde el detalle de un cierre (`?caja=`) — esperar a que salgan los rediseños en curso de esas pantallas.
