...
- [ ] **Pegar `20260922100000_colaboradores_endurecimiento.sql` en producción** (con ok de Felipe; antes: `select count(*) from retail.colaboradores where rol = 'colaborador' and ubicacion_asignada_id is null` debe dar 0). Cubre #2, #4 y #5. Correr antes en local `pnpm pruebas:colaboradores-endurecimiento --en-seco` (escrita, **nunca corrida**: faltó Docker).
