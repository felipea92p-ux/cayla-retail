...
## 🎯 Prendas sin registrar: se venden con rastro y almacén las regulariza (2026-09-23, ADR-0179) — CONSTRUIDO y verificado en local, rama `claude/untagged-products-pos-fe22a4`; NO está en producción
...
- [ ] **Pegar las dos migraciones en producción, en orden** (con OK de Felipe): verificar antes contra `pg_proc` que `registrar_venta`/`anular_venta` tengan una sola sobrecarga y que la versión viva sea la de `20260922150000`/`20260922151500` (si otra migración las cambió después, rehacer la copia sobre la viva). Después, `select retail.fn_rls_una_vez_por_consulta();` y refrescar el diccionario.
- [ ] Fusionar la rama a `main` (el ADR-0178 lo tomó «escalón admin» mientras tanto: esta rama ya usa 0179).
