...
## 🎯 Compras: cada tienda compra, ve y paga lo suyo (2026-09-21, ADR-0151) — ACEPTADO, F1–F5 en local (falta «parte nueva»/F3-b)
...
- [x] **F1 paso 1 — el permiso (aplicada en el Postgres local compartido; no en producción):** `20260922120000_compras_compradores_de_tienda.sql` crea `compradores_de_tienda`, `fn_compras_ubicaciones()`, `fn_puede_comprar_en()` y `agregar_/quitar_comprador_de_tienda` (solo líder). **No abre ninguna puerta**: con la tabla vacía nada cambia. `pnpm pruebas:compras-compradores` → 23/23 y ya está en el CI.
- [x] **F1 paso 2 — la LECTURA por tienda (aplicada en el Postgres local compartido; no en producción):** `20260922130000_compras_dinero_por_tienda_lectura.sql`. La puerta de LECTURA se abre a «líder o comprador» y todo lo que cuelga de ella se acota con `fn_compra_es_de_mis_tiendas` (5 políticas, bucket, 5 funciones de dinero, `fn_aplicar_candado_de_dinero` repone el filtro); notas de crédito y ESCRIBIR siguen solo del líder. Anclajes verificados contra la definición real de **producción** (solo lectura): coinciden. `pnpm pruebas:compras-dinero-por-tienda` → 25/25, en el CI; `dinero-compras`, `compras-indicadores`, `compras-reparto`, `recibir-envio`, `pagos-compras-endurecimiento` y `proveedores-cuentas` siguen en verde con las migraciones inyectadas. **Pegar en producción: 20260922120000 y luego 20260922130000, y después `select retail.fn_aplicar_candado_de_dinero();` debe dar `{}`.**
- [ ] **Ya fallaban SIN mis cambios (data residual de otra sesión en el Postgres local compartido, un saldo a favor de S/ 200 con un proveedor):** `pruebas:pago-por-lote-medios` (3 casos) y `pruebas:notas-credito` (7 casos). No son de F1; conviene limpiar esa data o aislar esas pruebas.
- [ ] **Antes de pegar F1 en producción (nada cambia para nadie mientras `compradores_de_tienda` esté vacía, pero conviene):** cargar una fila por cada **terminal administrativa** de tienda (el comprador de tienda, decidido en la ronda 6 del ADR-0151: cuenta compartida, cada pago queda a nombre de la terminal) y una por tienda para la persona de Compras de R-10. **Roles y permisos a medida (ADR-0150, `main`) se descartó el 2026-09-22 — no hay que reconciliar con eso.** Sí falta fusionar con **terminales por tienda (ADR-0152 provisional, rama `claude/terminales-cuentas-129fd4`, 4 commits, ya construida y probada — 75 casos — pero sin subir a GitHub)**: choque esperado en `menu.ts`, `ci.yml` y `package.json`; su migración también usaba `20260922140000`, la mía pasó a `20260922150000`/`20260922160000`.
- [ ] **Hallazgo de otra sesión, sin resolver, relevante para Compras:** en producción `productos`, `variantes`, `proveedores` y `ubicaciones` tienen política de `select` con `auth.role() = 'authenticated'` — las lee cualquier sesión iniciada (Dynamic tiene ~30 personas con login), no solo quien tiene acceso a retail. `proveedores` guarda CCI/Yape (ADR-0134). No se toca en este ADR; queda para quien retome permisos.
- [x] **F2 — el dinero partido por tienda (aplicada en el Postgres local compartido; no en producción):** `20260922150000_compra_parte_por_tienda.sql`, la vista `compra_parte_por_tienda` (una fila por factura y tienda; no guarda nada; `security_invoker`). Reparte la **cabecera** con el método del mayor resto: la suma de las partes es siempre `compras.total`, al centavo. `pnpm pruebas:compras-parte-por-tienda` → 20/20, en el CI. Producción hoy tiene 0 facturas de Compras, así que no hay datos reales con los que verificarla; la consulta corrió limpia contra su esquema. **Pegar en producción después de las otras dos.**
- [ ] **Hallazgo de F2, sin arreglar (no es de este plan):** `registrar_compra` calcula el subtotal de la cabecera con el costo tal como llegó (33.333) pero `compra_items.costo_unitario` guarda 2 decimales, así que con costos de 3 decimales `compras.subtotal` puede diferir de la suma de sus líneas por centavos. Decidir si el costo se guarda con 3 decimales o si la cabecera se calcula con los costos ya redondeados. La vista no depende de esto (reparte la cabecera).
- [x] **F3 — registrar y gestionar (aplicada en el Postgres local compartido; no en producción):** `20260922160000_compras_tienda_gestora.sql`. `compras.ubicacion_gestion_id` (la tienda que gestiona: reusa el parámetro `p_ubicacion_destino_id` que ya tenía `registrar_compra`, no uno nuevo — evita crear una sobrecarga). Un comprador registra con SU tienda como gestora y sin pago; la gestora tiene que tener parte en el reparto (candado de esquema, también para el líder); anular y adjuntar: líder o comprador de la gestora; la gestora ve la factura entera; `cambiar_tienda_gestora_compra` solo del líder. `pnpm pruebas:compras-tienda-gestora` → 24/24, en el CI. Repurpasar ese parámetro rompió 3 suites que ya existían (`compras_reparto`, `compras_indicadores`, `pagos_compras_endurecimiento`) porque lo usaban sin relación con el reparto real; las tres quedaron corregidas y en verde.
- [x] **Resuelto (2026-09-22, con ok de Felipe) — el Postgres local compartido tenía residuo de ADR-0150 (roles, abandonado):** el número `20260922120000` lo había usado antes, en otra sesión, `20260922120000_roles_y_permisos_base.sql` (archivo que no existe en el repo — rama borrada, nunca fusionada — pero que SÍ llegó a aplicarse ahí). Se borraron las 4 tablas (`roles`/`permisos`/`rol_permisos`/`roles_historial`), las 2 columnas (`colaboradores.rol_id`, `colaboradores_suspendidos.rol_id`) y las 6 funciones del residuo; se liberó el número con `migration repair --status reverted`; se aplicaron mis 4 migraciones de F1–F3 directo por SQL (idempotentes, ya probadas) y se registraron con `migration repair --status applied`. **El Postgres local ya tiene F1, F2 y F3 aplicadas de verdad.** Dos suites propias que asumían «antes de que exista la fase siguiente» (imposible de probar en aislamiento una vez todo aplicado) ganaron un salto explícito, mismo patrón que `dinero_compras_solo_lider.mjs`.
- [x] **F4 — pagar por tienda (aplicada en el Postgres local compartido; no en producción):** `20260922170000_compras_pagar_por_tienda.sql`. `compra_pagos.ubicacion_id` + `fn_saldo_de_tienda(compra, ubicacion)` (su parte menos lo que ya pagó ESA tienda; **todavía no resta notas de crédito por tienda**, eso es D5/F6). Las tres RPC de pago (`registrar_pagos_compra`, `registrar_pago_compras`, `registrar_pago_compras_medios`) ganan `p_ubicacion_id` al final; sin tienda (solo líder) el pago no queda atado a ninguna, como siempre; con tienda, no puede superar SU saldo — candado de esquema (disparador diferido), no solo de la RPC. «Pagar juntos» respeta la regla por cada comprobante del lote (todo o nada). `pnpm pruebas:compras-pagar-por-tienda` → 23/23, en el CI.
- [x] **Hallazgo real al construir F4:** recrear una función con `drop` + `create` (para agregarle un parámetro sin crear sobrecarga) genera un objeto NUEVO que no hereda los `grant` — Postgres le da `EXECUTE` a `PUBLIC` por defecto. Las tres RPC de pago quedaron momentáneamente abiertas a `anon`; lo detectó una prueba que ya existía (`pagos_compras_endurecimiento`) y se corrigió con `revoke`/`grant` explícitos. Repetible: cualquier migración futura que le agregue un parámetro a una función existente tiene que reponer sus grants a mano.
- [x] **F5 — pantallas, en local, sin aplicar en producción (commits `9d12b611`, `c7240ed8`, `44421d42`):** el layout de `/compras` deja pasar a líder o comprador (`exigirLiderOCompradorDeTienda`, `persona-actual.ts`); `lib/menu.ts` gana el permiso `verDineroCompras` (distinto de `verDinero`, que sigue solo-líder en Producción/Facturación) y abre Proveedores/Comprobantes/Por pagar con él; Comprobantes/Por pagar/Proveedores no necesitaron cambio de código (ya acotados por RLS de F1-F4); Registrar (`CompraFormV2`) ofrece solo las tiendas del comprador y calcula la gestora correcta (`tiendaGestora()`, corrigiendo un bug real de `tiendasReparto[0]` que podía mandar una tienda ajena como gestora); Pagar (`RegistrarPagoModal`, `BotonPagar`, `PagoDesdeUrl`, `PagoJuntosModal`, `PorPagarLista`, `PorPagarVistaRapida`) manda `p_ubicacion_id` con la tienda del comprador, con selector «Pagas desde» cuando tiene más de una. `pnpm --filter web test` → 2549/2549.
- [ ] **F3-b — bloquea «parte nueva», mayor de lo que parecía:** el comprador de una tienda con parte (pero no gestora) sigue sin ver nada de una factura repartida (`fn_compra_es_de_mis_tiendas` no lo deja pasar), así que no hay dónde poner el badge «parte nueva» del ADR. Abrir SOLO la RLS de `compras`/`compra_items` (un booleano por factura) sin más sería mostrarle a esa tienda la deuda TOTAL de la otra — una fuga real de dinero entre tiendas, lo opuesto de lo que este ADR existe para evitar. Hace falta construir DOS piezas juntas, no una: (1) RLS por LÍNEA en `compra_items`/`compra_item_destinos` (filtrar a las líneas con destino en su tienda, no toda la factura) y (2) que `CompraDetalle`, `CompraDetallePanel`, Comprobantes y Por pagar muestren `compra_parte_por_tienda`/`fn_saldo_de_tienda` en vez de `compras.total`/`compras.saldo` cuando quien mira no es líder ni gestora — hoy ninguna pantalla lo hace. A medio camino (solo RLS, sin cambiar qué monto se muestra) es peor que no tocarlo. Decisión pendiente de Felipe: 2026-09-22, ver `docs/BITACORA.md`.
- [ ] **F6–F7:** notas de crédito por tienda (D5, resta de `fn_saldo_de_tienda`) → resultado por tienda.
