...
- [ ] **R2 — Resumen** (no se cierra hasta la migración de `fn_ventas_del_dia` y la franja de proformas): tarjetas de vidrio, «Actividad de hoy», comparativo «mismo día de la semana pasada, a esta hora», línea viva y buscador.
  ...
  - [ ] **Migración de `fn_ventas_del_dia`** (necesita el OK de Felipe; no se aplicó nada): hoy no filtra por `ventas.estado` (una venta anulada saldría en la lista del día) y repite la venta que tiene dos comprobantes (un anulado y su reemplazo). Mientras tanto `ventasUnicas` cubre las tarjetas, y la referencia de la semana pasada sí filtra `completada` (el comentario de `lib/ventas-comparativo.ts` documenta la asimetría). **Bloquea cerrar R2.**
...
- [ ] **OK de la migración de `fn_ventas_del_dia`** (arriba). Se propone y espera; bloquea cerrar R2.
