...
## 🎯 Productos: las alertas de stock solo cuentan activas, «Stock total» y números que no mienten (2026-09-22, ADR-0150) — hecho en local, FALTA PEGAR 1 MIGRACIÓN EN PRODUCCIÓN
...
- [ ] **Pegar en producción la migración** `20260922120000_productos_alertas_solo_activas_y_variantes_distintas.sql` (ya trae `set search_path`; idempotente) y verificar con las consultas del ADR-0150. El orden con el despliegue del código no importa. Hasta entonces «no está en producción».
...
- [ ] **Revisión adversarial (3 revisores) — lo que quedó abierto a propósito, ADR-0150 «Lo que NO toca»:** Inventario (Existencias y Resumen) sigue contando descontinuadas como sin stock / stock bajo / reponer piso, y «Desactivar» en bloque no toca `variantes.activo` — decidir con Felipe si Inventario sigue a Productos; Inicio cuenta «Productos activos» sobre todas las filas (descontinuadas y el producto especial incluidos); R-48 en `15-COMO-OPERA-CAYLA.md` debe llevar la excepción cuando se decida.
