...
## 🎯 Dominio de Inventario — comportamiento comercial reconstruido sobre modelo canónico (2026-09-24, ADR-0180, sucede a ADR-0179) — LOCAL, NADA en producción
Felipe envió la definición canónica completa del dominio y pidió auditar lo construido hoy en ADR-0179 contra ella
...
[docs/adr/0180-dominio-inventario-comportamiento-comercial.md](adr/0180-dominio-inventario-comportamiento-comercial.md).
...
- [ ] Drill-down conceptual de Análisis → Movimientos (ADR-0183, sección 6): los datos ya son compatibles (`variante_id`+sede+fechas), pero `fn_movimientos` no acepta `p_variante_id` (solo producto completo o texto libre) — falta agregar el parámetro y el enlace en la UI. Explícitamente NO construido hoy, a pedido de Felipe.
