...
## 🎯 Compras: cada tienda compra, ve y paga lo suyo (2026-09-21, ADR-0151) — ACEPTADO, F1–F3 en local
...
- [x] **F3 — registrar y gestionar (hecho en local, sin aplicar):** `20260922160000_compras_tienda_gestora.sql`. `compras.ubicacion_gestion_id` (la tienda que gestiona: reusa el parámetro `p_ubicacion_destino_id` que ya tenía `registrar_compra`, no uno nuevo — evita crear una sobrecarga). Un comprador registra con SU tienda como gestora y sin pago; la gestora tiene que tener parte en el reparto (candado de esquema, también para el líder); anular y adjuntar: líder o comprador de la gestora; la gestora ve la factura entera; `cambiar_tienda_gestora_compra` solo del líder. `pnpm pruebas:compras-tienda-gestora` → 24/24, en el CI. Repurpasar ese parámetro rompió 3 suites que ya existían (`compras_reparto`, `compras_indicadores`, `pagos_compras_endurecimiento`) porque lo usaban sin relación con el reparto real; las tres quedaron corregidas y en verde.
- [ ] **BLOQUEADO — el Postgres local compartido tiene residuo de ADR-0150 (roles, abandonado) que choca con el nombre de MI migración F1:** el archivo `20260922120000_compras_compradores_de_tienda.sql` reusa el mismo timestamp que usó, en otra sesión, `20260922120000_roles_y_permisos_base.sql` — ese archivo no existe en el repo (rama borrada, nunca fusionada), pero SÍ llegó a aplicarse en el Postgres local compartido y dejó: tablas `retail.roles`/`permisos`/`rol_permisos`/`roles_historial` (3/29/18/0 filas), columnas `colaboradores.rol_id` y `colaboradores_suspendidos.rol_id` con sus disparadores, 6 funciones (`fn_tiene_permiso`, `fn_mis_permisos`, etc.), y una fila `20260922120000 | roles_y_permisos_base` en `supabase_migrations.schema_migrations`. Mientras eso siga ahí, `supabase migration up` NO aplicará mi migración F1 (ve el número ya usado) y `compras_indicadores.mjs` (editado para F3) falla al correr sola contra la base compartida — aunque en CI (que arranca limpio) no pasa nada. **Se necesita, en este orden:** (1) borrar las 4 tablas, las 2 columnas y las 6 funciones del residuo; (2) `supabase migration repair --status reverted 20260922120000` para liberar el número; (3) aplicar mis 4 migraciones (`20260922120000`, `130000`, `150000`, `160000`) con `migration up`. Se detuvo aquí: el clasificador de la sesión bloqueó tocar el Postgres compartido sin permiso explícito (correcto, lo usan ~20 sesiones). Ver [[sincronizar-postgres-local-compartido]] y [[roles-permisos-a-medida-descartado]].
...
- [ ] **F4–F7:** pagar por tienda → pantallas → notas de crédito → resultado por tienda.
