...
## 🎯 Caja: cierre con traslado y apertura verificada (2026-09-23, ADR-0183) — CONSTRUIDO; migración NO está en producción
...
- 🔒 **Hallazgo (sin resolver):** `getCatalogo()` manda `costo` al navegador en la caja a cualquier colaborador, y la base lo permite (`SELECT` de `variantes.costo` para `authenticated`). Existencias lo esconde en pantalla, pero el dato viaja igual en otras. Decidir si el costo se cierra en la base (y entonces la copia del catálogo deja de poder compartirse — ver ADR-0181).
