...
- [x] **`DRIFT.md` ya no dice «sobra» de funciones en uso** (2026-09-26). `comparar.mjs` cuenta como usada toda función que la pantalla nombra entre comillas fuera de un `.rpc("…")` directo (un ternario, un ayudante), **ignorando los comentarios** (10 de las 29 solo aparecían en comentarios); la lista bajó de 29 a 21 y se llama «Funciones sin llamada detectada desde `apps/web`», con la advertencia de que **no prueba que sobren** (las puede llamar otra función, un disparador, un script o Dynamic). La lectura vive en `scripts/datos/comparar-lectura.mjs` con prueba propia en el CI y un oráculo contra el parser de TypeScript en los 769 archivos de `apps/web`. El informe ahora dice la **fecha de su foto**: una función posterior sale como «no existe» aunque ya exista (las 8 «rotas» del 2026-09-26 existían en producción). Sigue abierto: refrescar la foto de funciones (`docs/datos/generado/COMO-REFRESCAR.md`) y, de las 21, mirar las 17 sin pantalla, sin función y sin disparador antes de decidir retirar alguna (p. ej. `catalogo_crear_producto`, que el código ya da por retirada por el ADR-0109).
