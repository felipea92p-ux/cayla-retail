...
- [x] **`DRIFT.md` ya no dice «sobra» de funciones en uso** (2026-09-26). `comparar.mjs` cuenta como usada toda función que la pantalla nombra entre comillas fuera de un `.rpc("…")` directo (un ternario, un ayudante), **ignorando los comentarios y las pruebas** (7 de las 29 solo aparecían en comentarios); la lista bajó de 29 a 21 y se llama «Funciones sin llamada detectada desde `apps/web`», con la advertencia de que **no prueba que sobren** (las puede llamar otra función o un disparador, usar un script o Dynamic, o ser una herramienta de mantenimiento que se corre a mano). Lee el código con el **parser de TypeScript** (`scripts/datos/comparar-lectura.mjs`; un primer intento con un escáner a mano se equivocaba en código real: regex con `//`, el idioma CSV), con pruebas en el CI (la lectura y un `comparar.mjs` de extremo a extremo con un repo de juguete). De paso desaparecieron 8 avisos falsos «no manda `p_x`» (un comentario entre la coma y la clave impedía reconocerla). El informe ahora dice la **fecha de su foto** y habla de llamadas «sin respaldo en la foto», no de pantallas «rotas»: una función posterior sale como «no está» aunque ya exista (las 8 del 2026-09-26 existían en producción) y dice en qué migración se define. Sigue abierto: refrescar la foto de funciones (`docs/datos/generado/COMO-REFRESCAR.md`); de las 21, mirar las 17 sin pantalla, sin función y sin disparador antes de decidir retirar alguna (p. ej. `catalogo_crear_producto`, que el código ya da por retirada por el ADR-0109; ojo: `recalcular_stock`, `recalcular_compras` y `archivar_*_prueba` son herramientas que se corren a mano); y una llamada indirecta a una función que NO existe en producción sigue invisible.
