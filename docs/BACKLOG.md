...
- [ ] **Antes de pegar F1 en producción (nada cambia para nadie mientras `compradores_de_tienda` esté vacía, pero conviene):** cargar la lista de compradores por tienda y reconciliar con la sesión de roles y permisos (`claude/roles-permisos-migration-6506e1`), para que la persona de Compras (no es líder) no quede con dos sistemas de permisos.
