...
## 🎯 Compras por tienda: cada tienda ve y paga lo suyo (2026-09-23, ADR-0179 — nació como 0145/0150/0151) — hecho y probado en local, FALTA PEGAR 5 MIGRACIONES EN PRODUCCIÓN
...
- [ ] **Pegar en producción, en orden:** `20260923180000`, `180100`, `180200`, `180300`, `180400` (con `set search_path` ya incluido). Después: `select retail.fn_aplicar_candado_de_dinero();` → `{}` y refrescar el volcado + `pnpm datos:generar:produccion`. Hasta entonces «no está en producción». El orden con la web no importa: el líder paga sin tienda como siempre, hoy nadie más tiene los módulos de Compras, y «Tu parte» se degrada a vacío si la base aún no tiene las funciones.
