...
- [ ] **F1 paso 2 — abrir la puerta CON los filtros (van juntos):** reescribir `fn_aplicar_candado_de_dinero()` (hoy inyecta «solo líder → 42501»; debe pasar a filtrar por `fn_compras_ubicaciones()`), cambiar `fn_puede_registrar_compras()` / `fn_puede_ver_dinero_de_compras()` a «líder o comprador de alguna tienda», y filtrar por tienda las políticas de `compras`, `compra_items`, `compra_pagos`, `compra_adjuntos`, `compra_notas_credito`, el bucket y las funciones `security definer` (`listar_*`, indicadores), partiendo de `pg_get_functiondef` de PRODUCCIÓN. Abrir la puerta sin filtrar dejaría a un comprador leer el dinero de todas las tiendas.
