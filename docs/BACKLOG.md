...
- [ ] **Cuándo subir** (ensayo del 2026-09-21, sin tocar nada de la rama). La rama va **57 commits adelante y 192 detrás** de `origin/main` (base: 2026-09-19). Una fusión tendría **5 conflictos, todos de «sumar de los dos lados»**: `.github/workflows/ci.yml`, `package.json`, `docs/BACKLOG.md`, `docs/BITACORA.md` y `docs/SESIONES-ACTIVAS.md`; `globals.css`, `Chip.tsx` y `ARQUITECTURA.md` se funden solos. El **ADR 0124 sigue libre** en `main` (allá hay 0121 a 0123 y 0125 a 0139). Se encontró y arregló un choque real: dos de las migraciones de hoy tenían la misma versión que otras de `main` (`20260921120000` y `20260921160000`) y se renombraron a `…121500` y `…161500` (`7369b09e`); `main` no redefine ninguna función que ellas tocan. **Antes de pushear, con el OK de Felipe:** fusionar `origin/main` (con PowerShell, no Bash), resolver esos 5, comprobar que no queden versiones repetidas (`git ls-files supabase/migrations | sed 's#.*/##' | cut -c1-14 | sort | uniq -d` debe salir vacío), correr tipos, pruebas y build, y re-medir en el navegador el umbral de 64 rem de las pestañas con el lateral plegable de `main`; después, el push.
