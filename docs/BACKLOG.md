...
- [ ] **`LUCODE_ENTORNO` a `sandbox` en Vercel y volver a desplegar** (lo hace Felipe). Hoy está en `produccion`: los 2 comprobantes transmitidos el 08 y 09-sep fueron a la SUNAT real. **Tiene que estar hecho ANTES de que esta rama llegue a `main`**, o cada venta de prueba se declarará sola a SUNAT.
- [ ] **Pegar en producción** `20260922193700_comprobantes_tomar_para_reintento.sql` y `20260922234100_series_archivar.sql` (con OK de Felipe, receta de huellas). Sin la primera, el barrido falla sin ruido; sin la segunda, «Archivar» falla y «Nueva serie» sigue reemplazando.
