...
- [x] **`scripts/demo/deshacer-90-dias.sql`** (2026-09-22): borra solo lo sembrado, en una transacción. Aborta si una fila real apunta a la demo (lo recorre solo, desde las claves foráneas), o si una fila «5eed» es en realidad un uuid real que empieza así por azar (pasó en local con un código de barras). Apaga los 3 candados de historial solo dentro de la transacción y devuelve los contadores a su valor de antes de la carga, salvo que haya un número real posterior. Ensayado en local (83 tablas: todas idénticas salvo las series NC01/NC02, que se dejan a propósito) y en producción, abortado a propósito: borraba todo y devolvía B004 a 28 y los traslados a 4.
- [ ] **Antes de correr el deshacer:** hay una venta REAL de 7 prendas demo (TRU, 2026-09-22 17:46 Lima, boleta B004-4647 `pendiente`). El deshacer se niega mientras exista; hay que anularla primero (o decidir otra cosa).
...
- [x] **Fase 3 — inventario inicial y abastecimiento** (`scripts/demo/sembrar-90-dias.sql`): 157 compras, ~13.200 movimientos, 52 traslados, saldos ≥ 0 comprobados día a día, A2/A3/A4/A5/A6/A7/A14. Ensayada en local con el arnés (`scripts/demo/local/`); estructura idéntica a producción en 34 de 35 tablas. **Falta el ensayo contra producción con el archivo completo (114 KB).**
- [x] **Revisión adversarial de la Fase 3** (3 lentes + escépticos, 2026-09-22): 4 hallazgos reales, 1 descartado. Corregidos en el generador: `setval()` de `transferencias.numero` (bloqueante — con interruptor `cayla_seed.definitivo`, porque `setval()` no es transaccional) y el chequeo (13) del reparto por tienda (alto). Documentados para la Fase 7: el lock de `codigos_correlativos` durante toda la corrida (alto) y la receta de `deshacer-90-dias.sql` con su ciclo de FK (alto). Detalle en el ADR, sección «Revisión adversarial de la Fase 3».
- [x] Revisor de «pantallas» de la Fase 3 (relanzado tras el choque de infraestructura): completado. 2 hallazgos — ver abajo y el ADR.
- [x] **Fase 4 — ventas, caja y comprobantes** (`scripts/demo/sembrar-90-dias.sql`): 6.995 ventas (S/ 932.430), 267 cajas, 6.658
...
- [x] Consecuencia: la pantalla de Notas de Crédito y `compras_nota_pendiente()` (que alimenta «Por pagar»/listas de Compras) mostraban el mismo dinero dos veces: una vez como nota ya aplicada, otra como «esperando nota» — sobrestimaba cuánto crédito faltaba reclamar a proveedores.
- [x] Arreglo: en ambas funciones, se cambió `not exists (select 1 from compra_notas_credito n where n.compra_id = c.id and n.motivo = 'faltante')` por comparar contra `n.cierre_id = k.id`, sin filtrar por motivo. Migración `20260922151800_notas_credito_pendiente_por_cierre.sql`.
- [x] Verificación: reproducido y corregido en Docker local (compra + cierre + nota `devolucion` → antes duplicaba, después 1 sola fila resuelta); regresión con un cierre genuinamente sin nota sigue apareciendo pendiente. Pegado en producción por MCP el 2026-09-22 (proyecto `vovjyyiafkxteijimpuy`), confirmado en vivo con la huella de la definición (ya no contiene la lógica vieja). Producción no tenía hoy ningún cierre real con nota de motivo distinto a `faltante` — el bug no había mordido a nadie visible todavía; la siembra de 90 días lo iba a activar, por eso lo encontró el reviewer antes de que pasara en la vida real.
- [x] No es un bug de la siembra de 90 días: el generador construye A5 correctamente (motivo `devolucion`, porque la compra no está resuelta); fue la primera vez que hubo compras suficientes para que este bug de la app se notara.
