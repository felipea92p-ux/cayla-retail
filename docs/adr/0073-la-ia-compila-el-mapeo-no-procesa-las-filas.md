...
**Nota de restauración (2026-09-16):** este ADR se creía un ADR-0035 fantasma —
cuatro documentos de `docs/datos/` lo citaban por ese número y el archivo no
existía en el árbol. Investigado en `git log --all`: sí existió. Nació como
`0031-la-ia-compila-el-mapeo-no-procesa-las-filas.md` (2026-09-11), se renombró
a `0035` ese mismo día al desduplicar `main` (0031 ya era `recalcular_stock`), y
se borró íntegro — junto con todo el módulo de importación (`lib/importacion/`,
las rutas `api/importacion/*`, la pantalla `/inventario/importar`) — al día
siguiente en `0af2f1b`, el corte V1→V2. La documentación de los "14 módulos"
que lo cita se escribió ese mismo 12-sep en una rama paralela que nunca se cruzó
con el corte, de ahí la referencia rota. Contenido recuperado íntegro de
`0af2f1b^:docs/adr/0035-la-ia-compila-el-mapeo-no-procesa-las-filas.md`, con el
encabezado original intacto abajo (incluida su propia nota de numeración de
2026-09-11, previa a esta). Renombrado a **0073** porque 0035 ya lo ocupa
Compras en esta rama. El importador en sí sigue sin existir en el repo — este
ADR queda como diseño de referencia para cuando se reconstruya sobre V2,
después del censo (20-sep).
