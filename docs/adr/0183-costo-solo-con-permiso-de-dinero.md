...
podía pedirle a la base el costo —y con el precio, el margen— de todo el catálogo. Tres funciones con permisos propios
lo devolvían sin preguntar: `fn_productos` (Productos), `fn_costo_historial` y `censo_crear_variante`. La regla de negocio ya decidida (ADR-0126, P5) decía otra cosa; la base no la cumplía.
...
**Excepción (Felipe, opción 2 — mantiene P5 de ADR-0161):** el módulo **Análisis** también da el costo, **solo dentro
de Análisis y solo de su sede**. `fn_resumen_comparacion` no cambia: ya exige `fn_puede_analizar()` y
`fn_puede_operar_ubicacion`. Sin costo no hay rotación (costo de lo vendido ÷ inventario a costo) ni márgenes, y darle
Análisis a un rol ya es una decisión consciente del líder.
...
- **Análisis no cambia** (excepción P5): quien tiene el módulo ve el costo de su sede ahí, y en ningún otro lado.
...
| Análisis con costo (sin cambio, P5) | 1.150 | 0 (no tiene Análisis) | 0 (no tiene Análisis) |
