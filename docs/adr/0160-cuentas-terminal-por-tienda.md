...
- **Inicio** (`lib/inicio-reglas.ts`, sobre el Inicio nuevo de main): la terminal administrativa no ve «Hoy» —no vende— y su acción principal es Inventario, no Vender; su contador de traslados usa el mismo permiso que el del menú.
...
- **El tipo de terminal decide el MENÚ, no cierra rutas.** La terminal de ventas ve solo Ventas, pero si escribe `/inventario` en la barra entra: en la base sigue siendo un colaborador de su tienda, y un colaborador puede contar, trasladar y recibir. Lo mismo al revés (la administrativa puede abrir `/vender`). Lo que cambia por tipo es el menú y los **poderes extra** (cerrar caja, ajustar stock…), que sí los hace cumplir la base. Si algún día se quiere cerrar también las rutas, es el mismo mecanismo de las capacidades aplicado a las funciones de inventario (contar, trasladar, mover, recibir) o de venta, una lista corta que ya está medida en la conversación de origen.
