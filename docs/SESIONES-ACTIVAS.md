...
|---|---|---|---|
| Claude | `claude/producto-etiquetado-legal-16434c` | Producto: etiquetado legal (país de origen, fabricante, material — Ley 28405 / RTA, ADR-0097) — 3 columnas nullable en `retail.productos` + 3 RPCs actualizadas (`catalogo_crear_producto`, `catalogo_actualizar_producto`, `crear_producto_con_variantes`) + campos en `NuevoProductoForm.tsx`/`ProductoForm.tsx`. Verificado en navegador de punta a punta. Reconciliado con la taxonomía de tejidos/patrones (ADR-0095/0096, ya en `main`) — `crear_producto_con_variantes` queda con los 7 parámetros de esa rama + los 3 propios; de paso, limpiada una sobrecarga huérfana preexistente en `main` en `catalogo_crear/actualizar_producto` (sin relación con esta tarea, ver ADR-0097). Pusheada, sin PR abierto todavía. | 2026-09-17 |
