...
- [ ] **Step 4: UI.**
  - `vender/page.tsx`: sumar al `Promise.all` las tres lecturas `supabase.from("categorias").select("id, nombre").eq("activo", true).order("nombre")`, `from("tallas").select("id, valor").eq("activo", true).eq("estado", "aprobado").order("orden")` (si `tallas` no tiene `orden`, ordenar por `valor`), `from("colores").select("codigo, nombre").eq("activo", true).order("orden")`, y pasarlas a `<PuntoDeVenta listasPrendaLibre={{ categorias, tallas, colores }} />`.
