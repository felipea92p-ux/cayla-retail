...
6. **Docblock desactualizado que dejó la Task 4.** El comentario de `onLiberar` (alrededor de la línea 254) dice «mismo patrón que `onEmitir`/`onRegistrarSerie` en este mismo componente», pero `onEmitir` ya no vive ahí: déjalo como «mismo patrón que `onRegistrarSerie` en este mismo componente». Es solo un comentario.
...
| 4 | Los contadores coinciden con la base y se ocultan si su consulta falla | Compara el número de «Comprobantes» con `select count(*) from retail.comprobantes where estado in ('pendiente','rechazado')` en la base local, y el de «Proformas» con las vigentes que aún no vencieron (`select count(*) from retail.proformas where estado='vigente' and (vence_at is null or vence_at > now())`). **En la base local del repo las tablas también viven en el schema `retail`** (verificado el 2026-09-19 con `docker exec supabase_db_cayla-retail psql -U postgres -d postgres -c "…"`, que es como se consulta). Línea base local de ese día: 19 comprobantes `pendiente` (del 2026-09-18), ninguna proforma, 2 tiendas y 4 series; por eso «Comprobantes» debe mostrar **19** en ámbar y «Proformas» ningún contador hasta que crees una. Para «se ocultan»: rompe a propósito el nombre de la tabla en `getResumenPorEnviar` (por ejemplo `"comprobantes_x"`), recarga y mira que la pestaña queda **sin** contador y sin error de pantalla, **y que la salida del servidor trae la línea `console.error` «Facturación: no se pudo leer la cola «por enviar» …»** (es la única evidencia viva de ese registro: no tiene prueba commiteada); haz lo mismo con `getResumenProformas` (su línea dice «no se pudo leer las proformas vigentes»); **revierte** | Coinciden; sin contador cuando falla; la vista sigue cargando |
| 5 | Un error en una vista no tumba la cabecera | Pon `throw new Error("prueba de error de vista");` como primera línea del cuerpo de `proformas/page.tsx`, abre `/vender/facturacion/proformas` y mira la tarjeta; luego **revierte** con `git checkout -- "apps/web/app/(app)/vender/facturacion/proformas/page.tsx"` **y pulsa «Reintentar» SIN recargar la página**: la vista debe cargar (en Next 16 `reset` por sí solo solo vuelve a pintar el error, no vuelve a pedir datos al servidor; el botón hace `router.refresh()` + `reset()`) | Sale la tarjeta «Esta vista no está mostrando datos» con «Reintentar»; la cabecera y las pestañas siguen; la pestaña Resumen funciona; **«Reintentar» recupera la vista una vez revertido el error** |
| 6 | «Emitir comprobante» y «Nueva proforma» abren su modal desde cualquier pestaña y la emisión conserva su token | Ábrelos desde la cabecera en las cuatro pestañas y desde el botón de Comprobantes/Proformas. **Token:** compara `EmitirComprobanteModal.tsx` con `ComprobantesPanel.tsx` tal como era antes de la Task 4 (`git log --oneline -- apps/web/components/ComprobantesPanel.tsx` te da el hash del commit de la Task 4; `git show <hash>~1:apps/web/components/ComprobantesPanel.tsx`): `tokenEmision` sigue siendo un `useRef` inicializado una vez y se renueva solo tras un Emitir exitoso. Con la base local arriba, emite una boleta de prueba (nunca «Transmitir» sin confirmar `LUCODE_ENTORNO=sandbox`). **Además, lo que la revisión de la Task 4 dejó para el navegador:** (a) al reabrir «Emitir comprobante», total y cliente vuelven vacíos y la ubicación y el tipo conservan lo último que elegiste; (b) «Cancelar» sale con animación; (c) una emisión exitosa cierra de golpe y la lista se refresca; (d) con la pestaña de red abierta, dos «Emitir» fallidos seguidos (rompe la RPC a propósito o corta la red, y **revierte**) mandan el MISMO `p_token`. **Lo equivalente para «Nueva proforma» (revisión de la Task 5):** (e) abre el modal, cambia la tienda, escribe cliente y total, «Cancelar» y reábrelo: cliente y total vacíos, «Vigente por» en 7 y la tienda conservada; (f) guarda una proforma en la base local y comprueba que aparece en la tabla (es el único recorrido de la RPC `crear_proforma` desde el componente nuevo); (g) «Convertir» de una proforma vigente sigue abriendo su modal. Y en el código: el shell dibuja `<EmitirComprobanteModal abierto={…}>` y `<NuevaProformaModal abierto={…}>` **una sola vez cada uno y sin condicional sobre `abierto`**, y los dos paneles dejaron de dibujarlos (un `{abierto && …}` o dos instancias darían un token nuevo por apertura) | Los dos modales abren desde todas; la boleta aparece en Comprobantes; (a) a (d) se cumplen; sin diferencias de lógica |
...
| 9 | Con Tab se llega a las pestañas y el foco se ve | Con el teclado, Tab hasta las pestañas; en consola `getComputedStyle(document.activeElement).outlineStyle` | Cada pestaña recibe foco y `outlineStyle` es `solid` (no `none`: el par `outline-none` + `focus-visible:outline*` no dibuja el anillo en Tailwind 4 si falta `outline-solid`), con contorno visible `outline-rojo/60`. **El anillo de la primera y de la última pestaña NO se recorta**: el contenedor lleva `overflow-x: auto`, que recorta en el borde del padding, y el anillo (offset 2 px + ancho 2 px) mide lo mismo que el padding de 4 px; mira una captura ampliada (`zoom`) de esos dos extremos |
...
2. **`docs/BACKLOG.md`:** en la sección Facturación, marca `- [x]` en R1 y agrega una línea con la fecha y lo verificado. Anota como pendientes conocidos, con su porqué: (a) hasta R3 la tarjeta «Pendientes de enviar» del panel de Comprobantes sigue contando `pendiente`+`enviado` **del mes**, mientras el contador de la pestaña cuenta `pendiente`+`rechazado` **sin mes** (definición del spec §9): no es un error de R1; (b) hasta R3 la tarjeta «Proformas vigentes» cuenta también las vencidas; (c) afina la línea de «Fuera de esta entrega» que dice «si producción incluye ventas anuladas en `fn_ventas_del_dia`, migración con OK de Felipe»: la Task 0.2 ya lo confirmó (la función no filtra ni devuelve `ventas.estado`), así que reescríbela como hecho, con el OK de Felipe pendiente y la migración bloqueando el cierre de R2.
