...
## 2026-09-17 (Productos gana `estado_publicacion` — y el contenedor Postgres compartido se resetea solo, 3 veces, mientras se verificaba)

Se cerró `estado_publicacion` en `productos` (borrador/activo/archivado, ADR-0094) — eje aparte de `estado`, mismo criterio que ADR-0007 ya sentó para comprobantes/proformas. Migración + `fn_productos`/`fn_productos_resumen`/`catalogo_actualizar_producto` con `p_estado_publicacion` (drop+create explícito, no `create or replace` a secas — mismo incidente de sobrecargas de `20260917200000` horas antes); `/productos` bloquea a un colaborador a "activo" en el SERVIDOR (no solo la UI); `/vender` excluye borrador/archivado para cualquiera. La tarea llegó con instrucción de trabajar en el worktree `ecstatic-booth-676259` (compartido por al menos 6 sesiones paralelas hoy) en vez del worktree propio de la sesión — confirmado sin choque de columnas con la tarea paralela de etiquetado legal antes de tocar nada, coordinado en `SESIONES-ACTIVAS.md`. Aprendizaje caro: el Postgres LOCAL de este worktree es un único contenedor Docker compartido por TODAS las sesiones de la máquina, no uno por worktree — en los ~15 minutos de esta tarea se lo vio pasar de tener la columna nueva, a no tenerla (otra sesión corrió `db reset` con su propia carpeta de migraciones), a perder la tabla `personas` entera, porque distintas sesiones ejecutan `db reset`/`stop`/`start` sobre el mismo recurso sin coordinarse. `gen-types` contra ese estado fluctuante trajo una vez un `Database = {}` completamente vacío (casi se comitea así); `packages/database/src/types.ts` terminó editado a mano, mismo remedio que ya usó ADR-0093 el mismo día por la misma causa. La verificación en navegador (click-through) quedó bloqueada por dos huecos de entorno separados de esta tarea: el tooling de `preview_start` de la sesión arranca en el worktree con el que se lanzó (`producto-estado-publicacion-e3c13e`), no en el real, y ese worktree además está cientos de commits atrás de `main` (sin la carpeta `/productos` siquiera) — se verificó en su lugar el candado real por consulta directa a las RPCs. `pnpm --filter web typecheck`/`lint`/295 tests en verde (4 errores preexistentes de `ConteoPanel.tsx`/`lib/conteos.ts`, ajenos, ya documentados hoy por otra sesión).

## 2026-09-17 (Producción del Taller confirmada en producción — contradicción de 09-16 cerrada)

Antes de repartir más tareas paralelas se lanzó una ronda de 12 verificaciones contra el código real (mismo criterio que el mediodía: no proponer nada sin confirmarlo primero). Una de ellas resolvió sola una duda que llevaba abierta desde el 16: si `20260915130000_produccion_del_taller` estaba realmente aplicada en producción. Consulta de solo lectura contra la base real (Supabase MCP, `begin transaction read only`) confirmó que sí — tablas, las 5 RPC y la ubicación "Taller" existen — gana la versión de `DiegoN`, no la de `main`. Lo único desactualizado es el volcado versionado de `docs/datos/generado/`, congelado antes de que se completara la aplicación. Aprendizaje: cuando dos ramas dicen cosas opuestas sobre el estado de producción, la pregunta correcta nunca es "¿cuál rama tiene razón?" — es "¿qué dice la base ahora mismo?", y a veces la respuesta cuesta una sola consulta.
