...
— la columna de texto que esta misma rama ya reemplazó por `talla_id` (ADR-0097,
...
## 2026-09-17 (etiquetado legal de producto: país de origen, fabricante, material — ADR-0097)
...
columnas nullable en `productos` (`20260917220000`), sin backfill. Al ir a conectar el
...
**Seguimiento, misma sesión — la reconciliación anunciada arriba dejó de ser hipotética:**
PR #75 (taxonomía cerrada de variante) se fusionó a `main` mientras se cerraba esta
tarea. Al traer `origin/main`, las 3 funciones tocadas por esta ADR ya tenían
`p_tejido_id`/`p_patron_id` fusionados (`20260917210001`) — se reescribieron partiendo de
ESE cuerpo, no de la copia vieja, y la migración se renumeró de `20260917210000` a
`20260917220000` (colisión de timestamp con `20260917210000_catalogo_actualizar_producto_recupera_color_codigo_fotos.sql`,
ya en `main`). De paso, verificando con `pg_get_function_identity_arguments` después de
un `db reset` limpio, se encontró un bug real preexistente en `main` sin relación con
esta tarea: `catalogo_crear_producto`/`catalogo_actualizar_producto` tenían DOS
sobrecargas ambiguas cada una — `20260917190000` (fix de fotos por color) hizo
`CREATE OR REPLACE` con la firma vieja de 8/10 parámetros sobre una base que ya tenía la
de 10/12 con tejido/patrón, y como las firmas no calzan, Postgres creó una función nueva
en vez de reemplazar. Se limpió en esta misma migración (ya tocaba las dos funciones).
Smoke test por SQL directo (transacción + `ROLLBACK`) contra el esquema reconciliado:
`crear_producto_con_variantes` con tejido/patrón en null y los 3 campos legales con
valor — el producto se crea y los 3 quedan guardados tal cual. `pnpm --filter web
typecheck`/`lint` en verde otra vez después de la reconciliación. No se repitió la
verificación de navegador contra este esquema ya reconciliado — ver "Lo que falta" del
ADR.
