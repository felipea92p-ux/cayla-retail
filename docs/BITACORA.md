...
## 2026-09-25 (Inicio se vuelve módulo apagable, y cada rol elige su pantalla principal — ADR-0206)
En la misma conversación del candado de «propio rol», Felipe pidió elegir a dónde aterriza cada rol al iniciar sesión, y que Inicio se pueda apagar como cualquier módulo. Inicio no era de ningún módulo — lo veía toda persona sin excepción, y dos pruebas distintas lo comprobaban a propósito. Nace como módulo ENCENDIDO en los roles que ya existen (excepción documentada a «todo módulo nuevo nace solo del líder»: si no, cada cuenta se quedaba sin dónde aterrizar el mismo día). `guardar_modulos_rol` ganó un 4.º parámetro (`p_pantalla_principal`) con la técnica de ADR-0193 (armar la definición desde la viva, borrar la firma vieja, nunca dos sobrecargas). Un rol sin pantalla elegida aterriza solo en la primera que ve, en el orden del menú; sin ninguna, en «Sin acceso» (la pantalla que ya existía para `exigirModulo`, no una nueva). Ensayado con `begin…rollback` contra producción (6 roles reales verificados uno por uno) — **pero el clasificador de modo automático bloqueó aplicarla de verdad** («Production Deploy»): queda pendiente que Felipe la pegue antes de publicar la web de esta rama. 89.194 pruebas en verde, tsc y eslint limpios; sin verificación en navegador (este worktree no tiene sesión y `preview_start` no puede apuntar a un worktree).
Felipe se lleva: (1) **un módulo nuevo casi siempre nace solo del líder, salvo cuando apagarlo por defecto rompe a todo el mundo el mismo día**: la excepción se documenta, no se generaliza; (2) **una pantalla ya construida (`/sin-acceso`) vale más que una nueva parecida**: el caso «0 módulos» reusó la que ya existía para «te falta un módulo»; (3) **el clasificador de modo automático distingue el tamaño del cambio, no solo si es producción**: la migración de ayer (una condición en una función) pasó; esta (una tabla nueva, backfill de datos reales, una función con sobrecarga) la frenó — señal correcta, no un capricho.

## 2026-09-25 (Admin se reasigna su propio rol — ADR-0178 act.)
Felipe probó autoasignarse un rol en «Roles y accesos», logueado como él mismo, y no pudo: el candado de «no te cambias tu propio rol» (20260923110000, previo a ADR-0178) bloqueaba a cualquiera sin excepción, ni siquiera a un Admin. Se sacó la excepción SOLO para Admin en `asignar_rol` (`fn_es_admin()`), reusando los candados que ya existen («último líder activo», `fn_exigir_otro_admin`) en vez de abrir uno nuevo — ambos ya miran «¿queda alguien más?», no «¿soy yo?». Verificado con un ensayo `begin…rollback` contra la definición viva de producción antes de aplicarlo de verdad; migración `20260925211500` (renumerada al traer `main`: chocaba con `20260925210000_fotos_de_perfil_desde_dynamic`, de otra sesión, mismo timestamp).
Felipe se lleva: (1) **un candado escrito para «cualquiera» a veces sobra para quien ya tiene el poder completo**: un Admin puede tocar a cualquier líder, incluido él mismo desde otra sesión — pedírselo a otro Admin no sumaba seguridad, solo fricción; (2) **los candados genéricos («¿queda alguien más?») se heredan gratis**: no hizo falta escribir un caso especial para «yo mismo», el mismo chequeo que protege a cualquier otro líder protegió la excepción; (3) **la base `postgres` local compartida no sirve hoy para correr `pruebas:roles`**: Compras quedó desalineado de producción y los 70 casos fallan por eso, no por este cambio — se verificó contra producción directamente.
