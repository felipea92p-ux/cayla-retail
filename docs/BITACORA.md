...
## 2026-09-17 (Conteo prioriza por valor en riesgo, no solo por unidades vendidas)

`fn_prioridad_conteo` desempataba qué contar primero por `ventas_30d` en unidades — una prenda cara de baja rotación perdía contra una barata de alta rotación aunque tuviera mucho más dinero parado en el estante, el mismo hueco que marcó el benchmark de esta tarde contra prácticas reales de cycle counting (ABC por valor). Se agrega `valor_en_riesgo` (stock × costo, el mismo cálculo que ya usa `conteo-varianza.ts`) como desempate principal, con `ventas_30d` como segundo desempate — lo nunca contado sigue yendo primero siempre, eso no cambió. Al escribir la migración se encontró que hay que basarse en la versión YA CORREGIDA de la función (20260917100800, que la pasó de `variantes.talla` a `tallas` vía `talla_id`), no en la original de 20260916 — copiar la original a ciegas habría reintroducido `column va.talla does not exist`, lo que efectivamente pasó y se atrapó recién al correr `db reset` local, no al escribir el SQL. Probado en navegador: la lista de "Conviene contar primero" ordena por soles en el estante dentro de cada empate de "nunca contada". `db reset`, typecheck, lint y 295 tests en verde.
