...
## 2026-09-21 (Historial de ventas: nombre de la prenda, antigüedad del comprobante y tres atajos)
Primeras tres tareas de la auditoría `docs/pantallas/vender-historial.md` (#3, #1 y #9). El título de cada venta usa `productos.descripcion` («Casaca Emilia») y cae a `referencia` («CHO-001») si no la tiene; el chip de un comprobante `pendiente` dice «Pendiente hace N días» (contado en días de Lima desde que se emitió) en vez de verse igual que uno recién emitido; y los filtros suman los atajos «Hoy», «Ayer» y «Pendientes de comprobante» (`?comp=pendiente`, con su opción en el desplegable y su chip). Sin migraciones: todo sale de columnas que producción ya tiene.
Felipe se lleva: (1) **un dato faltante en catálogo se ve primero en pantallas como esta**: 18 de 39 líneas vendidas (46 %) no tienen descripción, y el código solo puede mostrar la referencia como respaldo; el arreglo de fondo es cargar los nombres; (2) «pendiente» dejó de ser un rótulo y pasó a ser una edad: producción tiene 17 de 19 comprobantes sin transmitirse, el más viejo del 14-sep; (3) el rojo se reservó para lo que ya lo usa —no se pintó de rojo lo viejo porque con 17 filas rompería el máximo de dos por pantalla.
Sin resolver: por qué esos comprobantes no se transmiten (es de Facturación, no del Historial); las otras 9 tareas de la auditoría; y verlo con clics reales en el navegador, que no se hizo (el worktree no tiene base local ni `.env`).
