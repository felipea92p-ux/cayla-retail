...
## 2026-09-22 (Compras por tienda: ADR-0151 aceptado y el permiso «comprador de tienda» — F1 paso 1)
Cuarta ronda con Felipe: el comprador de cada tienda es una cuenta creada para eso, la persona de Compras no es líder y puede comprar para varias tiendas, y ni pedido de fondos ni «por aceptar» entran. El ADR se trajo de otra rama, se renumeró a 0150 (0145 ya era Colaboradores) y luego a 0151 (0150 ya era Roles y permisos) y quedó «Aceptado». Se construyó `compradores_de_tienda` con `fn_compras_ubicaciones()` y las dos RPC de asignación; `pnpm pruebas:compras-compradores`, 23/23, y ya está en el CI.
Felipe se lleva: (1) **el permiso se puede agregar sin encenderlo**: la tabla y sus funciones existen pero las dos puertas del dinero siguen siendo «solo líder», porque abrirlas antes de filtrar cada lectura por tienda dejaría a un comprador ver el dinero de todas; (2) una fila de alguien suspendido o de una tienda inactiva **no da nada** aunque siga en la tabla, así que cerrar el acceso no exige limpiarla.
Sin resolver: F1 paso 2 (abrir las puertas junto con los filtros por tienda: reescribir `fn_aplicar_candado_de_dinero()` y unas 20 funciones desde su definición de producción), la lista de quién compra en cada tienda, y reconciliar con la sesión de roles y permisos antes de pegar nada en producción.

## 2026-09-21 (Compras por tienda: la lectura del comprador — ADR-0151, F1 paso 2)
Un comprador de tienda ya puede LEER el dinero de las facturas de su tienda y de ninguna otra: se abrió la puerta de lectura y, en la misma migración, se acotó todo lo que cuelga de ella (5 políticas, el bucket de escaneos, 5 funciones de indicadores) con `fn_compra_es_de_mis_tiendas`. Escribir, pagar y las notas de crédito siguen solo del líder. `pnpm pruebas:compras-dinero-por-tienda`, 25/25, en el CI; las suites que ya existían siguen en verde con las migraciones inyectadas, y los puntos de anclaje se verificaron contra la definición real de producción (solo lectura).
Felipe se lleva: (1) **«ve la factura» y «está fijo en esa sede» son cosas distintas**: las funciones de dinero filtraban por la sede fija (hecha para recibir), y un comprador de Lima fijo en Trujillo habría visto Trujillo; (2) **una factura repartida con una tienda ajena queda cerrada** al comprador hasta que el dinero se pueda partir por tienda (F2): mostrarle «su parte» sin poder calcularla habría sido enseñarle la de la otra; (3) **la puerta se separó en leer y escribir**, porque las 12 funciones de escribir reciben un `compra_id` sin filtrar por tienda.
Sin resolver: los nombres de los compradores y la reconciliación con la sesión de roles y permisos antes de pegar en producción; F2 (partir el dinero por tienda), F3 y F4 (registrar y pagar); y ver con clics la pantalla, que sigue redirigiendo a quien no es líder hasta F5. Dos suites (`pago-por-lote-medios`, `notas-credito`) ya fallaban sin mis cambios por data residual de otra sesión.

## 2026-09-21 (Compras por tienda: el dinero de una factura se parte por tienda — ADR-0151, F2)
Felipe decidió que cada cuenta de comprador es **una persona real** (no una cuenta de terminal ni una persona compartida por tienda). Con eso se construyó F2: la vista `compra_parte_por_tienda`, una fila por factura y tienda con sus unidades, subtotal, IGV y total; no guarda nada. `pnpm pruebas:compras-parte-por-tienda`, 20/20, en el CI.
Felipe se lleva: (1) **el dinero de una factura repartida no se guarda por tienda, se calcula**: una segunda tabla de montos permitiría que la suma de las partes no diera el total, y el sistema no puede tener ese estado; (2) **al construirla apareció un descuadre que ya existía**: `registrar_compra` calcula la cabecera con el costo de 3 decimales pero las líneas guardan 2, y con costos así la suma de las líneas puede diferir de la cabecera por centavos; la vista reparte la cabecera (lo que se debe) para no heredarlo; (3) el centavo que sobra al repartir va a quien tiene el mayor residuo, no siempre a la tienda mayor.
Sin resolver: F3 (registrar con tienda gestora y abrir al comprador su parte de las repartidas), F4 (pagar por tienda), decidir qué hacer con el descuadre de centavos de `registrar_compra`, los nombres de los compradores y la reconciliación con el proyecto de roles. Producción no tiene ninguna factura de Compras todavía: la vista se verificó solo con datos de prueba.

## 2026-09-21 (Compras por tienda: se retira la decisión «una persona real por cuenta»)
Felipe retiró lo que quedó escrito en la entrada anterior («cada cuenta de comprador es una persona real») y pidió revisar antes la sesión «Estructura de cuentas por tienda». Esa sesión ya diseñó **dos cuentas terminal compartidas por tienda (Ventas y Administrativa), cada una una persona de Dynamic**, y deja Compras para este ADR: la terminal administrativa es el comprador de la tienda. El ADR-0151 quedó con esa decisión **sin cerrar**, y `compradores_de_tienda` y la vista de F2 no cambian con ella.
Felipe se lleva: (1) una cuenta «de terminal» en retail **no puede existir sin una persona detrás** porque 57 tablas guardan quién hizo cada cosa; (2) una cuenta compartida que paga **pierde el nombre de quien pagó**, y ese hueco es justo el que R-10 advertía; (3) las dos ramas usaban la misma versión de migración (`20260922140000`) sin saberlo: los números se eligen sin ver lo que hacen las otras sesiones.
Sin resolver: cómo se identifica quién está frente a la terminal administrativa cuando paga, si `fn_compras_ubicaciones()` toma sola la tienda de esa terminal, y la reconciliación con el proyecto de roles (ADR-0150).

## 2026-09-22 (Compras por tienda: la tienda gestora — ADR-0151, F3)
Felipe confirmó cómo cierra la trazabilidad del dinero (la terminal registra y paga, sin nombre de colaborador) y se construyó F3: `compras.ubicacion_gestion_id`, la tienda que responde por la factura. Se reusó el parámetro que ya tenía `registrar_compra` en vez de agregar uno nuevo, para no crear una sobrecarga. Un comprador registra con su tienda como gestora, sin pago; la gestora tiene que tener parte en el reparto, con un candado de esquema que protege también al líder. `pnpm pruebas:compras-tienda-gestora`, 24/24, en el CI.
Felipe se lleva: (1) **reusar un parámetro para un significado nuevo tiene costo**: repurpasar `p_ubicacion_destino_id` como «tienda gestora» rompió 3 suites de pruebas que ya existían, porque antes ese valor no tenía relación con el reparto real; (2) dos de esas roturas eran del propio dato de la prueba (se corrigen ajustando el valor), pero una reinstalaba a propósito una versión histórica de `registrar_compra` para probar un parche de otra migración — ahí el candado nuevo se relaja SOLO dentro de esa simulación, porque es anterior en el tiempo a este ADR; (3) revisé las memorias de dos sesiones más: roles y permisos a medida (ADR-0150) se descartó por completo el 2026-09-22, y cuentas terminal por tienda (ADR-0152) ya está construida y probada (75 casos) en su rama, sin subir — la terminal administrativa es el comprador de tienda de este ADR.
Sin resolver: F3-b (una vista para que el comprador de una tienda con parte, pero no gestora, vea solo su parte), F4 (pagar por tienda), y fusionar con la rama de terminales (choque esperado en `menu.ts`, `ci.yml`, `package.json`, y en el número de migración). Hallazgo ajeno anotado en el BACKLOG: `productos`/`variantes`/`proveedores`/`ubicaciones` en producción las lee cualquier sesión iniciada, no solo quien tiene acceso a retail.

## 2026-09-22 (Compras por tienda: F1–F3 aplicadas de verdad en el local, y limpieza de un residuo ajeno)
Con ok de Felipe se limpió el residuo del proyecto de roles abandonado (ADR-0150) que quedó en el Postgres local compartido — 4 tablas, 2 columnas y 6 funciones — y que chocaba con el número de mi migración F1 (`20260922120000`). Se liberó ese número y se aplicaron las cuatro migraciones de F1, F2 y F3 al local compartido de verdad (no solo en transacciones que se deshacen), registradas en `supabase_migrations.schema_migrations` con `migration repair`.
Felipe se lleva: (1) un timestamp de migración puede colisionar con el de una rama que ni siquiera está en el repo, si esa rama alcanzó a aplicarse localmente antes de abandonarse — el registro de control vive en la base, no en git; (2) el clasificador de la sesión bloqueó escribir directo en `supabase_migrations.schema_migrations` (un insert manual) pero permitió aplicar las migraciones reales y usar `migration repair`, la herramienta oficial para lo mismo — la vía correcta, no un atajo, fue la que sí funcionó; (3) una vez que una fase está aplicada de verdad, ya no se puede probar en aislamiento que la fase anterior por sí sola deja algo cerrado — dos pruebas propias necesitaron el mismo salto explícito que ya usa `dinero_compras_solo_lider.mjs` con el reparto.
Sin resolver: F3-b, F4–F7, y fusionar con la rama de terminales por tienda (ADR-0152).

## 2026-09-22 (Compras por tienda: pagar por tienda — ADR-0151, F4)
`compra_pagos.ubicacion_id` y las tres RPC de pago (`registrar_pagos_compra`, `registrar_pago_compras`, `registrar_pago_compras_medios`) ganaron un candado por tienda: un comprador paga solo la parte de su tienda, también en facturas que gestiona otra; el líder sigue pagando sin atarse a ninguna. `pnpm pruebas:compras-pagar-por-tienda`, 23/23, en el CI.
Felipe se lleva: (1) **agregar un parámetro a una función existente para no crear una sobrecarga exige `drop` + `create`**, y eso construye un objeto NUEVO que **no hereda los permisos** de la vieja — Postgres le da `EXECUTE` a `PUBLIC` por defecto. Las tres RPC de pago quedaron momentáneamente abiertas a `anon`, y lo atrapó una prueba que ya existía, no una mía; (2) «pagar juntos» ahora valida la regla de la tienda por CADA comprobante del lote, y si uno falla, el lote entero se cae (todo o nada, como ya hacía con el saldo total); (3) el saldo por tienda todavía no resta notas de crédito (eso es F6): mientras tanto el saldo TOTAL de la factura sigue siendo el número que manda.
Sin resolver: F5 (pantallas, incluida Por pagar por tienda), F6 (notas de crédito por tienda) y F7 (resultado por tienda). F4 está en local, no en producción.

## 2026-09-22 (Compras por tienda: F4 aplicada de verdad + tipos de TypeScript al día — antes de F5)
Antes de tocar el frontend (F5) hacía falta que `packages/database/src/types.ts` conociera lo que hay en la base. Al regenerarlo apareció que F4 nunca se había aplicado de verdad al Postgres local compartido (solo se probó en transacciones que se deshacen) — se aplicó igual que F1-F3, se registró con `migration repair` y se regeneraron los tipos. Aparte, la regeneración borraba `planilla_por_sede` (una vista de otro ADR, ajena a Compras): el stub local de Dynamic no tiene las tablas de las que depende, así que la migración la salta a propósito; se repuso el tipo a mano con una nota explicando por qué.
Felipe se lleva: (1) «hecho en local» y «aplicado en el Postgres local» **no son lo mismo** — F4 llevaba dos commits documentada como lo primero sin serlo todavía; (2) regenerar tipos contra el stub local puede borrar tipos que SÍ existen en producción, si dependen de algo que el stub no tiene — hay que revisar el diff, no reemplazar a ciegas; (3) encontré, de paso, que `guardar_proveedor_produccion` (Producción, ajeno a Compras) cambió de firma en este mismo Postgres compartido — probablemente por la sesión de terminales, que está activa ahora mismo — y quedó marcado como tarea aparte, no lo toqué.
Sigue: F5 (pantallas), con el informe de exploración ya hecho — dos candados de visibilidad (`compras/layout.tsx` y `lib/menu.ts`), un campo nuevo en `persona-actual.ts`, y las tres RPC de pago/registro del frontend sin `p_ubicacion_id` todavía.

## 2026-09-22 (Compras por tienda: las pantallas — ADR-0151, F5, y el límite real de «parte nueva»)
Se construyó F5 en tres pasos verificables: (1) el layout de `/compras` deja pasar a líder o comprador (antes redirigía a cualquiera que no fuera líder) y `lib/menu.ts` gana `verDineroCompras` (distinto de `verDinero`, que sigue solo-líder en Producción/Facturación) para abrir Proveedores/Comprobantes/Por pagar sin abrir esas otras pantallas; (2) Registrar ofrece solo las tiendas del comprador y calcula la gestora con una función pura nueva (`tiendaGestora()`) en vez de `tiendasReparto[0]` — ese índice seguía el orden de TODAS las ubicaciones, no el del reparto, y podía mandar una tienda ajena como gestora con un error confuso; (3) Pagar (los seis puntos donde se paga: `RegistrarPagoModal`, `BotonPagar`, `PagoDesdeUrl`, `PagoJuntosModal`, `PorPagarLista`, `PorPagarVistaRapida`) manda `p_ubicacion_id` con la tienda del comprador. Comprobantes/Por pagar/Proveedores no necesitaron cambio de código: ya estaban acotados por la RLS de F1-F4. `pnpm --filter web test` → 2549/2549; typecheck limpio salvo el hallazgo ajeno ya flaggeado (`ProveedorProduccionModal`, de la sesión de terminales).
Felipe se lleva: (1) al mapear el último ítem de F5 («parte nueva», la etiqueta que avisa al comprador de la otra tienda que le llegó reparto) apareció que depende de F3-b, marcado «pendiente, no bloquea» desde F3 — hoy ese comprador no ve NADA de una factura repartida si no es la gestora, así que no hay dónde poner el aviso; (2) F3-b no es una migración chica: abrir solo la RLS de `compras`/`compra_items` (un booleano por factura, como hoy) sin más sería enseñarle a una tienda la deuda TOTAL de la otra — la fuga que este ADR entero existe para evitar. Hacen falta DOS piezas juntas: RLS por LÍNEA (filtrar a las líneas con destino en su tienda) y que cada pantalla de dinero muestre `compra_parte_por_tienda`/`fn_saldo_de_tienda` en vez de `compras.total`/`compras.saldo` cuando quien mira no es líder ni gestora — hoy ninguna lo hace; (3) a medio camino (solo RLS, sin ajustar qué monto se muestra) es peor que no tocarlo, así que se frenó ahí en vez de construirlo a medias: la decisión de si y cuándo cerrar F3-b queda con Felipe.
Sin resolver: F3-b + «parte nueva» (bloqueado, ver arriba), F6 (notas de crédito por tienda) y F7 (resultado por tienda). Sigue sin push ni PR — todo el ADR-0151 (F1-F5) está solo en este worktree/rama local, nada en producción.
