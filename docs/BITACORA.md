...
## 2026-09-22 (Historial de ventas: búsqueda única, atajos y KPI — de la maqueta al código real)
Felipe pidió una maqueta del Historial con el lenguaje de CAYLA Dynamic (Stitch) y, viéndola, que se subiera a la interfaz de verdad, no solo el HTML. Se agregó una barra de búsqueda («boleta, clienta o prenda», `?q=`) resuelta en el servidor con el mismo patrón que ya usan Cambios y Devoluciones (`clasificarBusqueda`, varias consultas chicas por fuente — comprobante, `clientes`, `productos` — nunca un `.or()` entre tablas embebidas, porque eso recorta filas donde no se quiere); atajos «Hoy», «Ayer» y «Pendientes de comprobante · N» (nuevo valor de filtro `comprobante = "pendiente"`); y una franja de 4 cifras en el encabezado, con «por día vendido» dividiendo entre los días CON venta (H5 de la auditoría del 21), no entre todos los del rango.
Felipe se lleva: (1) **una maqueta en Stitch no es el final del camino** — sirve para acordar el lenguaje visual, pero «que se vea en el sistema» significa cablearlo contra las tablas reales, con RLS y sin romper lo que ya filtraba; (2) una búsqueda que cruza varias tablas se resuelve a IDS en consultas separadas y se intersecta con los demás filtros al final — no se mete la palabra buscada dentro de un filtro combinado, porque ahí es donde otro código de esta misma pantalla ya había encontrado el error de recortar filas; (3) un helper usado por dos pantallas (`literalParaIlike`) se exporta una vez, no se copia.
Sin resolver: esta sesión no tuvo Supabase local disponible (Docker con el puerto tomado por otra sesión en paralelo) — typecheck, lint y las 51 pruebas de reglas pasan, pero nadie tocó la barra ni el atajo con el mouse contra datos reales.
...
## 2026-09-22 (Archivar datos de prueba de producción antes de que TRU salga en vivo — D-54, ADR-0152)
...
## 2026-09-22 (Ficha de clienta v1, backend — ADR-0152, D-76/D-77)
