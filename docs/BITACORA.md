...
## 2026-09-25 (Spike: ayuda guiada por módulo)
Felipe pidió una ayuda en cada módulo que liste todo lo que se puede hacer y enseñe el paso a paso con un círculo donde presionar y el resto oscuro. Se armó un spike de un archivo con Vender, Caja y Traslados: panel «¿Qué quieres hacer?» con buscador, recorrido de la pantalla y 11 guías; la guía opera la pantalla real (la venta se hace de verdad), se termina antes con «Terminar guía» o `Esc` y se retoma desde el panel. Las 11 se recorrieron completas por script a 1024 y 375 px, sin errores.
Felipe se lleva: (1) **una guía que simula enseña otra cosa que la real**: por eso el clic pasa al botón de verdad; (2) **una guía no se rompe si la persona se salta un paso**: cada paso deja la pantalla lista antes de apuntar; (3) **el objetivo se marca con un nombre propio (`data-guia`), no con el estilo**: así un rediseño no deja a la guía señalando al vacío.
