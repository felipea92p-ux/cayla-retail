...
## 2026-09-24 (Comportamiento comercial piso vs. almacén en Análisis — ADR-0179, LOCAL, nada en producción)
...
## 2026-09-24 (Dominio de Inventario reconstruido sobre el modelo canónico de Felipe — ADR-0180, sucede a ADR-0179, LOCAL, nada en producción)
Felipe envió la definición completa del dominio comercial y pidió auditar lo de esta mañana (ADR-0179) contra ella sin darle prioridad por existir. Se corrió una auditoría de 8 hallazgos independientes ANTES de tocar código: confirmó que el reloj de exposición se reiniciaba al volver a piso (debía pausar/reanudar) y que «Rotación piso»/«Rotación total» estaban en soles (COGS) cuando debían ser en unidades — y encontró, de paso, un bug real YA EN PRODUCCIÓN (no de hoy): el bucket «total» del ledger zeroaba movimientos sin `sububicacion_id` resuelta, inflando en +711 unidades el stock de AQP/TRU desde la activación piso/almacén del 2026-09-14, afectando también columnas de ADR-0138 ya en producción. Se reconstruyó `lib/inventario-exposicion.ts` (dominio reutilizable, prefijo correcto por convención real del repo): cohortes con pausa/reanudación (señal `esMovimientoInterno` desde `motivo='movimiento_interno'`, la única real piso↔almacén), `rotacionUnidades` separada y sin tocar `rotacion.ts`, y las 7 categorías de Lectura con el texto EXACTO de Felipe (A–G). SQL corregido: bucket total, promedio de piso no diluido, «es venta» unificada en una función (antes duplicada) e incluye liquidación de prenda dañada como venta (ya ordenado el 17-09, seguía sin cumplirse). 24.267 pruebas en verde; verificado con clics reales contra Tienda Lima.
...
Sin resolver: las mismas decisiones pendientes de ADR-0181/0182; drill-down de Análisis a Movimientos (los datos ya lo permiten, falta `p_variante_id` en `fn_movimientos` y el enlace en la UI — no construido hoy, a pedido de Felipe); verificación visual — la sesión local sigue cerrada.
