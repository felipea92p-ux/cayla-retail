...
## 2026-09-21 (Datos de demostración de 90 días — Fase 2, demanda — ADR-0150)
Solo tablas temporales: 7.003 boletas y 11.004 prendas repartidas TRU 68 % / AQP 27 % / LIM 5 %, con julio fuerte, pico el 29-jul, hora fuerte 16-20 y las reglas de descuento de `registrar_venta`. La primera corrida dio S/ 1.145.841 (el plan decía ≈ S/ 900.000: se vendía demasiado de lo caro); se subió el peso de la baratura y quedó en S/ 962.230, con 24 productos sin venta en 30 días. Además se corrigió que «hoy» era el día UTC y no el de Lima, y se dejó una prenda descontinuada con stock (A14). Falta: fases 3-7.

## 2026-09-21 (Datos de demostración de 90 días — Fase 3, inventario inicial y abastecimiento — ADR-0150)
Cinco lectores en paralelo estudiaron compras, recepción, traslados, clasificación de movimientos y reglas del negocio (con un crítico que halló 15 contradicciones); de ahí salió el generador: 157 compras con su reparto por tienda, envíos, lotes, pagos, A4 y A5, 52 traslados del Taller (Lima se surte por olas martes/viernes) y ~6.400 subidas al piso, todo derivado de la demanda con una simulación diaria que garantiza saldos ≥ 0 (ya cazó un fallo real) y 12 chequeos que abortan. Como el archivo ya pesa 114 KB, se armó un arnés local (`scripts/demo/local`) con datos de la forma de producción, y se comprobó que 34 de 35 tablas y 22 de 23 funciones clave son idénticas a producción; la determinista se verificó con tres huellas iguales. Falta: revisión adversarial en curso, ensayo contra producción, fases 4-7.

## 2026-09-22 (Datos de demostración de 90 días — revisión adversarial de la Fase 3 — ADR-0150)
Tres lentes de revisión (estados imposibles, pantallas, riesgo para producción viva), cada hallazgo refutado por un escéptico aparte, encontraron dos bugs reales que un ensayo con `ROLLBACK` no delata: `transferencias.numero` se calculaba a mano y nunca sincronizaba la secuencia real (el primer traslado real tras el COMMIT habría chocado en cadena), y el segundo `SET CONSTRAINTS ALL IMMEDIATE` no revisaba nada (el candado de reparto por tienda lo pone una excepción propia de `recibir_compras()`, no el trigger diferido). Los dos están corregidos y verificados en local (tres corridas: por defecto, con el interruptor de la corrida definitiva, y por defecto otra vez). De paso se confirmó empíricamente que `setval()` sobrevive a un `ROLLBACK` (a diferencia de un `INSERT`): el interruptor nuevo queda documentado como de un solo uso, junto con el `COMMIT`, nunca antes. Dos hallazgos más (el lock de `codigos_correlativos` durante toda la corrida, y la falta de `deshacer-90-dias.sql` con su ciclo de FK) quedan documentados para la Fase 7. El revisor de pantallas se interrumpió por un choque de infraestructura (dos `Workflow` a la vez); relanzado como agente aparte, en curso.

## 2026-09-22 (Datos de demostración de 90 días — Fase 4, ventas/caja/comprobantes — ADR-0150)
Convierte la demanda de la Fase 2 en ventas reales: 6.995 ventas (S/ 932.430), 267 cajas (una por tienda y día, la cierra siempre un líder — D-13), 6.658 comprobantes (3,6 % factura, 34,4 % boletas con DNI; LIM se queda sin comprobante en sus 337 ventas porque no tiene serie registrada, un hueco previo, no algo que este seed deba tapar). Los 9 chequeos pasaron a la primera corrida, sin necesitar recalibración. Se corrigió un bug propio antes de correrlo: un ticket con pago mixto que sorteaba el mismo medio dos veces podía dejar `recibido` por debajo del monto al fusionarse, violando el CHECK real de `venta_pagos`. Se verificó, aprendiendo de la Fase 3, que ninguna de las 6 tablas que toca esta fase tiene un trigger diferido. Falta: la revisión adversarial (como la de la Fase 3) y las fases 5-7. El revisor de pantallas de la Fase 3 sigue corriendo en segundo plano.

## 2026-09-22 (Datos de demostración de 90 días — revisor de pantallas de la Fase 3, completado — ADR-0150)
El tercer revisor (interrumpido antes por un choque de infraestructura) terminó su segundo intento: 2 hallazgos. El importante es un bug real de producción, no del generador — `notas_credito_tablero()` y `compras_nota_pendiente()` comparan «¿esta compra ya tiene una nota con motivo faltante?» en vez de «¿este cierre concreto ya tiene alguna nota?», así que un cierre resuelto con motivo `devolucion` (como A5, construido correctamente) sigue apareciendo como pendiente para siempre — la pantalla de Notas de Crédito duplicaría el dinero. Reproducido contra las definiciones vivas de producción. Queda en BACKLOG como bug de producto (migración aparte, con el OK de Felipe) y como tarjeta separada (`task_55581ede`) para no mezclarlo con el trabajo del generador. El segundo hallazgo (Análisis de Inventario sale degenerado si alguna vez se comete Fase 1-3 sin la Fase 4) ya no es un riesgo real: la Fase 4 ya existe.

## 2026-09-22 (Doble conteo de notas de crédito de compras — corregido antes de la Fase 5)
Felipe pidió corregir el bug del punto anterior antes de seguir. Reproducido y corregido en Docker local (compra + cierre + nota `devolucion` → antes 2 filas, después 1 sola nota resuelta; un cierre genuinamente sin nota sigue apareciendo pendiente, sin regresión). Migración `20260922151800_notas_credito_pendiente_por_cierre.sql` pegada en producción por MCP y confirmada en vivo por la huella de la definición.
Felipe se lleva: (1) al ir a pegar apareció una decena de migraciones de OTRAS sesiones aplicadas en producción en la última media hora (colaboradores, ventas, devoluciones, comprobantes…) — antes de tocar dos funciones puntuales conviene comprobar que ninguna migración reciente ya las tocó (se hizo con un `ilike` sobre `supabase_migrations.schema_migrations`, no solo mirando nombres); (2) producción no tenía hoy ningún caso real duplicado — el bug llevaba tiempo ahí sin que nadie lo notara porque casi todas las notas hasta ahora son de motivo `faltante`; la siembra de 90 días iba a ser la primera en activarlo, por eso lo encontró el revisor antes que un líder real; (3) tarjeta `task_55581ede` retirada: se hizo en esta misma sesión, no aparte.

## 2026-09-22 (Investigación de la Fase 5 y primer parche: anulaciones + serie de NC)
7 agentes en paralelo (uno por subsistema de postventa/gastos/Taller) + una síntesis cruzada verificaron en vivo contra producción, no contra el plan de hace un día. Encontraron drift real que el plan no tenía: `crear_devolucion` ahora exige `motivo_codigo` de lista cerrada, y `anular_venta` (reescrita hoy mismo por otra sesión) bloquea anular una venta cuyo comprobante ya está `aceptado` — exactamente el estado de las 6.995 ventas que la Fase 4 ya sembró. También destapó que producción no tiene ninguna serie de Nota de Crédito para TRU/AQP: `aprobar_devolucion()` abortaría en casi todas las ~140 devoluciones planeadas. Felipe decidió: que el script registre NC01 (TRU)/NC02 (AQP) — única excepción autorizada a "no tocar series_comprobantes". Con eso, parcheé la sección 4.6b (ya comprometida): elige ~50 ventas para anular de forma determinista, con margen real antes de que cierre su caja; el loop de comprobantes de 4.7 les da `no_emitido` en vez de `aceptado`, con el mismo motivo/firmante/instante que la Fase 5 usará para el resto del acto de anular. Un chequeo nuevo confirma que coinciden exacto. Ensayado en Docker local: 54 anulaciones (dentro de 35-65 esperadas), todos los chequeos de la Fase 4 en verde, `ROLLBACK` limpio.
Felipe se lleva: (1) **un plan "verificado ayer" no sigue verificado hoy** — en un repo con varias sesiones escribiendo migraciones en paralelo, un día es tiempo suficiente para que una recomendación del propio plan quede obsoleta (la del RUC de proveedor de producción también lo estaba: apuntaba a un patrón que en realidad solo existe en el fixture local, no en el generador real); (2) parchear código YA comprometido de una fase anterior, cuando el drift lo exige, es más barato que aparentar que la fase sigue "cerrada" y dejar el hueco para más tarde; (3) el resto de la Fase 5 (cambios, devoluciones+NC, conteos, cuarentena, gastos, producción del Taller, cierre financiero de cajas) sigue en curso — 6 recetas más ya verificadas, falta escribirlas.

## 2026-09-22 (Fase 5.2 anulaciones y 5.9 cierre financiero de cajas)
Retomada en otra sesión desde el traspaso del ADR-0150. Antes de escribir, se releyó en vivo `anular_venta()` y `cerrar_caja()` (sin cambios desde la investigación) y los candados de `ventas`/`cajas`/`venta_anulacion_items` (ninguno impide los UPDATE). 5.2 completa el acto de anular sobre las 54 ventas de 4.6b: una fila por línea, reingreso `anulacion_venta` al mismo bucket solo si la prenda vuelve vendible (60 de 76), `ventas.estado = 'anulada'`. 5.9 recalcula todas las cajas sembradas con la fórmula completa de `cerrar_caja()` leyendo las tablas reales, preservando A1. Ensayo local: todo en verde, 30 cajas con anulación en efectivo recalculadas bien, `ROLLBACK` limpio.
Felipe se lleva: (1) **un solo cierre que lee las tablas reales vale por cuatro parches**: cambios, devoluciones y gastos van a tocar las mismas cajas, y ninguno de ellos tendrá que acordarse de ajustarlas; (2) anular una venta cuyo efectivo ya estaba en el cajón baja lo que el sistema espera al cierre — sin 5.9, 30 cajas habrían mostrado un sobrante falso; (3) quedan cambios, devoluciones+NC, conteos, cuarentena, gastos y Taller, todos entre 5.2 y 5.9.

## 2026-09-22 (Fase 5.3 cambios de talla y Fase 6 stock derivado)
Felipe pidió cerrar rápido para probar hoy. 5.3 siembra 172 cambios con el cuerpo de `registrar_cambio()`: 25 por defecto (la prenda devuelta entra a cuarentena con su `prendas_danadas`), el resto a la talla vecina del mismo color; nunca sobre una venta anulada y la prenda nueva solo sale si el piso la tiene en ese instante y hasta el final. La Fase 6 deriva `stock` (5.447 filas) con la misma regla que `fn_aplicar_movimiento()`, después de comprobar que el libro completo sembrado nunca baja de cero en ningún instante.
Felipe se lleva: (1) **la regla de «hay stock» no es la foto de hoy sino todo lo que viene después**: sacar una prenda del piso el martes puede dejar sin stock una venta real del jueves, por eso se mide el saldo mínimo futuro, no el actual; (2) ese filtro descartó dos tercios de los candidatos (79 de ~230) y hubo que sortear más; (3) quedan fuera de esta carga devoluciones+NC, conteos, gastos y el Taller, y todavía no existe `deshacer-90-dias.sql`.

## 2026-09-22 (Carga de 90 días en producción)
Felipe pidió cargarla hoy. Ensayo contra producción con el archivo completo, abortado a propósito al final: pasaron las 6 fases salvo un chequeo que miraba también la numeración real (B004 trae los números 2 y 3 con el contador en 28); se acotó a los sembrados. Con el OK de Felipe en el chat, carga definitiva por `supabase db query --linked` con `cayla_seed.definitivo = 'true'` y `COMMIT`, verificada en vivo: 7.000 ventas, 176 cambios, 6.663 comprobantes, 23.868 movimientos, stock sin negativos, secuencia de traslados sincronizada, las 3 cajas abiertas reales intactas.
Felipe se lleva: (1) **el ensayo contra la base real sí encuentra cosas que el local no**: producción había sido vaciada y su serie B004 traía un salto propio; (2) los comprobantes sembrados usan las series reales, así que la próxima boleta real de TRU será B004-4647; (3) falta `deshacer-90-dias.sql` — hasta que exista, la única vuelta atrás es el respaldo de las 03:36 de hoy, que borraría también todo lo real posterior.

## 2026-09-22 (deshacer-90-dias.sql y un tropiezo con setval en producción)
Escrito y ensayado el deshacer: borra solo lo sembrado, aborta si algo real cuelga de la demo, y devuelve los contadores. Probado en local (sembrar → deshacer → 82 de 83 tablas idénticas; la otra, por las series NC que se dejan a propósito) y en producción con aborto forzado. El primer ensayo en producción movió de verdad la secuencia de traslados (56 → 4), porque `setval()` no se deshace con un ROLLBACK: se devolvió a 56 al momento, antes de que ningún traslado real chocara, y ahora ese paso solo corre con el interruptor `cayla_seed.definitivo`.
Felipe se lleva: (1) **la marca «5eed» no es infalible**: un uuid real al azar empieza así 1 de cada 65.536 veces (pasó en local), por eso el deshacer revisa que cada fila «5eed» cuelgue de algo sembrado antes de borrarla; (2) ya hay una venta real de 7 prendas demo (B004-4647, pendiente), y mientras exista el deshacer se niega a correr; (3) **una advertencia escrita en un ADR no protege si no está en el código**: la trampa de `setval` estaba documentada y aun así mordió.
