...
La barra de 5 pestañas de abajo (Inicio, Punto de Venta, +, Inventario, Caja) mostraba 4 de 15+ módulos y el resto no tenía entrada desde el celular. Se retiró: el celular usa ahora EL MISMO lateral de escritorio como cajón que entra desde la izquierda con un ☰ junto al logo, siempre expandido, con «Nuevo» arriba y el perfil abajo; se cierra con la ✕, el velo, Escape o al tocar un destino. Las tres barras fijas que se subían 4.25 rem para esquivar las pestañas (`BarraFija`, el «Ver ticket» de Vender y la ficha de Nuevo producto) bajan al fondo. Probado a 375 px con el Supabase falso (abrir, cerrar por velo y por Escape, foco de vuelta al ☰, scroll bloqueado) y a 1280 px (lateral igual que antes).
