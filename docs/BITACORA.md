...
## 2026-09-25 (Rama del rediseño de Punto de Venta publicada + local nivelado con `main` — worktree `inspiring-herschel-9e65af`)
Felipe pidió una rama para GitHub para el rediseño de Punto de Venta y que este chat corriera en local viéndose igual a producción — el mismo pedido que ayer a otro worktree (ver entrada del 2026-09-24 en [[sync-datos-produccion-local-2026-09-24]]). La rama ya existía (`claude/rediseno-punto-venta-b0a101`, creada por el panel al abrir esta conversación, parada exactamente en el mismo commit que `main`): se empujó tal cual, sin crear una segunda. El Postgres local compartido (mismo contenedor que usan los demás worktrees activos) seguía con el espejo de datos reales de producción intacto — 3 productos, 12 variantes, 18 filas de stock, 26 movimientos, 6 sedes, 26 personas, 9 filas de `roles_historial` — pero el ESQUEMA había quedado 25 migraciones atrás de `main` (Finanzas avanzó fuerte el 24 y 25 de setiembre: planes de cuenta, gastos, cuentas y dinero, cierre de mes, presupuesto, balance...). Se nivelaron con `npx supabase migration up --local --include-all` (incluye la migración fantasma `20260924010700`, que ya tiene archivo real desde el release consolidado del 24 y aplicó sin error) — respaldo previo por si acaso (`pg_dump` de `retail`+`public` al scratchpad de la sesión, no committeado) — y los datos se verificaron intactos después: mismos 7 conteos. Quedan 282/294 archivos aplicados; los 12 restantes son scripts para pegar a mano en producción (`pegar-en-produccion-*.sql`, `activacion-*-produccion.sql`, etc.), nunca migraciones automáticas. Este worktree tampoco tenía `apps/web/.env.local` (nunca se versiona) — se creó apuntando al Supabase local (`http://127.0.0.1:54421`); no se levantó un `next dev` propio porque el puerto fijo `3010` de `launch.json` ya lo tiene tomado el worktree `ajuste-conteo-fisico-5a80df` (mismo commit que `main` + 1 doc): se verificó navegando a esa instancia desde el panel — sesión de Felipe ya iniciada, dashboard de Tienda TRU con los datos reales.
Felipe se lleva: (1) **"crear la rama" casi nunca es un paso real**: el panel ya la crea sola al abrir cada conversación nueva; lo que faltaba de verdad era publicarla y nivelar el local, no fundar una rama nueva; (2) **los datos no se desincronizan solos (nadie resetea el Postgres compartido), pero el esquema sí, en silencio, cada vez que `main` avanza y nadie corre las migraciones pendientes en el local compartido** — hoy fueron 25 de un tirón; vale la pena decidir si esto se automatiza en vez de depender de que alguien lo pida; (3) cada worktree nuevo nace sin servidor propio utilizable hasta que alguien crea su `.env.local` a mano — no es un dato incorrecto, es un archivo ausente, y el mensaje de error (puerto ocupado) no lo delata directamente.
Sin resolver: producción sigue con el catálogo de prueba chico (3 productos) — "ver exactamente producción" hoy significa ver ese catálogo chico, no un volumen real; este worktree sigue sin su propio `next dev` corriendo (usa el del worktree vecino) porque el puerto de `launch.json` es fijo — si esa otra sesión se cierra, levantar el servidor aquí (ya queda listo con `.env.local`).
