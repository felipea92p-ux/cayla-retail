...
## 2026-09-25 (El menú: Ventas primero)
Felipe pidió que en las cajas Ventas saliera arriba, con Catálogo e Inventario debajo. Se movió una sola fila del árbol del menú (`lib/menu.ts`): Ventas pasa de quinto a segundo, justo debajo de Inicio, para todos los perfiles. Una caja (que no tiene Inicio) ve ahora Ventas → Catálogo → Inventario; el líder, Inicio → Ventas → Catálogo → Compras o Producción → Inventario → Finanzas. La foto del menú (`menu-hoy.golden.json`) se actualizó a propósito, sin tocar otra cosa que esa fila, y una prueba nueva fija el orden de la caja. 89.192 pruebas en verde, y el lateral real se vio con una caja de TRU.
Felipe se lleva: (1) **un solo orden para todos es más barato que uno por perfil**: cada excepción es una regla más que alguien tiene que recordar al agregar un módulo; (2) **la foto del menú existe para que un cambio de orden no pase sin que nadie lo decida**: por eso se cambia a mano y en el mismo commit; (3) **el orden de las filas no cambia qué grupo se abre al entrar a una pantalla**, porque ninguna ruta de Ventas cuelga de la de otro módulo.
