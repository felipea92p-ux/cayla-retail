...
Felipe se lleva: **«nadie la nombra» no es «sobra»** — una función de la base puede tener quien la llame dentro de la base (otra función, un disparador) o fuera del front (script, Dynamic). Verificado: `node --test` 48 pruebas (29 de la lectura, 19 de extremo a extremo) y 19 mutaciones a mano que muerden. Sin resolver: refrescar la foto de funciones; una sobrecarga en producción no cambia el código de salida; una llamada indirecta a una función ausente sigue invisible.
