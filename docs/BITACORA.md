...
## 2026-09-26 (Los combos abren tan rápido como el selector de sede — ADR-0211 act. b)
Felipe pidió que todo combo abriera como el selector de sede. Medido en el navegador, la diferencia no estaba en los combos: fuera de un modal todos empataban con el de sede (se ven en 5–16 ms, enteros en 250–340 ms). Dentro de un `<Modal>`, en cambio, la lista colgaba como hija directa de la hoja y la cascada de entrada del modal la animaba como a una pieza del contenido: en «Registrar gasto» quedaba invisible 508 ms y entera recién a 1.008 ms, en los cinco tipos de combo. Ahora la hoja trae una capa vacía fuera de la cascada y `useDestinoFlotante` cuelga ahí la lista. Resultado: 3–18 ms para verse y 243–347 ms entera, dentro y fuera de un modal, también a 375 px en el ticket del Punto de venta. `ComboBuscable`, que aparecía de golpe, entra ahora como los demás.
Por qué así: un solo punto (la hoja y el hook) arregla los cinco combos de hoy y cualquier combo futuro que use el hook, sin que nadie tenga que acordarse de nada. Marcar cada combo con `data-sin-cascada` también funcionaba, pero el bug habría vuelto con el primero que se olvidara. Qué se rompería sin esto: la colaboradora toca «Categoría» en un gasto y por medio segundo no pasa nada; vuelve a tocar pensando que no entró el clic, y el segundo toque cierra la lista.
Felipe se lleva: (1) **«lento» no siempre es cálculo o red: a veces es una animación que espera su turno.** La lista se montaba en 5 ms y la cascada la tapaba 500 ms más. (2) **Medir antes de arreglar evitó un arreglo equivocado.** Contando cuadros, hasta el selector de sede parecía tardar 900 ms: el navegador integrado dibuja a 1 cuadro por segundo cuando está detrás. Se midió leyendo las animaciones, que no dependen de los cuadros.
Sin resolver: la misma cascada retrasa todo bloque que aparece DESPUÉS de abrir un modal (hasta ~0,6 s de espera). Hoy se esquiva a mano en 14 lugares; la propuesta de arreglarlo en la raíz está en el BACKLOG y la decide Felipe (toca la regla de modales, ADR-0136).
...
## 2026-09-26 (Caja como pantalla de trabajo — ADR-0226)
Felipe pidió que Caja ayudara a trabajar y no solo a mirar, en computadora y en celular. Hubo análisis, tres rondas de spike (`docs/maquetas/caja-tablero-spike-2026-09/`) y la implementación, solo web, sin migración. La cifra principal es el efectivo que debería haber en el cajón. Lo cobrado ya no cuenta el adelanto de las separaciones. Hay botones para hacer y píldoras para ver lo que cada una elige. «Cierres anteriores» tiene cuatro vistas, con una predeterminada por rol. En el celular hay una barra fija como la del Inicio.
Felipe se lleva: (1) **una pantalla de dinero puede tener un número de más**: el `anticipo` inflaba lo cobrado hoy porque se cobró otro día. (2) **Lo que la base ya da no se vuelve a calcular en la pantalla**: el efectivo esperado lo sigue diciendo `fn_calcular_esperado_caja`, y la pantalla solo muestra sus piezas. (3) **Leer no debe escribir**: la Caja lee los apartados sin «vencerlos», porque se relee cada vez que entra una venta.

## 2026-09-26 (Apartados: recordar en lote, con cada aviso anotado — ADR-0227)
Primer paso de las funciones del spike. En «Todos» aparece «N clientas por avisar hoy» y un botón que abre WhatsApp con el mensaje listo, una clienta tras otra; cada aviso queda anotado con quién lo dio y cuándo, y quien ya fue avisada hoy se ve con un visto verde. El mensaje a una clienta vencida ya no le dice «te espera hasta» una fecha pasada. La migración (`20260926233000`) está probada 53/53 en el Postgres local y **todavía no está en producción**: se pega antes de fusionar la web.
Felipe se lleva: (1) **anotar el aviso como una fila nueva, no como un dato que se pisa**, es lo que después deja contar la historia de cada apartado en «Actividad». (2) **El navegador solo abre una ventana en el mismo toque**: por eso primero se abre el chat y después se anota; si la anotación falla, la pantalla lo dice. (3) **«Hoy» se cuenta en hora de Lima**: un aviso de las 10 p. m. no cuenta como de mañana aunque el servidor ya esté en el día siguiente.
...
## 2026-09-26 (Inventario con la cabecera de Caja, Historial y Posventa — ADR-0220)
Pediste que las cabeceras de Inventario tomen como referencia las de Ventas. Las 9 pantallas usan ahora la misma pieza que Caja, Historial, Cambios y Devoluciones (`EncabezadoPagina`): la sede y la fecha arriba con el hilo, el nombre de la pantalla en grande (el del menú: «Existencias», no «Tienda TRU») y los botones bajo la frase, como «Cerrar caja». Las cifras no se movieron a la caja de la derecha: las de Inventario dicen más que un número y se habría perdido ese detalle. Se verificó sin sesión (ruta temporal + los componentes reales de Conteo); falta mirarlo con sesión real.
Felipe se lleva: (1) **una regla que nadie actualiza deshace el cambio**: CLAUDE.md mandaba a usar la otra cabecera en toda pantalla nueva; la próxima sesión habría devuelto Inventario a ella sin saberlo. Ahora dice «la cabecera es la de su módulo». (2) **Existencias es una foto, no un tablero en vivo**: por eso su línea de arriba dice «vista de las 10:21» y no un reloj que corre. (3) **Quedan tres cabeceras en el ERP** (Ventas e Inventario; Finanzas; y las escritas a mano en Catálogo, Compras, Producción…): cuál manda en el resto es tuyo.

## 2026-09-26 (Apartados en el celular: pasos y pestañas abajo — ADR-0223)
Primer paso del spike: en el teléfono, Apartar / Entregar / Todos bajan a la altura del pulgar, Apartar va en tres pasos (prendas → clienta → adelanto) con una barra negra que dice cuánto es y lleva al paso siguiente, y la cámara lee el QR como en Vender. Entraron también los arreglos de tus capturas: color · talla · código en cada prenda, el celular que empieza en 9, avisos apenas se deja el campo y los botones del modal que ya no se cortan. Sin migración; en computador la pantalla queda como estaba.
Felipe se lleva: (1) **el «· 1 u.» no era un error de pantalla sino de dato**: en producción casi ninguna variante tiene `sku` (el código vive en `codigo`), y Apartados leía el campo equivocado. (2) **Un `position: fixed` dentro de algo que se anima con `transform` deja de ser fijo**: por eso las barras del celular se montan fuera de la hoja. (3) **Validar al final no ayuda en el mostrador**: el DNI de 9 dígitos de tu captura pasaba sin aviso hasta tocar «Confirmar»; ahora avisa al salir del campo.

## 2026-09-26 (Existencias: «Reponer a piso hoy» lleva la vista a la tabla filtrada)
El clic en la tarjeta filtraba una tabla que quedaba fuera de la vista (en celular empieza a 1.054 px, con una pantalla de 812): parecía que no pasaba nada. Ahora la vista baja suave hasta los filtros y la tabla y los deja justo bajo la cabecera fija; al quitar el filtro no se mueve, y con «reducir movimiento» del sistema salta de una vez. De paso, al paginar, la cabecera tapaba 23 de los 40 px del buscador (margen de 16 px); ahora son 96, como en el resto del repo. Probado en un banco temporal sin sesión, ya borrado.
Felipe se lleva: (1) **si el resultado de un clic ocurre fuera de la vista, para quien lo usa el clic no hizo nada**: la respuesta tiene que llegar adonde está la mirada. (2) **Una cabecera fija le quita alto a todo destino de scroll**: por eso esos destinos llevan `scroll-mt-24`. (3) **El PR #445 cambia esa misma línea** (`setEstado` → `setAccion`): quien lo rebase debe conservar `mostrarTablaFiltrada()`.
...
## 2026-09-26 (Apartados v2: análisis, investigación y spike en computador y celular)
Analizamos Apartados con tus 15 capturas: 6 arreglos (el ticket sin color ni talla, «· 1 u.» en Entregar, el código vacío, el modal que corta sus botones, el DNI sin validar y «S/0.00 en el cajón») y el problema del celular: por debajo de 1024 px el cobro queda enterrado bajo la búsqueda. Elegiste ver las 8 funciones nuevas y las 3 formas de celular en una demo con «Opciones» y presets, en vez de decidir a ciegas: `docs/maquetas/apartados-v2-2026-09/` (spike interactivo, 7 capturas, README con la investigación de 13 referentes). Sin código ni migraciones.
Felipe se lleva: (1) **nadie grande en Perú hace «apartado» como CAYLA**: Ripley y Falabella hacen recojo ya pagado; el apartado con adelanto es de Burlington, Lightspeed y Odoo, y de ellos salen los abonos con una sola boleta final. (2) **Lo que no se copia importa tanto como lo que sí**: Square descuenta el stock recién al pago total, y eso vendería la misma prenda dos veces. (3) **Casi todo lo nuevo pide migración**: solo los arreglos, la cámara QR y la forma de celular salen en un PR solo web.


## 2026-09-26 (Productos ▸ «Eliminar», solo Admin y Líder — ADR-0218)
Pediste que las cuentas de Admin y Líder puedan eliminar productos. Existe ahora, con una regla: **solo se elimina lo que nunca se movió** (ninguna venta, ningún movimiento de stock, ninguna compra…), porque la historia del inventario es inmutable por diseño. La ventana pregunta a la base antes de ofrecer el botón, y si el producto tiene historia dice cuál y sugiere descontinuarlo. Con los datos de hoy, 6 de los 13 productos de producción se podrían eliminar; **Top Aurora, el de tu captura, no** (65 unidades y 7 líneas de venta). La migración `20260926220000` está probada 35/35 en un Postgres desechable (mutaciones y carreras reales con COMMIT incluidas) y **ya está en producción** (aplicada el mismo día con tu «dale», con ensayo previo revertido y verificada por efectos), antes que la web.
Felipe se lleva: (1) **«Eliminar» y «tiene historia» no se llevan**: por eso el botón explica en vez de fallar, y el camino para un producto de prueba con ventas es archivarlo (ADR-0159), no borrarlo. (2) **Una regla sana puede romper una pieza del sistema**: «Prenda sin Registrar» no es una prenda, es el cobro «Monto manual» del mostrador, y en producción no tiene movimientos: «sin historia = borrable» la habría dejado eliminar. Se protegió por nombre propio. (3) **Borrar más cuenta más que borrar menos**: por eso es solo del Líder, aunque editar el catálogo lo puedan hacer más roles.
...
## 2026-09-25 (Alta de producto sin red, combo con memoria y service worker — ADR-0210 «(b)»)
Se cerró lo que faltaba del modo sin conexión para agregar productos. **Alta**: sin red, la prenda queda «pendiente de código» (la base reparte el código al subir) y sus fotos esperan en IndexedDB y suben después del producto; la pantalla de éxito cambia sola a «ya subió». **Combo Responsable**: recuerda la última lista de turno por sede (máx. 12 h), porque sin ella una pantalla abierta sin red no dejaba guardar nada. **Service worker**: guarda la última copia de Vender, Recibir, Ingreso sin comprobante y Nuevo producto y la abre sin red. Vender solo abre con una copia de hoy (Felipe eligió «Copia de hoy»); cualquier otra pantalla muestra «Sin conexión». Las copias se borran al salir o al cambiar de cuenta; las colas no. Probado en local apagando el servidor: las 4 copias, un clic de menú a una copia, Finanzas con «Sin conexión», Vender con copia de ayer bloqueado, el aviso con la hora de Lima, y un alta sin red que volvió como `CIN-0001` con 3 variantes. `tsc`/`eslint`/77.033 pruebas en verde.
Felipe se lleva: (1) **un service worker solo guarda pantallas, nunca responde por la base**: lo que se guarda sin red lo guarda la cola de cada módulo; el SW solo hace que la pantalla abra; (2) **qué pantallas se guardan es una decisión de seguridad**: solo las 4 con cola; Finanzas en el disco de un equipo compartido sería un riesgo sin ganancia; (3) **el conteo es otro módulo**: guarda escaneo por escaneo, y hacer offline solo su alta al vuelo no serviría. Queda como pregunta abierta.
...
## 2026-09-25 (Buscador y paginado: la regla global de combos — ADR-0209, núcleo)
Felipe pidió una regla pareja: más de 8 opciones, se puede buscar; más de 50, la lista pagina sola al bajar el scroll — y estandarizar los combos, que hoy son 4 implementaciones distintas del mismo problema. Se construyó el núcleo puro (`lib/combo-reglas.ts`, testeado) y el estado compartido (`useCombo.ts`), y lo aplican `Desplegable`/`CampoSelect` y `ComboBuscable` sin romper el caso corto (con 8 opciones o menos, ninguno cambia). Piloto en `FiltrosProductos.tsx` (Proveedor/Marca/Categoría/Color/Estado/Stock), verificado en navegador con una página temporal (120 proveedores, borrada) y confirmado que en producción real (75 proveedores, 79 marcas) el efecto ya se nota. Suite completa en verde (24 373 pruebas).
Felipe se lleva: (1) **"un combo" ya eran cuatro cosas distintas** (`<select>` nativo, `SelectNativo`, `Desplegable`, `ComboBuscable`) resolviendo el mismo problema cada una a su manera — media hora de auditoría antes de escribir código evitó construir una quinta; (2) **el escalonado de entrada (30ms por fila) que hacía ver bien una lista de 6 opciones dejaba una lista de 100 invisible casi 3 segundos** — una animación pensada para un caso no sobrevive sola a otro caso 20 veces más grande; (3) **migrar ~30 pantallas más (`SelectNativo`, `DesplegablePildora`, `ComboResponsable`) queda para decidir alcance con Felipe**, no se hizo de una por cruzar varios módulos a la vez.
## 2026-09-25 (Triaje del BACKLOG contra producción y seis avances sin decisión pendiente)
Felipe pidió «dar avances» sin decir cuáles. En vez de elegir a ciegas se clasificaron las 525 casillas abiertas del BACKLOG contra el repo y contra producción (solo lectura): 44 % tenía el texto desfasado, y solo 15 de 133 «Claude lo hace solo» sobrevivieron a un escéptico. Con esas se construyeron seis ramas, cada una en su PR (Reintentar, prueba de arquitectura cliente/servidor, «apartada» en Vender y Cambios, ficha sin tarjetas TODO, CI con 15 pruebas cableadas y un candado de una sola firma por función, diccionario de datos refrescado), cada una con dos revisores independientes que intentaron romperla y, donde hubo hallazgos, un tercero que comprobó que quedaron cerrados.
Felipe se lleva: (1) **un backlog de 6.000 líneas que nadie poda miente**: hasta un agente dio por «URGENTE» algo que Felipe ya había pegado en producción; manda lo que se comprueba contra la base, con fecha; (2) **una réplica en texto del motor pierde contra el motor**: el triaje afirmó dos funciones duplicadas y una base real con las 277 migraciones las desmintió (534 funciones, 534 firmas), así que el candado se rehízo contra la base; (3) **hoy el cuello de botella no es construir sino decidir**: las acciones de más peso (Apartados en los roles, las series `BC04`/`BC05`, Lima) son de Felipe.
Sin resolver: fusionar los PR, la captura a 375 px de Vender y Cambios (PL-105; la rama «apartada» debe integrar el stock en vivo de `main` antes de fusionarse), reescribir el BACKLOG con `/backlog`, y las decisiones de dinero listadas en el BACKLOG (nota de crédito con descuento, caja sin líder, plazo de Devoluciones).

## 2026-09-25 (El menú de la caja, plano)
Felipe pidió aplanar el menú de la caja y eligió «Ventas suelto» entre tres opciones. La caja (una terminal que ve el Punto de venta) ya no abre el grupo Ventas: ve Punto de Venta, Caja, Historial, Cambios, Devoluciones y Comprobantes directo en el lateral, y debajo Inventario y Catálogo como grupos (en el orden que dejó #402). Son ocho filas y entran sin scroll. Es un dato del árbol (`sueltoEnMostrador` en el grupo Ventas) más una definición única de «caja del mostrador» (`esMostrador`), que ya usaba el aterrizaje en `/vender`. El menú del líder y el de las personas no cambian: la foto del menú quedó igual. Se vio en el lateral real, abierto, plegado y en celular. 77.004 pruebas en verde.
Felipe se lleva: (1) **el mostrador es el aparato, no quien lo usa**: una persona que vende sigue viendo Ventas agrupado, porque su menú tiene más cosas; (2) **aplanar todo no es simplificar**: 14 filas sin grupos cuestan más que 8 con dos grupos que casi no se abren; (3) **la regla que ya existía alcanzó**: un grupo con una sola pantalla ya se disolvía; aplanar es aplicar esa misma regla un nivel más arriba.

## 2026-09-25 (El piloto de CI vuelve a verde, y las ADR de #397 dejan de decir «solo local»)
Se reprodujo la falla de `pruebas:roles` con bases limpias de cada commit: la de `0bec5912` da el mismo 69/70 que el CI y la de `main` da 70/70, con las otras tres suites de Análisis en verde. El cuerpo de `fn_resumen_comparacion` es idéntico en una base limpia de `main` y en producción. Al revisar, las ADR 0199–0203 seguían diciendo «nada aplicado a producción», pero las dos migraciones de #397 están allí. Se corrigió eso, con un aviso: volver a pegar `010700` sola hoy rompería Análisis. La ADR-0202 atribuía la falla al seed y se corrigió también.
Felipe se lleva: (1) **una ADR que dice «local» cuando ya está en producción invita a pegar otra vez**, y pegar un archivo viejo deshace lo nuevo sin ningún error; (2) **si una prueba falla en el CI, que parte de una base limpia, la causa no es el seed de nadie**: es el código; (3) **dos sesiones arreglaron lo mismo en paralelo** (#404 y #405) y por suerte no chocaron: la migración nueva revisaba si el filtro ya estaba puesto.

## 2026-09-25 (Etiqueta de precio: letras que la térmica sí imprime, y el papel de la Mac — ADR-0180)
Felipe mandó una foto: la etiqueta no llenaba el papel y las letras finas salían débiles. Medida con la perspectiva corregida, el corte era de 62 × ~100 mm (el papel «62 mm» de la Mac) y lo impreso era la versión del 23-sep; aun así, sus letras eran del tamaño de las de hoy y se cortaban los trazos de ~1,7 puntos («CUFLLO»). Chrome en la Mac no elige papel ni escala: el corte de 40,1 mm pide un papel propio creado con ⌥⌘P, y así lo dicen ahora la pantalla y el ADR. En el código: trazo mínimo 0,2 mm (textos chicos en 600–700 y un poco más grandes, colibrí y reglas de 0,25), QR de 19 mm para que el código de 16 caracteres ya no salga «CMS-0011-NAR…», y la forma elegida (A/B) vuelve a leerse. Banco de 6 variantes a 300 dpi, app local con `@media print` emulado y 77.000 pruebas en verde; falta imprimirla en la Brother.
Felipe se lleva: (1) **en una térmica el enemigo es el trazo, no el tamaño**: una letra de 2 mm en peso 400 tiene barras de 1,7 puntos y se cortan; en 600 ya pasan de 2,4; (2) **el largo del corte lo decide el papel del diálogo, nunca la página web**: por eso la solución de «no llena el papel» está en la Mac, no en el CSS; (3) **la fecha impresa es un control de versión**: si no dice hoy, la pestaña es vieja y hay que recargar.

## 2026-09-25 (Ticket de la boleta en 80 mm: un tercio menos de papel)
El ticket salía demasiado largo, incluso con una sola prenda. Se compactó `#comprobante-print` en `globals.css` sin cambiar el diseño: el texto base baja de 9,5 a 8 pt, los espacios se reducen a la mitad, el logo pasa de 21 a 13 mm, el QR de 32 a 25 mm y la marca, el número y el TOTAL bajan en proporción. El TOTAL sigue siendo lo más grande y las reglas no cambian. En `ReciboTermico` el código y el P.U. de cada prenda van ahora en una sola línea. Medido en una maqueta con el mismo marcado: con 1 prenda, 259 → 171 mm; con 3, 297 → 190 mm. Cada prenda pasa de ~19 a ~10 mm. El ticket del apartado usa las mismas clases y se acorta igual.
Felipe se lleva: (1) **en una térmica, cada mm de aire se paga en papel en CADA venta**: más que la letra, lo que alargaba el ticket eran los márgenes; (2) **8 pt es el piso**: más chico, una térmica de 203 dpi empasta la Arial; (3) **falta probarlo en la térmica real** y escanear el QR de 25 mm con un celular.

## 2026-09-25 (Descuento en Vender: argumento pasado el 15 % y guía de paso)
En Vender ▸ Descuento, todo descuento manual que pase el 15 % pide ahora un argumento escrito, lo aplique quien lo aplique. Antes solo se le pedía a un Líder y pasado el 20 %. El descuento pasa a ser solo en %: atajos 5, 10 y 15, el resto en «Otro», y sale la entrada en S/ (con sus dos funciones, ya sin uso). El código de la Colaboradora sigue igual: Felipe descartó por ahora la idea de pedir código solo para «Otro». El campo que falta se ilumina con la misma luz que da el foco; al completar uno, el foco salta al siguiente. «Todo el ticket» ahora se desmarca al tocarlo de nuevo, en celular la barra «Ver ticket» se esconde mientras el ticket está a la vista, y al pasar a Cobro (o a Descuento, o de vuelta) la vista sube a la cabecera del ticket en vez de quedarse en el pie con aire vacío abajo (`soltarPaginaEstable`: un cambio de vista sin cambiar de URL se trata como un enlace, ADR-0185). Se probó a 375 px con clics reales en local. La migración `20260925230000` se probó en local dentro de una transacción que se deshizo al final, y se aplicó en producción el mismo día, verificada en solo lectura.
Felipe se lleva: (1) **la pantalla y la base cuentan el 15 % igual, al céntimo**: si no, un 15 % justo se pediría en un lado y en el otro no; (2) **guiar no es quitarle el foco a quien escribe**: el salto ocurre al tocar un botón o con Enter, nunca a mitad de una cifra; (3) **una regla de dinero vive en la base**: la pantalla solo la adelanta.

## 2026-09-25 (Stock en vivo, ampliado a Apartados y Cambios — y auditoría de todo lo demás)
Felipe pidió llevar el arreglo de Vender a "todo el sistema" y auditar producción entera. Se lanzaron 3 agentes en paralelo sobre los ~30 módulos: el hallazgo central es que casi toda escritura sobre una fila compartida YA pasa por un candado de servidor (`for update`/`pg_advisory_xact_lock`, gran parte del propio Felipe el día anterior en Inventario, ADR-0188/0189/0190) que corta limpio si dos personas chocan — "tiempo real" acá es pulido de pantalla, no lo que evita un descuadre, eso ya lo hace la base. El único patrón real de "pantalla vieja sin ningún candado" —mismo bug que Vender— estaba en Apartados y Cambios; se extendió ahí el mismo sondeo, ahora factorizado en `lib/useStockEnVivo.ts` (compartido por los tres, `PuntoDeVenta.tsx` se simplificó para usarlo también). El resto (Devoluciones, Inventario, Compras, Producción, Finanzas, Colaboradores) se dejó sin tocar a propósito: agregar sondeo ahí sería tráfico de fondo sin ganancia de integridad. Un hallazgo aparte —la ficha de clienta compartida entre sedes sin candado en su UPDATE— resultó, al mirar el código, no aplicar: esa función de editar no existe todavía (solo alta y búsqueda), así que no había nada que candar; quedó la nota correcta en BACKLOG para cuando se construya (concurrencia optimista con `updated_at`, no un `for update`). `tsc`/`eslint`/76.992 pruebas en verde.
Felipe se lleva: (1) **"todo sincronizado" y "cero estados inconsistentes" son cosas distintas**: lo segundo ya lo garantiza la base con candados, independiente de lo que muestre cualquier pantalla — el sondeo solo evita el mensaje de error ocasional, no un dato corrupto; (2) **la tercera vez que se repite un patrón, se factoriza**: con Vender+Apartados+Cambios pidiendo lo mismo, el sondeo pasó de código pegado en `PuntoDeVenta.tsx` a un hook propio; (3) **un hallazgo de auditoría puede no sobrevivir la lectura del código real**: la recomendación sobre la ficha de clienta sonaba bien en la sala pero la función que había que candar simplemente no existe — se corrigió en vez de forzar un cambio sobre código imaginario.

## 2026-09-25 (Vender: el stock se repone en vivo, sin recargar — cierra un hueco de ADR-0192)
Felipe escaneaba con la cámara del teléfono, leyó una prenda «agotada», la repusieron en otra máquina con la cámara todavía abierta, y no se sumó hasta reiniciar el navegador. `PuntoDeVenta` solo corregía el stock que ESTA MISMA caja vendía (`releerStock`/`ajustesStock`, ADR-0192); nada releía la base mientras la pantalla seguía montada — el propio ADR-0192 ya dejaba anotado el hueco ("otra caja vende la misma prenda: esta pantalla no se entera hasta la próxima carga"), y afecta igual al lector físico: no es un problema de la cámara. `releerStock` gana un modo sin `ids` (relee TODA la sede, sin `.in`, para no armar una URL con cientos de ids) y un efecto nuevo lo sondea cada 10 s — mismo patrón que `useCajaEnVivo`: sondeo, no Supabase Realtime (sigue apagada, ADR-0018, «Actualización 2026-09-25»). `tsc`/`eslint`/76.992 pruebas en verde; verificado contra el Postgres local por PostgREST (repuse stock desde una conexión aparte simulando otra máquina, y la siguiente lectura sin ids lo vio al instante) — sin sesión de navegador con usuario, mismo límite que ya tenía ADR-0192.
Felipe se lleva: (1) **"tiempo real" en este ERP es sondeo, no Realtime**: Supabase Realtime sigue apagado a propósito (ADR-0018, activarlo toca el proyecto compartido con Dynamic) y cada pantalla que necesita verse al día arma su propio sondeo corto sobre la misma lectura que ya tenía; (2) **el hueco no era de la cámara**: cualquier Vender abierto un rato (lector físico incluido) se quedaba con el stock del momento en que se entró — la cámara solo lo hizo visible porque esa sesión se queda abierta más tiempo escaneando; (3) **releer TODA la sede, no solo el ticket**: pedir con `.in` cientos de ids de golpe arma una URL gigante, así que el sondeo de fondo lee la sede entera sin filtro — igual de barato a esta escala (3 tiendas + 1 taller).

## 2026-09-25 (Vender en el teléfono: escanear el QR con la cámara)
En un teléfono no hay lector que «escriba» el código, así que en Vender el campo de escaneo pasa a ser un botón **Escanear QR** que abre la cámara (`EscanerCamara`, con `<Modal>`); la lupa de al lado cambia a buscar por nombre. Cada lectura va por el MISMO camino que el Enter del lector (`resolverCodigoV2` + `agregar`), la cámara queda abierta para pasar varias prendas, y la misma etiqueta se ignora 2 s para no sumarla ocho veces. Lee con el `BarcodeDetector` del navegador (Android) o con `jsQR` (iPhone), todo en el aparato. Una tablet con lector bluetooth sigue con el campo: solo cambia con pantalla táctil y angosta. Una prueba nueva genera el QR que imprime la etiqueta y comprueba que `jsQR` lo lee.
Después, a pedido de Felipe, la cámara pasó a pantalla completa con visor, linterna (donde el teléfono la tiene) y una bandeja con las últimas prendas y el ticket; al leer, la tarjeta de la prenda vuela en arco hasta la bolsa del ticket y recién ahí cambian el número y el total. Lo que no entra aparece en ámbar y se apaga en su lugar. `<Modal>` ganó la variante `camara`.
Felipe se lleva: (1) **la cámara es otro lector, no otra forma de vender**: entrega el mismo texto y la regla de qué prenda es sigue en un solo lugar; (2) **una cámara ve la etiqueta muchas veces por segundo**: sin la pausa, una prenda se cobraría varias veces; (3) **la cámara del teléfono solo se abre en HTTPS** (o en localhost): para probar desde el celular sirve una vista previa de Vercel, no la IP de la red local.

## 2026-09-25 (Celular: el menú pasa a un cajón lateral — primer paso del trabajo responsive)
La barra de pestañas de abajo mostraba 4 de 15+ módulos. Ese mismo día otra sesión le sumó «Más» (una hoja con todo el menú), lupa y avatar (ADR-0205); al traer `main`, Felipe eligió el cajón y se retiraron la barra y «Más» (ADR-0206; la lupa se queda, el avatar sale porque el perfil vive al pie del cajón). Se retiró la barra: el celular usa ahora EL MISMO lateral de escritorio como cajón que entra desde la izquierda con un ☰ junto al logo, siempre expandido, con el perfil abajo («+ Nuevo» ya no existe, ADR-0204); se cierra con la ✕, el velo, Escape o al tocar un destino. Las tres barras fijas que se subían 4.25 rem para esquivar las pestañas (`BarraFija`, el «Ver ticket» de Vender y la ficha de Nuevo producto) bajan al fondo. Probado a 375 px con el Supabase falso (abrir, cerrar por velo y por Escape, foco de vuelta al ☰, scroll bloqueado) y a 1280 px (lateral igual que antes).
Felipe se lleva: (1) **una sola navegación para los dos tamaños**: un módulo nuevo en `lib/menu.ts` aparece en celular sin tocar nada más; (2) **quitar una barra fija obliga a revisar quién se apoyaba en su altura**: tres piezas tenían el 4.25 rem escrito a mano; (3) **el siguiente paso es Caja en celular**, ya con el menú resuelto.
...
## 2026-09-25 (Navegación en celular: «Más», avatar y lupa construidos — ADR-0205)
Felipe aprobó el spike (dos correcciones en vivo: sin «＋» en ningún lado, «Más» al final; sacar «Colaboradores»/«Roles y accesos» de «Más» porque duplicaban con el avatar) y pidió implementar. `AppShell.tsx`: barra del celular a 5 columnas (Inicio/Punto de Venta/Inventario/Caja + **Más**), cabecera con lupa (`/buscar`) y avatar (reutiliza `setPerfilAbierto`, ya existía). `components/MasMovil.tsx` (nuevo): una hoja `<Modal>` que pinta `menu.riel` tal cual —el mismo dato del lateral, sin árbol paralelo—, por decisión explícita de no recortar lo que ya está en la barra (ver ADR-0205, D2). `typecheck`, `lint`, `build` y la suite completa (126 archivos, 21 290 pruebas) en verde.
Felipe se lleva: (1) **construido en el mismo checkout que ADR-0204 (retiro de «＋Nuevo») y ADR-0209 (combos), ambas sesiones ajenas y sin commitear** — `git status`/`git diff` antes de tocar `AppShell.tsx`/`lib/menu.ts` evitó pisar ese trabajo, y de paso confirmó que «Más» debía sumarse a las 4 columnas que ADR-0204 ya dejaba, no inventar una reubicación del «＋» que nadie pidió; (2) **reusar el dato en vez de recortarlo evita una fuente de verdad nueva**: la hoja no excluye Inicio/Punto de Venta/Existencias/Caja aunque se repitan con la barra — un árbol "todo menos lo de la barra" habría sido una segunda cosa que mantener sincronizada con `lib/menu.ts`; (3) **sin verificar con clics reales**: no hay ruta pública de prueba en este checkout y no corresponde iniciar sesión desde acá — queda para Felipe, logueado, en 375px.

## 2026-09-25 (Navegación en celular: spike visual de «Más» + avatar + lupa)
Felipe reportó que el sistema no está adaptado para celular porque no aparecen todas las opciones. Se leyó el código antes de diseñar (`COLUMNAS_MOVIL`, ADR-0130 D8, `InventarioLayout.tsx`, el `<aside hidden … sm:flex>` de `AppShell.tsx`) y se armó `docs/maquetas/menu-movil-spike-2026-09/`, un HTML autocontenido que cuenta el problema en vez de solo describirlo: para un líder de tienda, 18 de ~26 pantallas no tienen hoy ninguna puerta en celular —incluida Traslados, la única con insignia— y no hay forma de cerrar sesión desde un teléfono. Propuesta: «Más» al final de la barra (árbol completo del lateral en una hoja) más lupa y avatar en la cabecera; en revisión con Felipe, sin construir.
Felipe se lleva: (1) **`git status`/`git diff` antes de dar una decisión por cerrada** — a mitad de la sesión apareció otra sesión retirando «＋Nuevo» entero (no reubicándolo) en `AppShell.tsx`/`lib/menu.ts` sin commitear todavía (cita a «ADR-0204», aún sin escribir); el primer diseño del spike (mover el «＋» a la cabecera) habría inventado una reubicación que nadie pidió — corregido a tiempo por leer el diff real; (2) **una cifra vale más que «no aparecen todas las opciones»**: 18 pantallas, contadas por el mismo árbol y las mismas reglas de `lib/menu.ts`, no una impresión.

## 2026-09-25 (Finanzas en producción, y las cuentas se corrigen y se eliminan — ADR-0195)
Felipe pegó las 37 ejecuciones de F3–F10. La 22 falló porque la 20 no había quedado: su «lock timeout» pasó sin verse. Se repitió la 20 y se validó todo preguntándole a la base: los objetos, el texto de cada función contra el repo, la seguridad y 42 lecturas corridas como líder sobre los datos reales. Después pidió poder editar y eliminar las cuentas: nombre y número siempre; tipo mientras nadie la usó; saldo inicial sin conciliación vigente y con su mes abierto. Una cuenta sin uso se borra de verdad (lo garantizan las llaves foráneas) y una usada se archiva.
Felipe se lleva: (1) **una ejecución que no se aplicó no avisa sola**: por eso se valida contra la base y no contra la lista de lo pegado; (2) **borrar es deshacer un error solo si nada depende de él**: la base lo sabe mejor que una lista escrita a mano; (3) **el tipo de una cuenta es parte de su historia**: cada cobro sellado se validó contra él, por eso no cambia después.

## 2026-09-25 (Análisis vuelve a abrirse al rol con el módulo — regresión de #397)
Desde que se fusionó #397, el CI marcaba 69/70 en `pruebas:roles`: un rol con Análisis que no es líder veía vacía la pantalla de su propia sede. #397 recreó `fn_resumen_comparacion` copiando el archivo de 2026-09-19, que todavía decía «solo líder»; el permiso de Análisis (ADR-0161 P5) se había puesto en vivo, así que no estaba escrito en ningún archivo que se pudiera copiar. Ningún ADR de #397 lo pedía. La migración `20260925223000` cambia solo ese filtro sobre la definición viva (idéntica en local y producción). En local, 70/70 y las otras tres suites en verde. Felipe la pegó el mismo día y se verificó en la base; en paralelo, #404 corrigió el filtro en los archivos de #397.
Felipe se lleva: (1) **cambiar una función en vivo deja un archivo viejo que miente**, y quien lo copie trae de vuelta lo que se había corregido; por eso el arreglo reemplaza una línea y no copia el cuerpo; (2) **una prueba en rojo que «no bloquea» igual avisaba bien**: el aviso se leyó como un problema del seed y el merge pasó; (3) **una pantalla vacía también puede ser un candado**: 0 filas no es un error, y por eso nadie lo vio.

## 2026-09-25 (El menú lateral, «lo de todos los días primero»)
...
En tienda, el orden nuevo es el mismo que ya usaban los grupos de Roles y accesos (`MODULOS` en `modulos.ts`). Así el líder encuentra cada módulo en el mismo puesto en las dos pantallas. En el Taller, «mostrador primero» dejaba Producción al final, y ahí es lo que se usa todo el día. Felipe pidió subirla: va pegada a Inicio. No hizo falta una regla por ubicación: basta ponerla segunda en el árbol, porque solo existe en el Taller y en una tienda su puesto no aparece.
Felipe se lleva: (1) **el orden de un menú es una decisión, no un accidente**: vive escrito en un solo lugar (`ARBOL`) con su porqué; (2) **dos pantallas que listan lo mismo lo listan igual**, o el líder tiene que aprender dos mapas, y el único desvío (Producción en el Taller) queda escrito; (3) **cuando un caso especial desaparece cambiando el orden de los datos, no hace falta un `if`**.

## 2026-09-25 (Las fotos de perfil salen de Dynamic)
Felipe vio su foto rota en «Mi perfil» y pidió «jalar todas las fotos del Dynamic». La causa: Dynamic guarda la RUTA dentro del bucket (`perfil/<persona>/<archivo>.jpg`) y retail la usaba como si fuera una dirección web; además, «Cambiar foto» subía el archivo y después Dynamic rechazaba guardarlo, porque retail le mandaba la URL completa. Ahora una sola función (`fn_fotos_personas`) da la ruta de cada colaborador, la web arma la dirección, y un solo avatar (`AvatarPersona`) pone la cara en el lateral, «Mi perfil», el combo «Responsable» y Colaboradores; si una foto no carga, salen las iniciales. Se probó con la foto real de producción (HTTP 200) y en el navegador (foto, sin foto y ruta rota). 89.197 pruebas en verde. La migración se aplicó en producción el mismo día (con OK de Felipe) y se verificó en la base; el combo se probó a 375 px.
Felipe se lleva: (1) **«jalar» no es copiar**: las fotos ya viven en el mismo proyecto, y una copia en retail quedaría vieja la primera vez que alguien cambie la suya en Dynamic; (2) **dos sistemas que escriben el mismo dato tienen que escribirlo igual**: retail guardaba una URL donde Dynamic guarda una ruta, y el candado de Dynamic lo rechazaba; (3) **una imagen que no carga no debe verse rota**: se cae a las iniciales, igual que quien no tiene foto.
...
## 2026-09-24 (Nuevo producto en 4 pasos, y por fin con fotos — ADR-0197)
El alta pasó de 7 tarjetas a 4 pasos en acordeón. Proveedor, marca y color se eligen con un buscador cuya lista flota sobre la pantalla (antes, los ~60 proveedores y los 40 colores salían como botones). Las variantes son una tabla talla × color, y a la derecha se ve la prenda tal como va a quedar. Las fotos no se podían subir porque cancelar dejaba archivos huérfanos: ahora quedan en el navegador y se suben recién cuando la base creó el producto. No hay migración. Probado en local con la pantalla real y en celular; el caso de falla de las fotos funciona (el almacén local no existe). 45.911 pruebas en verde.
Felipe se lleva: (1) **un archivo no entra en una transacción**: por eso se sube después, cuando lo que importa ya está guardado; (2) **una lista de 60 botones no es una elección, es una búsqueda**, y se resuelve con un buscador; (3) **una animación puede mover una lista flotante lejos de su campo**: el `transform` que deja al terminar cambia contra qué se mide la lista.

## 2026-09-24 (Finanzas: las pantallas se ajustan al spike — ADR-0195)
Felipe vio F1 y F2 andando y dijo que no se parecían a lo aprobado. Se capturó cada pantalla al mismo ancho que el spike y se comparó imagen contra imagen: la estructura era distinta (el «Ver» lejos del botón, los filtros fuera de la tabla, textos cortados, campos subrayados en vez de en caja, Configuración sin pestañas, un «Guardar» por fila). Se rehicieron Gastos, sus modales, Configuración y la meta de Caja con piezas propias de Finanzas (`components/finanzas/kit.tsx`, `app/estilos/finanzas.css`); cada casilla de Configuración se guarda sola al salir, y la lista de gastos ya dice cómo se pagó una factura al contado.
Felipe se lleva: (1) **«se parece» se mide con dos capturas al mismo ancho, no de memoria**: por eso ahora es regla antes de dar una pantalla por terminada; (2) **un spike también dice qué NO hacer**: sin botones «Guardar», sin pestañas vacías, sin céntimos en las cifras grandes; (3) **lo que no se puede calcular no se inventa**: «al ritmo cierras en…» espera la hora de cierre de cada tienda.

## 2026-09-24 (Finanzas F2b: activos fijos y gastos fijos del mes — ADR-0195)
F2a quedó en producción: las siete ejecuciones, verificadas en la base, y la web desplegada. F2b suma los **activos fijos**: un mostrador ya no se resta entero del mes en que se compra, sino que se reparte en su vida útil, desde el mes siguiente y sin arrastrar redondeos (los 120 meses de un mostrador de S/ 2,000 suman exactamente S/ 2,000). Suma también los **gastos fijos del mes**: el alquiler o la luz dicen si ya se registraron, si vienen o si faltan, y el sistema propone como fijo lo que se repite dos de tres meses. Gasto y activo comparten el paso «comprobante + pago», y un egreso de caja respalda una sola cosa. Hay 46 casos de base nuevos, 53 de F2a y 45.914 pruebas web en verde, y el recorrido completo se probó con clics.
Felipe se lleva: (1) **comprar un mueble no es gastar**: el gasto es lo que se usa cada mes; (2) **un recordatorio no es un gasto**: el fijo solo cuenta cuando se registra con lo que llegó de verdad; (3) **una regla compartida no se copia**: gasto y activo pagan por la misma función, así no hay dos formas de pagar lo mismo.

## 2026-09-24 (Finanzas F2a: gastos, con o sin factura de proveedor — ADR-0195)
Gastos ya se registra: la luz a crédito entra como una factura más de Compras (`naturaleza = 'gasto'`) y aparece en Por pagar; el mototaxi del cajón crea su egreso de caja y el gasto que lo señala en una sola operación; y lo que la tienda sacó del cajón sin decir qué era se clasifica como gasto, depósito, retiro o ajuste. Una sola línea en la vista del reparto por tienda hizo que Por pagar, la visibilidad y el tope de pago por tienda funcionen para gastos sin tocar las funciones de pago. Compras no cambió: nueve lecturas dan la misma huella antes y después. Hay 53 casos de base y 45.902 pruebas web en verde.
Felipe se lleva: (1) **una factura es una factura**: la luz y la tela comparten cabecera, así no hay dos IGV ni la misma factura dos veces; (2) **un egreso de caja dice cómo salió la plata, no en qué se gastó**: por eso se clasifica aparte y nunca se cuenta dos veces; (3) **cambiar una vista puede ser más seguro que cambiar diez funciones**, si se prueba que todo lo demás sigue dando lo mismo.
...
## 2026-09-24 (Finanzas F1: Configuración, meta y fondo de caja unidos a las campañas — ADR-0195)
Felipe aprobó el paquete y pidió construir todo el plan; se empezó por F1. Configuración (desde el perfil del líder, como Colaboradores) guarda la meta de cada día y el fondo de cada tienda, y cuánto cambia cada campaña; `fn_parametros_caja` es la única regla y la leen Caja e Inicio. El cierre dice «Deja S/ X», propone el traslado y pide confirmar sin bloquear; un disparador anota el fondo que regía sin tocar `cerrar_caja`. 20 casos de base, 11 de reglas y 24.375 pruebas web en verde; probado con clics y con sesión de líder en local.
Felipe se lleva: (1) **un disparador puede agregar un dato sin tocar una función delicada**: `cerrar_caja` sigue igual y el fondo queda anotado; (2) **la pantalla vieja sigue funcionando si la base va primero**, porque todo lo nuevo se lee tolerando que falte; (3) **una regla, un lugar**: Caja, Inicio y Configuración preguntan lo mismo a la misma función.

## 2026-09-24 (Finanzas: las temporadas son las campañas — ADR-0195 act. c, K corregida)
Felipe pidió unir las temporadas a las campañas. Revisado en producción: las campañas son 12 etiquetas de estilo «campaña» con fechas, descuento, categorías y tiendas, y se cruzan entre sí (el Día del Gato cae dentro de Fiestas Patrias). Se quitaron las temporadas: cada campaña dice por tienda cuánto sube la meta y qué fondo dejar, y si se cruzan gana la mayor. Nació Reportes ▸ Campañas: el margen extra de cada una y, para las que vienen, cuánto más hay que vender para compensar el descuento.
Felipe se lleva: (1) **una sola lista de fechas**: dos listas se desalinean el primer año; (2) **un descuento cuesta más de lo que parece**: con 15 % hay que vender 40 % más solo para quedar igual; (3) **si dos campañas se cruzan, gana la mayor**, la misma regla que el descuento de una prenda.

## 2026-09-24 (Finanzas: la plata conectada con todo el ERP, meta y fondo por temporada — ADR-0195 act. c)
Felipe pidió meta del día y fondo de caja por temporada, y que las cuentas aparezcan en todo lugar donde se mueve plata. Se midió en producción: 12 lugares con plata, 5 listas distintas de medios, ninguno guarda la cuenta, y 12 pagos en efectivo a proveedores (S/ 15,661) que el cierre de caja no cuenta. El plan tiene ahora las 22 situaciones con su cuenta y el spike muestra Tiendas y caja, la Caja con la meta y el cierre con una confirmación que no bloquea. El ADR se renumeró de 0194 a 0195 porque `main` ya tenía el 0194.
Felipe se lleva: (1) **medio y cuenta son dos cosas**: Yape es cómo pagaron, BCP es dónde está la plata; (2) **la cuenta se sella al guardar**, igual que el costo de una venta: cambiar la configuración no reescribe el pasado; (3) **una sola meta, no dos**: la del mes es la suma de las del día.

## 2026-09-24 (Finanzas: spike v2, la capa para decidir — ADR-0195 act. b)
Felipe preguntó si el spike era el mejor diseño. No lo era: mostraba números pero no decía qué hacer. Tras compararlo con Xero, QuickBooks, Float, Shopify y Odoo se sumaron salud en frases, punto de equilibrio por tienda, presupuesto contra real, escenarios, avisos de lo raro, gastos fijos que se proponen solos y conciliación con parejas; además el filtro «Ver» (la cabecera sigue siendo dónde trabajas), aporte o préstamo del dueño y el módulo general Gestión ▸ Configuración. 20 pasos probados con Chrome sin ventana, sin errores y sin desborde a 375 px.
Felipe se lleva: (1) **dónde trabajo y qué miro son dos cosas**: mezclarlas rompería el combo Responsable; (2) **el punto de equilibrio convierte «LIM pierde» en «LIM necesita vender S/ 24,763»**, que es algo sobre lo que se puede actuar; (3) **«inteligente» aquí son reglas que muestran su dato**, no una IA que inventa números.

## 2026-09-24 (Finanzas: spike visual completo — F0)
Las 11 piezas del plan quedaron navegables en `docs/maquetas/finanzas-2026-09/finanzas-spike.html`, en 6 entradas de menú (el tope de `lib/menu.ts`), con la paleta oficial, ventanas, loader y combo Responsable. Se puede ver como líder o como la encargada de TRU (solo su tienda) y mostrar de dónde sale cada número. El guion completo (gasto a crédito → Por pagar → clasificar el depósito → conciliar → cerrar agosto por unidad y consolidado) se probó con Chrome sin ventana: sin errores y sin desborde a 375 px.
Felipe se lleva: (1) **una pantalla que junta datos no debe pedir que se tipeen**: el Resumen no tiene ningún campo; (2) **el Balance que no cuadra no se dibuja**: mejor ningún número que uno falso; (3) **Yape y Plin no son cuentas**: son caminos que llegan a una cuenta del banco, por eso Vender no cambia.

## 2026-09-24 (Finanzas: desglose y plan, sin código — ADR-0195)
Felipe pidió el spike de Finanzas pero primero el desglose. Se auditó el repo y producción (S/ 943 mil cobrados, 52 % fuera del efectivo y sin ninguna cuenta bancaria en el sistema; `gastos` y `activos_fijos` con 0 filas) y apareció el PR #170 con ADR-0109/0117/0120 ya aprobados y sin fusionar. Quedó `docs/PLAN-FINANZAS.md` con 11 piezas, 5 módulos y fases F0–F10, y el ADR-0195 (nació 0194) con las decisiones A (un solo comprobante de proveedor para mercadería, gasto y activo) y B (cinco módulos, con cada tienda viendo solo lo suyo).
Felipe se lleva: (1) **finanzas y contabilidad no son lo mismo, pero viven juntas**: la gestión arriba para decidir, la contabilidad abajo, automática; (2) **gasto operativo se consume en el mes, activo fijo dura años**: mezclarlos hace mentir a los dos meses; (3) **antes de diseñar, buscar si ya se diseñó**: el PR #170 tenía la mitad del trabajo aprobado.
...
Etapas 2–5 en paralelo con 5 agentes, cada una con su PR (#376 cambios/devoluciones/conteo, #380 orden de candados y doble clic, #378 totales de Caja e Historial en la base, #379 Vender sin recargar y topes de 1.000, #377 «otra persona cambió esta prenda»). Integradas sobre `main` sin conflictos, 24.363 pruebas web en verde y las 5 migraciones corriendo en fila en local. Por pegar en orden 110000→120000→130000→140000→160000 antes de fusionar la web.
...
## 2026-09-24 (Prueba de carga: 15 a 50 personas a la vez sobre un año de datos — ADR-0194)
Felipe pidió probar todo el ERP con muchas personas a la vez (meta 15–50), en local y validando antes que el local fuera igual a producción. No lo era: 16 migraciones atrás y 46 funciones distintas; se midió en una base aparte (`cayla_carga`) con las funciones de producción y un año sintético (131.000 ventas, 24.000 variantes), con pgbench y colaboradoras reales. El ERP se saturaba antes de 30 personas por dos lecturas («ventas de hoy» sin índice, Existencias calculando todo el catálogo y cayendo en el plan genérico de PL/pgSQL desde la 6ª llamada); arreglado: de 23 a 110–141 operaciones/s y de 209 a 0 tiempos agotados con 50. También se reprodujo y cerró un bloqueo mutuo cambio × venta. Las pruebas existentes: 24.363 web en verde; 1.167/1.237 SQL, y ninguno de los 70 rojos es un bug de producción (44 deriva local, 25 entorno, 1 prueba vieja).
Felipe se lleva: (1) «lento con muchas personas» casi nunca es la venta: es una lectura pesada que acapara la base y hace esperar a todas, también a la caja; (2) un volumen chico esconde todo: producción hoy (3 productos) no dice nada del año que viene; por eso se mide con un año inventado; (3) aplicar migraciones viejas DESPUÉS de las nuevas pisa los parches: el orden importa tanto como el contenido.
Sin resolver: tres decisiones de diseño/dinero (catálogo y stock de la red en Vender, Resumen de inventario, tamaño del cómputo), el OK para alinear el local compartido, y por qué producción está casi vacía (`delete from ventas …` de 35.000 filas sin documentar).

## 2026-09-24 (Verificado: local sigue igual a producción — rama `claude/responsive-local-setup-c83b9b`)
Felipe pidió una rama para GitHub y que este chat corriera en local viéndose igual a producción. La rama ya existía (la creó el propio panel al abrir esta conversación, parada exactamente en el mismo commit que `main`); se empujó tal cual, sin crear una segunda. Verificado en el Postgres local compartido (el mismo contenedor que usan los otros 4 worktrees activos hoy): el espejo de datos reales de esta mañana seguía intacto — 3 productos, 12 variantes, 18 filas de stock, 26 movimientos, 0 ventas, 26 personas, 6 sedes, igual a lo ya documentado — y las migraciones locales están exactamente a la par de producción (falta solo `20260924180000`, que tampoco está pegada en producción todavía). `apps/web/.env.local` sigue apuntando al Supabase local, nunca al remoto.
Felipe se lleva: (1) el Postgres local no se resetea solo entre sesiones ni entre worktrees — por eso el espejo de esta mañana seguía ahí sin que nadie lo repitiera; (2) "local igual a producción" hoy significa un catálogo casi vacío, porque producción también lo está (la fila abierta sobre las 35.000 ventas borradas sigue sin explicación).
Sin resolver: quién es el dueño de repetir este espejo cuando la base real cambie de volumen, y las mismas decisiones abiertas de la entrada anterior.
## 2026-09-24 (Apartados sale del Punto de venta: módulo propio — ADR-0196)
Felipe vio que en Roles y accesos encender «Punto de venta» abría dos pantallas del menú, Punto de Venta y Apartados. Venía del ADR-0166, que los juntó por ser la misma caja. Felipe decidió separarlos: `apartados` es un módulo nuevo (orden 15, delegable, nace sin rol) y `/vender/apartados` exige ese módulo.
Felipe se lleva: (1) un interruptor en Roles y accesos debe abrir exactamente lo que dice su tarjeta; (2) un módulo nuevo nace solo para el líder, así que separar uno le quita la pantalla a quien la tenía hasta que se le vuelva a dar.
Sin resolver: pegar la migración antes de publicar, encender Apartados en los roles que lo usaban, y el candado por módulo en las funciones de Ventas.

## 2026-09-24 (Comportamiento comercial piso vs. almacén en Análisis — ADR-0199, LOCAL, nada en producción)
Pedido de Felipe: separar en «Comportamiento del inventario» qué tan bien vende una variante donde la clienta realmente la puede comprar (piso) de cuánto se mueve todo lo que la tienda posee (piso+almacén). Migración `20260924010700` extiende `fn_resumen_comparacion` (piso/total promedio ponderados por tiempo, última venta, eventos de piso); `lib/resumen-exposicion.ts` (nuevo) interpreta esos eventos como cohortes FIFO para el sell-through DE EXPOSICIÓN, evitando que una reposición reciente diluya el % o que un traslado piso→almacén→piso se cuente como venta. La tabla suma Rotación piso, Rotación total y «Sin venta» por exposición real; `resumen-lectura.ts` crece de 7 a 10 reglas (Reposición reciente, Agotamiento y Estancamiento reforzados, Problema de reposición, Sobrestock). 24.256 pruebas en verde; verificado con clics reales contra Supabase local de Tienda Lima — una reposición del mismo día se leyó, en vivo, «Reposición reciente: sin exposición suficiente todavía».
Felipe se lleva: (1) **una cohorte que sale del piso sin venderse dejó de existir para su propio sell-through**: por eso un traslado de ida y vuelta nunca se cuenta dos veces, sin tener que rastrear la unidad física; (2) **«sin ventas» y «rota lento» necesitaban la misma corrección**: los dos medían con calendario en vez de exposición real, y los dos son el mismo error con nombre distinto; (3) **el KPI y el filtro de arriba de la pantalla se quedaron con la fórmula vieja a propósito**: extenderlos también es la misma decisión, del mismo tamaño, y quedó pendiente en vez de tomarla por mi cuenta.
Sin resolver: si Felipe aprueba pegar la migración en producción; si unifica el sell-through del KPI/dona/filtro con el de exposición; extender las mismas reglas a Comparar períodos.

## 2026-09-24 (Dominio de Inventario reconstruido sobre el modelo canónico de Felipe — ADR-0200, sucede a ADR-0199, LOCAL, nada en producción)
Felipe envió la definición completa del dominio comercial y pidió auditar lo de esta mañana (ADR-0199) contra ella sin darle prioridad por existir. Se corrió una auditoría de 8 hallazgos independientes ANTES de tocar código: confirmó que el reloj de exposición se reiniciaba al volver a piso (debía pausar/reanudar) y que «Rotación piso»/«Rotación total» estaban en soles (COGS) cuando debían ser en unidades — y encontró, de paso, un bug real YA EN PRODUCCIÓN (no de hoy): el bucket «total» del ledger zeroaba movimientos sin `sububicacion_id` resuelta, inflando en +711 unidades el stock de AQP/TRU desde la activación piso/almacén del 2026-09-14, afectando también columnas de ADR-0138 ya en producción. Se reconstruyó `lib/inventario-exposicion.ts` (dominio reutilizable, prefijo correcto por convención real del repo): cohortes con pausa/reanudación (señal `esMovimientoInterno` desde `motivo='movimiento_interno'`, la única real piso↔almacén), `rotacionUnidades` separada y sin tocar `rotacion.ts`, y las 7 categorías de Lectura con el texto EXACTO de Felipe (A–G). SQL corregido: bucket total, promedio de piso no diluido, «es venta» unificada en una función (antes duplicada) e incluye liquidación de prenda dañada como venta (ya ordenado el 17-09, seguía sin cumplirse). 24.267 pruebas en verde; verificado con clics reales contra Tienda Lima.
Felipe se lleva: (1) **una auditoría propia antes de defender el trabajo de la misma mañana evitó construir cinco pantallas más sobre una base equivocada**: el pedido explícito de "no le des prioridad por existir" encontró contradicciones reales, no imaginarias; (2) **un bug de producción puede estar escondido dentro de un bug nuevo**: el defecto del bucket total es de ADR-0138 (ya vivo), la sesión de hoy solo lo heredó sin corregirlo hasta que la auditoría lo señaló; (3) **"pausar" y "reiniciar" un reloj son API casi idénticas y resultados opuestos**: la única diferencia real entre las dos versiones de `armarCohortes` es si una cohorte que vuelve del almacén recuerda su historia o no.
Sin resolver: si Felipe corrige el bucket-total en producción; si aplica el resto de la migración; unificar KPI/dona/filtro de sell-through; extender las 7 categorías a Comparar períodos; una primitiva compartida de ledger para el resto de Inventario.

## 2026-09-24 (Seis decisiones de negocio sobre el dominio de Inventario — ADR-0201, sucede a ADR-0200, LOCAL, nada en producción)
Tras el informe de verificación de ADR-0200 (con ejemplos numéricos, no solo referencias al ADR), Felipe respondió con 6 decisiones canónicas, ninguna asumida — todas eran preguntas explícitas o huecos declarados en ADR-0200. (1+2) La liquidación de prenda dañada queda fuera de la demanda comercial (Vendido/Ritmo/Rotación/Tendencia) y deja de resetear el reloj de «Sin venta», sin dejar de ser una venta contable real: `demanda_base.clase` gana una categoría explícita `liquidacion_danada` (nunca un `null` silencioso) que no entra a ninguno de los dos `clase` que se suman. (3) «Rotación» (COGS/soles) se renombra a «Rotación valorizada» en las 5 pantallas donde aparecía, con el texto exacto pedido para su fórmula; «Rotación piso»/«Rotación total» (unidades) no cambian, ya eran otra cosa. (4) Se investigó `aprobar_devolucion` y se confirmó que una devolución vendible YA entra directo a piso con `motivo='devolucion'` (nunca por almacén ni como `movimiento_interno`) — el reloj de exposición ya arrancaba en cero para una devolución real, sin necesitar cambio de código; se agregó la prueba que lo fija. (5) Contrato común de calidad del dato, módulo nuevo `inventario-calidad.ts`: exacto/estimado/no_disponible + 6 motivos, como proyección sobre los tipos existentes (sin reescribirlos), conectado ya en las celdas de Rotación piso/total y Sell-through de la tabla. (6) Primitiva compartida de reconstrucción de ledger, función SQL nueva `retail.fn_ledger_timeline` (variante+sede+ventana → intervalos de nivel de piso y total), sin tocar `fn_resumen_comparacion` ni rediseñar ninguna otra pantalla — verificada a mano contra movimientos reales de Tienda TRU, coincidencia exacta. 24.280 pruebas en verde, typecheck y lint limpios.
Felipe se lleva: (1) **una salida sin demanda comercial nunca es un `null` silencioso**: darle su propia categoría explícita (`liquidacion_danada`) resolvió DOS decisiones de negocio con un solo cambio de SQL, porque los dos consumidores (la suma de demanda y el reloj de «Sin venta») ya filtraban por `clase='venta'` exacto; (2) **cuando la pregunta ya tiene la respuesta en el código, la decisión correcta es probarlo, no programarlo**: la devolución-a-piso ya funcionaba bien desde ADR-0200 porque usa una señal distinta (`esMovimientoInterno`) a la del movimiento interno — agregar código nuevo ahí habría sido una solución para un problema que no existía; (3) **una primitiva de dominio puede "dejarse preparada" sin que nadie la use todavía**: `fn_ledger_timeline` no tiene consumidor a propósito — existir y estar correcta ya cumple el pedido, migrar Existencias/Movimientos es una decisión aparte y del mismo tamaño que las de hoy.
Sin resolver: si Felipe corrige el bucket-total en producción (+711 unidades fantasma en AQP/TRU); si aplica las dos migraciones de hoy y las de ADR-0200 juntas; unificar KPI/dona/filtro de sell-through; extender las 7 categorías a Comparar períodos; qué pantalla adopta primero `fn_ledger_timeline`; verificar en vivo en el navegador el renombre «Rotación valorizada» — la sesión local se cerró a mitad de esta ronda (parece que hubo un reseteo de la base local), pendiente que Felipe vuelva a iniciar sesión.

## 2026-09-24 (Fuente única del ledger + contrato de calidad completado en el dominio — ADR-0202, sucede a ADR-0201, LOCAL, nada en producción)
Felipe aprobó conceptualmente las 6 decisiones de ADR-0201 sin cambios, pero marcó dos huecos arquitectónicos antes de dar el dominio por cerrado: (1) `fn_resumen_comparacion` y `fn_ledger_timeline` seguían siendo DOS reconstrucciones independientes del mismo ledger, aunque dieran el mismo número hoy; (2) el contrato de calidad existía pero solo se llamaba desde 2 de las 7 celdas de la tabla, nunca desde el dominio. Función SQL nueva `retail.fn_ledger_puntos` (migración `20260924030000`): hace TODO el trabajo de reconstrucción —saldo inicial, clasificación por bucket, nivel acumulado— para una sede y, opcionalmente, un arreglo de variantes; con `null` reconstruye TODAS las variantes de la sede en UN solo recorrido (nunca uno por variante: Felipe fue explícito sobre no introducir N+1), con un arreglo explícito esa variante siempre aparece aunque no tenga movimiento. `fn_resumen_comparacion` y `fn_ledger_timeline` ahora la llaman en vez de reconstruir cada una por su cuenta. `CalidadDesempeno` (7 claves: exposición, ritmo, sell-through, rotación piso, rotación total, sin venta, tendencia) se calcula una sola vez en `analizarDesempeno()` —agregando el adaptador que faltaba, `calidadDeExposicion`— y `ResumenComportamiento.tsx` ya lee de ahí en vez de invocar los adaptadores dentro del render. Verificación nueva, `scripts/pruebas/fn_ledger_fuente_unica.mjs` (32/32 en verde): prueba ESTRUCTURAL (el código fuente de las dos funciones contiene la llamada real a `fn_ledger_puntos`, no es coincidencia numérica), equivalencia cruzada (un ciclo piso→almacén→piso reconstruido a mano desde `fn_ledger_timeline` coincide exacto con lo que reporta `fn_resumen_comparacion`) y los 4 escenarios de regresión de la prueba anterior. `explain analyze`: 10 ms, un solo Function Scan — sin señal de N+1 a la escala real de CAYLA. 24.286 pruebas de TypeScript en verde, typecheck y lint limpios.
Felipe se lleva: (1) **"la misma primitiva, sin N+1" tiene una solución de un solo arreglo opcional**: `fn_ledger_puntos(sede, desde, variantes?)` — con `null` es el recorrido de sede de siempre, con un arreglo es la garantía de respuesta de `fn_ledger_timeline`; una sola función sirve a los dos consumidores sin que ninguno pague el precio del otro; (2) **no toda duplicación es el riesgo que se pidió eliminar**: `fn_resumen_comparacion` sigue leyendo el stock de hoy por su cuenta (`actual`, 10 líneas) porque no es el ALGORITMO de reconstrucción —es una lectura directa de una tabla— y forzar su eliminación habría cambiado cómo se calculan columnas que ya llevan meses en producción, sin necesidad real; (3) **una prueba que demuestra delegación real, no coincidencia numérica, se escribe leyendo el código fuente de la función** (`pg_get_functiondef` + `like '%fn_ledger_puntos(%'`) — mucho más fuerte que solo comparar resultados, que también pasaría si las dos funciones coincidieran por casualidad.
Sin resolver: las mismas 5 decisiones pendientes de ADR-0201 (bucket-total en producción, aplicar las 3 migraciones de hoy, unificar sell-through del KPI, extender las 7 categorías a Comparar, qué pantalla adopta primero la primitiva); verificar en vivo en el navegador — la sesión local sigue cerrada (`/login?error=sin_persona`, confirmado de nuevo hoy). Hallazgo aparte, con chip de sesión enviado: el seed local cambió (nombres de sede, UUIDs de personas, estado de sedes) y rompe silenciosamente docenas de `scripts/pruebas/*.mjs` preexistentes — no es una regresión de hoy, pero esconde una si nadie lo repara.

## 2026-09-24 (Movimientos auditado, taxonomía Nivel A/B y cierre del dominio de Inventario — ADR-0203, sucede a ADR-0202, LOCAL, nada en producción)
Antes de cerrar el dominio, Felipe pidió corregir la prueba nueva (no hardcodear UUIDs del seed — se resuelven por rol/relación: cualquier tienda activa, cualquier admin, cualquier colaboradora de otra sede) y auditar Inventario > Movimientos, que ya existe y consume la misma tabla `retail.movimientos` — "no asumas que su UI es la fuente de verdad". La auditoría (dos agentes de exploración, solo lectura) encontró que Movimientos ya clasifica "interno" (piso↔almacén) con una condición ESTRUCTURAL (`tipo='traslado' and ubicacion_id=ubicacion_destino_id`), mientras el ledger nuevo comparaba el string `motivo='movimiento_interno'` — un bug latente real: ya existe una fila en PRODUCCIÓN (`activacion-piso-almacen-produccion.sql`, 2026-09-14) donde esas dos condiciones divergen, y el propio código (`movimientos-reglas.ts:133`) ya tenía que mantener una lista de motivos por esa misma razón. Corregido con `retail.fn_es_traslado_interno` (Nivel A del dominio: hecho estructural, hermana de `fn_es_venta_de_stock`, Nivel B: interpretación comercial) — editado directamente en la migración de hoy en la mañana (`20260924030000`, nunca salió de esta sesión). Además: auditando estado por estado, `calidadDeExposicion` resultó inventar `estimado` sin una técnica sustituta real detrás (exactamente lo que Felipe pidió evitar) — corregida a 2 estados (exacto/no_disponible); `muestraLimitada` sigue como su propio booleano, fuera del contrato. Nueva prueba SQL: cruce numérico Ledger↔Movimientos (recepción + 2 ciclos piso↔almacén + venta + devolución + ajuste: las dos capas suman 14, ninguna omite ni inventa un movimiento) y una demostración numérica de que `actual` no es una segunda reconstrucción (ventana histórica: inicio=0, cierre=15, stock de hoy=45 — tres números distintos y correctos). `fn_ledger_fuente_unica.mjs` pasó de 32 a 44 verificaciones, todas en verde. 24.286 pruebas de TypeScript en verde, typecheck y lint limpios.
Felipe se lleva: (1) **dos condiciones que dan el mismo resultado hoy no son la misma garantía**: nada ataba `motivo='movimiento_interno'` a `ubicacion_id=ubicacion_destino_id` — ya habían divergido una vez en producción, y el código ya lo sabía a medias (esa lista de motivos era la señal de que algo no calzaba, escrita pero nunca seguida hasta la causa raíz); (2) **"estimado" tiene una prueba de admisión, no es un estado por defecto**: solo corresponde cuando el CÁLCULO usó una técnica sustituta (el denominador de Ritmo cae a calendario, el sell-through aproxima por cantidad) — "el número es el mismo de siempre pero la muestra es corta" es una advertencia de confianza, no una aproximación, y confundir las dos cosas es inventar un estado; (3) **cruzar dos fuentes independientes con datos reales sintéticos encuentra lo que ningún argumento encuentra**: el hueco de `es_interno` era invisible leyendo el código de una sola función — apareció al comparar la condición de Movimientos contra la del ledger, línea por línea, con una auditoría dedicada a esa única pregunta.
Sin resolver: las mismas decisiones pendientes de ADR-0201/0182; drill-down de Análisis a Movimientos (los datos ya lo permiten, falta `p_variante_id` en `fn_movimientos` y el enlace en la UI — no construido hoy, a pedido de Felipe); verificación visual — la sesión local sigue cerrada.

**Addendum del mismo día:** grep exhaustivo (apps/, packages/, scripts/, supabase/, e2e/snapshots/fixtures, wildcard/dinámico/barrel) confirmó que `etiquetaActividad` no tenía ningún consumidor real — eliminada la función (`movimientos-reglas.ts`) y su `describe` de prueba (`movimientos-reglas.test.ts`, 3 casos). `etiquetaMovimiento`/`etiquetaConDireccion` revisadas de paso: son puramente presentacionales (formatean `categoria`/`motivo`/`delta` ya decididos por SQL), no duplican ninguna clasificación de dominio. 24.283 pruebas en verde (−3, las del código eliminado), typecheck y lint limpios, 44/44 en `pruebas:fn-ledger-fuente-unica`.

## 2026-09-24 (Release consolidado: migraciones fusionadas, Stock actual P/A, sincronizado con `origin/main` — LOCAL, nada en producción)
Antes de preparar el release, Felipe pidió identificar con certeza el Supabase real de producción (limitado, por su propia corrección, al schema `retail` — Dynamic queda fuera de alcance de este trabajo). Confirmado en solo lectura, sin escribir nada: `fn_resumen_comparacion` en producción sigue en su versión ADR-0138 (36 columnas, el bug del bucket total todavía vivo tal cual), ninguna de las funciones de hoy existe ahí, y las 4 dependencias que necesitan las migraciones nuevas (`fn_puede_operar_ubicacion`, `fn_puede_analizar`, `fn_es_lider`, `fn_actor_persona_id`) sí. Con el target confirmado: se consolidó el set de migraciones (`20260924020000` absorbida por completo en `20260924030000` y eliminada — nunca se aplicó a producción, no hacía falta conservar dos `create function` del mismo `fn_ledger_timeline`), y se agregó **Stock actual P/A** a `fn_resumen_comparacion` y a la tabla de Comportamiento: reutiliza `actual.en_venta`/`actual.utilizable`, ya calculados ahí — ninguna fuente de stock nueva — y da `N/D` (nunca un 0 inventado) cuando la sede no separa piso de almacén, verificado con los 4 casos exactos que pidió Felipe (5/60, 0/35, 0/0, N/D). La rama (211 commits detrás de `origin/main`) se sincronizó con `git merge --ff-only` (sin conflictos posibles: era un ancestro puro) y un `git stash` para proteger el trabajo sin comprometerlo; al reaplicar el stash, solo 2 archivos tuvieron conflicto real —este mismo archivo y BACKLOG.md, ambos documentos que se acumulan— resueltos manteniendo las dos mitades; los 5 archivos de código que se creía en riesgo (`ResumenComparacionDetalle.tsx`, `ResumenComportamiento.tsx`, `movimientos-reglas.ts`, `package.json`, `globals.css`) fusionaron solos.
Felipe se lleva: (1) **"producción" puede significar dos cosas distintas en la misma frase, y hay que decir cuál**: el proyecto Supabase completo (con Dynamic) y el schema `retail` de ese proyecto NO son la misma pregunta — acotar la auditoría al segundo la volvió mucho más rápida y sin ningún riesgo de tocar lo que no es de CAYLA Retail; (2) **una migración nunca aplicada se puede consolidar sin miedo**: la regla no es "nunca reescribas una migración", es "nunca reescribas una que ya corrió en algún lado" — verificarlo primero (por efecto, no por nombre) es lo que hace segura la consolidación; (3) **sincronizar 211 commits de diferencia no garantiza conflictos**: cuando el trabajo nuevo toca columnas/funciones que nadie más tocó, el merge automático resuelve casi todo solo — los conflictos reales aparecen donde de verdad hay dos decisiones distintas sobre el mismo texto (los documentos que registran decisiones), no donde hay código.
Sin resolver: las mismas decisiones pendientes de producción (bucket-total, aplicar las 2 migraciones del release); verificación visual de Stock actual P/A — pendiente que Felipe la vea con clics.

## 2026-09-25 (Se retiró el botón "+ Nuevo" global — ADR-0204)
Felipe pasó una captura del botón "+ Nuevo" del lateral de escritorio pidiendo eliminarlo ("no tiene mucha funcionalidad"). No era un botón suelto: era el disparador de un menú accesible completo (`MenuNuevo`), compartido con el "+" central del celular, con 6 atajos reales a otros tantos módulos. Confirmado el alcance con Felipe (afecta más de un módulo a la vez): se retiró TODO — botón, menú, las 6 acciones y el campo `nuevo` de `Menu` — en escritorio y celular, sin reemplazo; la barra del celular pasa de 5 columnas (con el hueco del "+") a 4. Suite completa (126 archivos, 21.290 pruebas) y `tsc --noEmit` en verde; verificado en el navegador.
Felipe se lleva: (1) **un botón que parece decorativo puede ser el único camino a una pantalla**: el líder parado en el Taller se quedó sin ningún link a "Recibir mercadería" (antes le quedaba solo ahí) — la ruta sigue viva, decisión aparte si se quiere destrabarla; (2) había otra sesión con un spike de navegación de celular (`docs/maquetas/menu-movil-spike-2026-09/`, sin aplicar) que asumía que `MenuNuevo` seguía vivo, reubicado a la cabecera — consultado, Felipe confirmó eliminarlo igual; si ese spike avanza, el panel se reconstruye desde cero.
Sin resolver: la decisión sobre «Recibir mercadería» en el Taller; qué pasa con el spike de celular en paralelo.

## 2026-09-25 (CLAUDE.md y 15-COMO-OPERA corregidos contra producción viva)
Felipe pidió corregir los dos documentos que cada sesión lee primero. Se auditaron por tramos contra la base viva (consultas de solo lectura), el código y los ADR; cada hallazgo pasó por dos verificadores y el diff final por tres revisores. CLAUDE.md deja `sedes`/`personas`/`sede_id`, «45 tablas», Nubefact/Culqi/Shopify y la regla de migraciones sin prefijo, y su ritual ya no pide leer BACKLOG y BITÁCORA completos. 15-COMO conserva cada regla como se dijo el 2026-09-12 y suma notas «Actualización (fecha, fuente)».
Felipe se lleva: (1) **151 agentes y dos verificadores por hallazgo dejaron pasar dos falsedades gruesas** —«miles de ventas reales» (era la demo de 90 días) y «ningún rol fuera del líder tiene Compras» (la Terminal Almacén sí)—; solo las atrapó preguntarle a la base en vivo, no al volcado; (2) esta misma corrección ya se había hecho el 2026-09-23 y se perdió en ramas sin PR: el trabajo no está hecho hasta que llega a `main`.
Sin resolver: construir lo que Felipe decidió al cierre (R-38 sin líder dentro de 15 días, con su ADR; descuentos de más de 15% resaltados en Caja antes de cerrar; R-20 con 14 días hasta tener 8 semanas de ventas); Audaces; `AGENTS.md`, `config.toml` y el hook de graphify siguen contradiciendo a CLAUDE.md (ver BACKLOG).

## 2026-09-25 (La prueba contra Postgres deja de ser opcional y AGENTS.md apunta a CLAUDE.md)
Antes de quitarle el `continue-on-error` al job `pruebas-postgres` se midió su historia: de 79 corridas, 20 fallaron y 19 fueron fallas reales del código (una sola de infraestructura, el límite de Docker Hub). La prueba de roles salió en rojo dos veces en el PR #397 y aun así se fusionó; 7 PR más heredaron el rojo hasta que se arregló a mano. `AGENTS.md` pasa a ser un puntero de siete líneas hacia `CLAUDE.md`: la copia vieja seguía mandando escribir migraciones sin prefijo.
Felipe se lleva: (1) **una prueba que no bloquea es un aviso que nadie lee**: el rojo del #397 estuvo a la vista y se fusionó igual; (2) el ruleset `main-protegida` ya tenía la regla de checks obligatorios, pero con la lista vacía: el candado existía y no cerraba nada; (3) el intento del 2026-09-23 de bloquear solo las pruebas «núcleo» no habría atrapado esta regresión, porque estaba en la prueba de roles.
Hecho al cierre: con OK de Felipe, el ruleset `main-protegida` exige los dos checks (19 de los 20 PR abiertos quedan trabados hasta traer `main`, porque reportan el nombre viejo). Sin resolver: 20 de las 77 pruebas `pruebas:*` siguen fuera del CI, entre ellas `fn-aplicar-movimiento` (el libro de stock) y las dos de concurrencia de la venta.

## 2026-09-25 (Actividad de cada módulo — ADR-0207)
Felipe preguntó dónde quedan los movimientos de cada colaboradora: cada acción está firmada, pero en la tabla de lo que se hizo (unas 55, cada una con su columna), sin un lugar para leerlas. Se construyó `retail.actividad` (solo agregar), llenada por disparadores sobre las tablas que ya guardan —sin tocar ninguna de las 343 funciones—, con Punto de venta, anulaciones, Caja y Cambios como primer paso y lo pasado reconstruido de las firmas. Se abre desde un botón «Actividad» junto a la sede que muestra el historial del módulo donde uno está; el líder ve todas las sedes, quien tenga el módulo «Actividad» solo la suya, una terminal nunca. `pnpm pruebas:actividad` 15/15, verificado en el navegador (escritorio y 375 px).
Felipe se lleva: (1) **«cada módulo tiene su historial» no significa una tabla por módulo**: es una sola tabla filtrada por módulo, así lo que se arregla una vez queda arreglado para todos; (2) **el disparador vive en la tabla, no en la función**: una función nueva que guarde una venta queda anotada sin que nadie se acuerde; (3) **un error del historial nunca debe impedir cobrar**, así que cada anotación está envuelta y, si falla, deja aviso en el log y la venta sigue.
Pegada en producción el mismo día (ensayo abortado, luego aplicada: 41 líneas de 3 sedes). Sin resolver: publicar la web (PR #424); encender el módulo en el rol de la líder de tienda y probarlo con esa cuenta; sumar los demás módulos. Aparte: `pruebas:registrar-venta` (2) y `pruebas:caja-cierre-traslado` (3) ya fallaban en `main` sin este cambio (reglas recientes de descuento y de traslado): no son de esta rama.

Sin resolver: agregar los dos checks al ruleset (lo hace Felipe, con permiso de administrador: comando en `CONTRIBUTING.md` §2); 20 de las 77 pruebas `pruebas:*` siguen fuera del CI, entre ellas `fn-aplicar-movimiento` (el libro de stock) y las dos de concurrencia de la venta.

## 2026-09-24 (Frescura del piso: el «mapa de calor» se volvió tres relojes y una carrera — ADR-0208)
Águila partió de dos hallazgos del benchmark y ambos se aclararon. Primero: SQL (`fn_productos`) y `calcularVelocidad` sí calculan cada uno «cuánto se vende por día», pero responden preguntas distintas (Compras en toda la empresa frente a reposición por sede), y se quedan separados a propósito. Segundo: «el traslado ignora el almacén del destino» era un bug de V1 (`inteligencia.ts`, borrado el 12-sep), que V2 ya había cerrado; el documento del módulo seguía describiéndolo como vigente y se corrigió.
De ahí salió la idea de Felipe: en 5 tandas de preguntas, con dos investigaciones verificadas (16 agentes), se diseñó «Frescura del piso» (tres relojes, modelo+color por sede, percentiles de su categoría con Kaplan-Meier, clásicos aparte, rebaja por sede) y se armó un documento con tablero simulado para las encargadas, dirección, Benja y los programadores.
Felipe se lleva: (1) «rotación» eran tres cosas distintas: dinero (rotación), ritmo (velocidad) y antigüedad en el piso (frescura); cada una responde a una decisión distinta; (2) un promedio calculado solo con lo vendido engaña, porque deja fuera justo lo que se quiere detectar: lo que sigue colgado; (3) una foto del piso siempre se ve peor que el negocio: lo rápido desaparece y lo lento se acumula.
Sin resolver: construirlo en el orden del ADR (primero la bajada al piso por escaneo), las decisiones abiertas (tallas clave, clásicos, metas) y compartir el documento con el equipo.

## 2026-09-25 (Frescura del piso, paso 1: bajar al piso escaneando, todo o nada, y la marca de bajada tardía — ADR-0208)
Felipe pidió construir el paso 1 (hoy, bloque 1). Siete lectores (solo lectura) corrigieron el diseño antes del código: `bajar_a_piso` y `devolver_a_almacen` no existen (hoy es `mover_interno`, una prenda por vez), el «fardo» no tiene código, producción tiene unos 42 movimientos y 92 de las 146 unidades del piso entraron por un ajuste, no por bajada. Se construyó, en paralelo y con una integración al final: la pantalla `/inventario/bajar`, que escanea con la pistola y confirma una vez; `bajar_al_piso`, todo o nada con token y huella, encima de `mover_interno`; las tablas `bajadas_piso` y `bajada_piso_items`; el módulo «Bajada al piso» (nace solo para el líder); y `fn_bajadas_del_piso`, que calcula al leer qué bajadas fueron tardías. La revisión y la fusión con main cambiaron después la entrada, el ajuste «Reposición» y la lectura (entrada siguiente). **Nada pegado en producción.** El número del ADR cambió tres veces antes de quedar en 0208; la historia está en el ADR.
Por qué así: una sola función produce la fila «almacén → piso» (la misma de «Reponer»), así la lectura de tardías ve igual las dos; y la marca no se puede guardar, porque cuando se baja la venta todavía no existe. Qué se rompería sin esto: si la bajada se registra al cobrar, el reloj de piso ve «0 días» y Frescura pinta estrellas falsas; y sin token, un reintento después de una red caída bajaría dos veces.
Felipe se lleva: (1) **los documentos a mano describían funciones de V1 que ya no existen**, y el propio ADR las citó («la función existe»); sus números («24.000 movimientos») eran de la demo que se retiró el 24-sep. Antes de diseñar sobre un nombre o una cifra, se pregunta a las migraciones y a la base, no a un documento; `ARQUITECTURA.md`, `05-SEGURIDAD.md` y los módulos 05 y 13 quedaron con notas «(V1; hoy: …)»; (2) «por fardo» no necesita un código de fardo: es una sesión de escaneo. Precargar lo recibido registraría lo supuesto, no lo colgado, que es justo lo que Frescura quiere medir; (3) una marca que depende de algo futuro (la venta) no se guarda: se calcula al leer, y si el stock y el libro no cuadran la fila dice «dudosa» en vez de acusar con un número inventado.
Sin resolver: lo que queda abierto está en la entrada siguiente.

## 2026-09-25 (Frescura del piso, bloque 1 corregido: tres revisores y la fusión con main cambiaron la entrada, «Reposición» y la lectura — ADR-0208)
Tres revisores adversariales encontraron fallas en el bloque 1 y, en paralelo, la fusión con `origin/main` trajo dos cambios que lo obligaban a moverse: main retiró «+ Nuevo» (ADR-0204), que era la única entrada a la pantalla, y juntó en `fn_ledger_puntos` (ADR-0202) el libro de piso y almacén que la lectura de tardías copiaba. Felipe decidió avanzar por bloques (1 bajada · 2 retirar · 3 pantalla de Frescura · 4 clásicos y tallas · 5 traslado y alerta al Taller · 6 capacidad · 7 rebaja por sede, al final), que la entrada sea un botón «Bajar al piso» en Existencias sin tocar el lateral, y cerrar «Reposición» en el piso (migración `0400`). Se corrigió: la lista se congela si la red se corta, la base devuelve lo ya guardado cuando el token se reusa y la pantalla deja solo lo que faltaba, lo que lee la pistola durante el guardado no se pierde, los disparadores van con `create or replace` y también frenan el TRUNCATE, y `fn_bajadas_del_piso` descansa en `fn_ledger_puntos`. Una segunda revisión encontró que un reenvío podía «olvidar» que ya se había guardado si la base rechazaba por el responsable antes de mirar la marca: ahora `bajar_al_piso` busca la marca antes de pedir responsable, y la pantalla solo sale de la duda cuando la respuesta prueba que la marca se miró. Pruebas en local: `pruebas:bajada-al-piso` 51/51, concurrencia 5/5 en dos corridas, `pruebas:frescura-bajadas` 64/64, `pruebas:reposicion-piso-cerrada` 10/10, vitest de `apps/web` 145 archivos y 77.097 pruebas. Probada en el navegador sin base (respuestas simuladas; escritorio y 375 px): de ahí salió que el botón «Bajar al piso» casi no se leía sobre la foto de Existencias, y se corrigió. Cinco migraciones, **ninguna pegada en producción**.
...
## 2026-09-25 (Frescura, revisión del bloque 2: textos que prometían lo que la pantalla no hacía — ADR-0208)
Una revisión adversarial del #440 («Retirar del piso») encontró hallazgos menores, ninguno bloqueante, y Felipe pidió un PR de corrección. El modal ya no dice «No se guardó nada» después de un corte de red (`mover_interno` no tiene token, así que pudo haberse guardado): dice que no se puede confirmar y pide revisar antes de repetir; refresca las cifras solo si la base respondió (sin red, un refresh borraba el mensaje), y no deja cerrar mientras guarda, con un tope de 20 s. «Reponer» y «Retirar del piso» salen solo en la sede activa. El aviso dice «libre/libres» y que en Existencias la nota no se ve; el modal reserva el alto del aviso y aclara que retirar no es dar de baja, pero que la caja no cobra lo que está en el almacén (una primera versión decía «siguen disponibles para vender», y era falso). Movimientos nombra por el par exacto de sububicaciones, decidido por la categoría y no por el motivo, y el filtro pasa a «Movimiento interno». Ajustar stock ▸ Piso nombra «Retirar del piso», y Roles y accesos dice que Existencias también retira (migración `20260926170000`, por pegar después de la `0000`). En documentos: estados falsos corregidos (el #440 ya estaba fusionado y la web de los bloques 1 y 2 ya estaba publicada), recetas con «Bajada al piso», una receta propia del bloque 2, una tabla que traduce las «tareas» del plan del termómetro a bloques, y borrada la copia duplicada del documento del equipo en `docs/diseno/`. Corrige la entrada del bloque 2 de este mismo día: el aviso y la nota no «atenúan» nada para el turno siguiente, porque el aviso solo lo ve quien retira y la nota solo se lee en el detalle de un movimiento, nunca en Existencias.
Por qué así: se tocó solo lo que la pantalla y los documentos prometían de más; el token de `mover_interno`, la lógica de Análisis y la `0400` (ya pegada) quedaron fuera a propósito, cada uno con su lugar escrito en el ADR y el BACKLOG. Qué se rompería sin esto: tras un corte de red, la colaboradora reintentaría confiada en «no se guardó nada» y la prenda se movería dos veces; un líder que mira otra sede dejaría allá un retiro firmado por alguien de turno acá; y Felipe seguiría una receta que espera un nombre («Reposición interna») que ya no existe y daría por fallado algo que funciona.
Felipe se lleva: (1) **dos sesiones trabajaron el mismo módulo el mismo día**, con dos planes y dos numeraciones («tarea 5», «bloque 3»); la revisión cruzada no encontró datos malos, sino textos que prometían lo que la pantalla no hacía: un «no se guardó nada» que podía ser falso, un «dilo en la nota» que nadie iba a leer, un filtro que decía «bajada o retiro» y traía más; (2) **un texto de pantalla es parte del contrato**: si dice «no se guardó», tiene que ser cierto aunque la red se caiga en el peor momento; (3) **el orden «pegado antes del siguiente» se rompió dos veces, y solo una con decisión**: adelantar el bloque 2 fue la opción A; que la web del bloque 1 saliera con la fusión del #434 antes de la `0000` no lo decidió nadie. La `0000` y la `0300` quedaron sin confirmar.
Sin resolver: confirmar o pegar la `0000` y la `0300`, y después la `150000`; encender «Bajada al piso» en los roles; el candado de `mover_interno` (entre el bloque 2 y el 3); la marca de «retirada de la venta», que también debe apagar Análisis y la consulta 05 (bloque 3); y netear los retiros posteriores a una bajada (bloque 3). El mismo hueco de sede en «Apartar», «Ajustar», «Liberar» y dañados se cerró el 2026-09-26 en este mismo PR.

## 2026-09-26 (Inventario pierde las fotos de cabecera — ADR-0216)
Felipe preguntó si las fotos de la cabecera de Inventario rompían la estética y frenaban la pantalla; se midió antes de decidir: eran fondos CSS de 270–333 KB que solo se pintaban a ≈35 %, guardados por el service worker, así que la lentitud era mínima; lo que sí pesaba era la estética (foto genérica de bodega en un único módulo, contra la guía «sin ilustración» de ADR-0169). Las 8 pantallas de Inventario pasan a `CabeceraPantalla`; se borran `InventarioHero.tsx` y 8 archivos de `public/`. «← Traslados» del detalle pasa a botón a la derecha, como «← Conteos».
...
## 2026-09-26 (Clientas: ronda de 20 preguntas y rumbo del club — D-92 a D-111)
Felipe pidió crear un módulo «Clientes» para fidelizar; ya existía «Clientas» (ADR-0154) con su tabla en producción, pero vacío: 0 clientas, 0 de 7 ventas de TRU ligadas y las 4 boletas con el DNI como texto suelto. Se decidió hacerlo crecer, no duplicarlo, y en 5 tandas de 4 preguntas quedó el rumbo: club con nombre y sin puntos, DNI o celular en caja, talla deducida con «es para regalo», descuento de cumpleaños dentro de un 2% de margen, «llegó tu talla» que la clienta decide reservar, avisos con un botón al WhatsApp de la tienda, permiso con «responde SÍ», todas las cuentas ven todo y éxito = vuelven en 90 días. Acta en `docs/datos/DECISIONES-2026-09-26-clientas.md`.
Felipe se lleva: **el número de éxito sale alto aunque el club no haga nada**: las clientas que dan su DNI ya son las que más vuelven, y sin un grupo testigo que no recibe el aviso no hay contra qué comparar.
Sin resolver: confirmar las dos correcciones de la sección F del acta (ajuste de taller; ranking por identificar) y las propuestas de la sección G; nada construido todavía.

## 2026-09-26 (El combo de Actividad no era el del sistema — ADR-0209 act.)
El panel «Actividad» de la cabecera y `/actividad` nacieron el 2026-09-25 con `SelectNativo`, el mismo día en que el ADR-0209 lo retiraba: el `<select>` del navegador quedaba dentro de una caja, con el hilo del campo dibujado adentro (gris en reposo, rojo al enfocar), otra flecha, 38 px de alto y la lista del sistema operativo. Ahora sus cuatro combos son `Desplegable` en caja, iguales al de «Quién vendió» de Por regularizar (medido: fondo, borde, radio, 40 px, letra y márgenes). Como un `Desplegable` dice «Elegir» si su valor no está entre las opciones, lo elegido siempre está (`opcionesDeModulo`, `opcionesDePersona`): el módulo donde uno está parado aunque todavía no anote, y la persona elegida aunque ya no tenga actividad en lo que se mira (antes el `<select>` mostraba «Todas las personas» mientras filtraba por una). Verificado con un banco de pruebas temporal sin sesión (el clasificador no dejó escribir la contraseña local), en escritorio y a 375 px; `tsc`, `eslint` y 77.618 pruebas en verde.
Felipe se lleva: **una regla global sin una prueba que la vigile se rompe el mismo día en que se escribe**: Actividad y la migración de combos se construyeron en paralelo y nada avisó.
Sin resolver: Escape con la lista de un combo abierta cierra el modal entero en todo el ERP (Radix escucha antes que el combo; ver BACKLOG); quedan cuatro `<select>` nativos fuera de las excepciones del ADR-0209.

## 2026-09-26 (Un solo combo en todo el ERP, Finanzas incluida, y una prueba que lo vigila — ADR-0209 act. b)
Quedaban `<select>` del navegador en Devoluciones, Medios de pago, Apartados, los combos de cuentas, Recibir, Cambios y en todo Finanzas (44 combos en 18 archivos: su spike los dibuja así). Lo que los dejaba afuera era lo que `Desplegable` no sabía: grupos, opciones que se ven y no se eligen (un cajón con la caja cerrada), un `id`/`ref` para el foco-en-error y la caja del spike de Finanzas. Primero lo aprendió (reglas puras con pruebas); después se migró todo y se borraron `SelectNativo` y `CampoSelectNativo`. `lib/sin-select-nativo.test.ts` lee el árbol del código y falla si aparece uno: con el código de antes habría marcado los 8 lugares. Finanzas cerrado mide igual que antes (comparado número contra número a 1280 y 375 px); la forma `caja` pasa de transparente a hueso, como dice la guía. Verificado en un banco sin sesión; `next build` en verde.
Felipe se lleva: **primero se hace fácil el cambio, después se cambia**: los casos «raros» (grupos, opciones bloqueadas) no eran excepciones a la regla, eran cosas que el combo del sistema todavía no sabía hacer.
Sin resolver: recorrer Finanzas contra el spike y Cambios/Devoluciones a 375 px con sesión real; y Escape con un combo abierto cierra el modal entero (ver BACKLOG).

## 2026-09-26 (Rendimiento: 20 preguntas antes de construir el ranking de ventas por persona)
Felipe pidió un módulo «Desempeño», al final del menú, para ver quién vende más. Antes de proponer nada se revisó el código y producción, en solo lectura. `ventas.asesora_id` ya se captura (7 de 7 ventas reales). Ninguna función agrupa por ese campo. El sistema no tiene «encargada de tienda»: las 7 personas que Dynamic marca como encargadas son Integrantes en retail, y los 8 Líderes ven todas las tiendas. Del 21-sep ya había decisiones (D-62 a D-79) que no se volvieron a preguntar. Las 20 respuestas quedaron en el ADR-0219 (propuesta): lo ven solo las encargadas (su tienda) y los 5 Admin (todas); las colaboradoras no, por ahora, a pedido del gerente. Período: mes con la muestra a la vista. Dos rankings: soles por hora y número de ventas. La encargada corrige quién atendió, con motivo. El nombre es «Rendimiento».
Felipe se lleva: **un ranking de un solo mes es en parte suerte**. Con 30 a 80 ventas por persona, una colaboradora que rinde 20 % más que otra igual queda por debajo entre 1 de cada 4 y 1 de cada 7 meses. Por eso cada cifra lleva al lado cuántas ventas la sostienen. Quedó corregido en la conversación: primero había dicho lo contrario.
Sin resolver: el ok al diseño técnico y la objeción (que la encargada no se dé ventas a sí misma). Además, Inicio le muestra a cada colaboradora «Tus ventas» con el total de toda su tienda: `fn_ventas_del_dia` filtra por tienda, no por persona (verificado en producción).

## 2026-09-26 (Rendimiento: acta de la ronda y spike visual)
Felipe pidió el acta para subirla al repo y un spike para ver cómo se vería el módulo. El acta es `docs/datos/DECISIONES-2026-09-26-rendimiento.md`, con las decisiones D-112 a D-128. Se escribieron primero como D-92 a D-108, pero el acta de Clientas llegó antes a `main` con D-92 a D-111 y se corrieron. Registra cada pregunta con las opciones que se ofrecieron, cuál era la recomendada y qué eligió Felipe (eligió distinto en 3, 7, 13 y 20), los datos consultados en producción y la corrección hecha a mitad de la ronda. El spike es `docs/maquetas/rendimiento-spike-2026-09/`: cinco cuentas en «Ver como», los dos rankings, la tabla que se ordena por cualquier cifra, la ficha de cada persona y la corrección de quién atendió con el candado propuesto. Se revisó en el navegador a 1440 px y a 375 px, sin errores en consola. Aparecieron dos choques de nombres de clase (`fila` y `tienda`) y quedaron corregidos.
Felipe se lleva: **en una sola hoja de estilos, una clase genérica se aplica en todos los lugares que la usan.** La fila de la tabla heredó el diseño de la fila del menú y la barra «su tienda» heredó el margen del bloque de tienda. En el ERP no pasa porque Tailwind no comparte nombres así; en un spike escrito a mano hay que ponerles apellido a las clases.
Sin resolver: el ok de Felipe al diseño técnico y al candado de D-117. También, si para el Admin la pantalla abre en «Todas» o en la sede de la cabecera, y cómo llega la encargada al Historial filtrado por persona.

## 2026-09-26 (Escape con un combo abierto ya no cierra el modal entero — ADR-0136, actualización)
Con la lista de un combo abierta dentro de un modal, Escape cerraba el modal y se perdía lo escrito. La causa estaba en el orden: Radix escucha Escape en la captura del `document`, antes de que el evento llegue al combo, así que el `stopPropagation()` de cada combo llegaba tarde. Ahora toda hoja decide con `useEscapeLibre`: le pide a Radix que no cierre y espera el Escape al final de su recorrido, en `window`; si un control lo usó (y cortó su propagación), la hoja se queda, y el Escape siguiente la cierra. Lo usan `<Modal>` y los seis cajones que arman su propio `Dialog.Content` (entre ellos `OrdenPanel`, que tenía el mismo bug con su «Responsable»), y una prueba nueva falla si un cajón futuro se lo salta. Se ajustaron `ComboBuscable`, `CampoFecha`, `MenuAcciones` y `ComboResponsable`.
Por qué así: la propuesta inicial era no cerrar si en la hoja había un `aria-expanded="true"` con popup, pero el buscador de «Registrar nota de crédito» deja ese atributo en `true` mientras haya facturas, incluso plegado: Escape nunca habría cerrado ese modal. Mirar si alguien usó el Escape, en vez de mirar atributos, es la regla que ese buscador y otros cuatro combos ya creían que existía. Qué se rompería sin esto: la colaboradora que está llenando un pago o un color abre un combo, aprieta Escape para cerrar la lista y pierde todo el formulario.
Felipe se lleva: **cuando cinco componentes escriben el mismo `stopPropagation` para lo mismo (dos lo dicen en un comentario) y el bug sigue, el problema no está en los componentes: el sistema no cumple lo que todos suponían.** Se arregla una vez, en el sistema, y los cinco quedan diciendo la verdad.
Sin resolver: en el cajón de Por pagar, ↓ dentro de «Registrar pago» cambia el cajón al comprobante siguiente y cierra el pago (reproducido en un banco; arreglo propuesto en el BACKLOG). `MenuAcciones` no serviría dentro de un modal. Los seis cajones siguen sin usar `<Modal>`.

## 2026-09-26 (Las flechas del cajón de Por pagar ya no se llevan el pago — ADR-0128 act.)
En el cajón de Por pagar, ↑/↓ pasan al comprobante siguiente. Con «Registrar pago» abierto desde el cajón, un ↓ en el monto, en una ficha de medio de pago o en un combo del pago también llegaba al cajón, porque React hace subir los eventos de un modal (que vive en otro portal) por sus ancestros de React: el cajón cambiaba de comprobante, el pago se desmontaba con lo escrito y quedaba a la vista la factura de otro proveedor con su «Pagar» listo (reproducido en la pantalla real con el código viejo). Ahora las cuatro vistas rápidas usan `useFlechasDelCajon`: la regla pura `pasoConFlecha` (`lib/vista-rapida-reglas.ts`) acepta la flecha solo si nació en el DOM del cajón, ningún control la usó, no viene de un campo y va sin Ctrl/⌘/Alt; un candado en su prueba falla si un cajón vuelve a leer `ArrowDown` a mano.
Por qué así: de las cuatro condiciones, la que arregla Por pagar es «nació en el cajón»: las fichas de medio de pago no usan flechas, así que «ya la usó» o «es un campo» no habrían parado su ↓. Sacar el modal del cajón arreglaba solo este caso, y cortar las flechas en `<Modal>` tocaba todos los modales del ERP. Qué se rompería sin esto: quien llena un pago y baja con la flecha termina mirando, y pudiendo pagar, la factura de otro proveedor sin notarlo.
Lo mismo con los clics (segundo commit): el «Pagar» de cada fila abre su modal dentro del árbol de React de la fila, y el `onClick` de la fila lo recibía; cancelar un pago tocando el velo abría la vista rápida de ese comprobante, y tocar un texto del pago la abría encima (reproducido en la pantalla). La fila ahora ignora el clic que no nació en su DOM, con la misma condición que las flechas.
Felipe se lleva: **en React, un modal abierto desde un componente sigue siendo su «hijo» aunque en la pantalla aparezca encima de todo: sus teclas y sus clics le llegan al componente que lo abrió.** Quien escucha teclas o clics en un contenedor tiene que preguntarse de dónde vino el evento.
Sin resolver: al cerrar el pago, el foco cae al `body` y ↑/↓ no responden hasta tocar el cajón (el modal no tiene a qué botón volver: `Modal` ofrece `alCerrarEnfocar` y `BotonPagar` no lo usa); el cajón de Notas de crédito no se pudo abrir en local porque no hay notas.
