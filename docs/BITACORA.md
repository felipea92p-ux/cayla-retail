...
Sin resolver: pegar la migración en producción (antes de publicar la web); encender el módulo en el rol de la líder de tienda y probarlo con esa cuenta; sumar los demás módulos. Aparte: `pruebas:registrar-venta` (2) y `pruebas:caja-cierre-traslado` (3) ya fallaban en `main` sin este cambio (reglas recientes de descuento y de traslado): no son de esta rama.
