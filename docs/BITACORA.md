...
## 2026-09-26 (Conflictos del PR #445 «reestructura Existencias» resueltos contra `main`)
Pediste resolver los conflictos del PR #445 y verificar si fusionarlo afecta lo que ya existe. Se trajo `main` (194 commits) a su rama: 4 archivos en conflicto y 3 choques que git no marca porque no son de texto. «Retirar del piso» usaba el semáforo que el PR borra, sus pruebas esperaban el umbral viejo de 7, y la prueba SQL nueva del PR no podía pasar con la siembra estándar (nunca corrió en el CI: con conflictos, GitHub no arma el merge). Todo lo que `main` sumó a Existencias después del PR se conservó y se probó con sesión de líder (marca y vacío que explica, salto a la tabla, tarjeta del celular, «Retirar del piso», nada que escriba mirando otra sede); tipos, 77.762 pruebas, lint y pruebas SQL (9/9 y 48/48) en verde.
Lo que cambia para la tienda si se fusiona, a propósito del PR: «Reponer a piso hoy» cuenta las tallas con 4 o menos libres en el piso (antes, 7 o menos con más de 10 en almacén: en TRU local pasa de 0 a 16); desaparecen «Stock bajo · pedir traslado» y «Sin stock»; «Ver recomendaciones» ya no propone cantidades ni traer de otra sede (eso queda solo en Análisis); «Disponible» se funde en «Stock actual»; sale la barra de distribución; el filtro Estado se parte en Acción y Estado; el CSV pierde «Apartadas»; el aviso de «Retirar del piso» salta con 4 o menos. Su migración ya estaba en producción (consultado en vivo: mismo cuerpo, md5 igual), aunque el volcado del repo decía que no; y si faltara, la pantalla no se cae: Ritmo y Cobertura quedan en «N/D» con aviso (probado en local sin la función).
Felipe se lleva: (1) **un conflicto que git no marca es el más peligroso**: el aviso del retiro compilaba contra una función borrada y habría prometido «Existencias sugerirá Reponer» con un umbral que ya no existe. (2) **Un PR en conflicto no corre su CI**: sus pruebas nuevas pueden estar rotas desde el día uno sin que nadie lo sepa. (3) **Una regla de negocio que vive solo en un comentario de código no es una decisión tomada**: el PR baja el umbral de «Reponer» de 7 a 4, el mismo 4 que ADR-0071 descartó el 16-09 porque «el aviso llegaba tarde», y no tiene ADR que diga por qué ahora sí.
