...
## 2026-09-26 (DRIFT.md dejó de decir «sobra» y de afirmar lo que no sabe — PR #454)
`docs/datos/generado/DRIFT.md` listaba 29 funciones como «que nadie llama… una función que sobra y habría que retirar», y en ese informe había 8 pantallas «rotas» que en producción existían. Causas: `comparar.mjs` solo veía `.rpc("nombre", …)` con el nombre literal (un ternario o un ayudante era invisible), contaba comentarios como usos, y comparaba contra una foto de producción sin decir de cuándo era. Ahora lee el código con el parser de TypeScript (`scripts/datos/comparar-lectura.mjs`), la lista se llama «Funciones sin llamada detectada desde `apps/web`» (21) y dice que **no prueba que sobren**, el informe trae la fecha de la foto y habla de «sin respaldo en la foto» (13 al fusionar, todas falsas alarmas de una foto vieja: verificadas por SELECT), y una llamada con parámetros ilegibles a una función ausente también se cuenta.
Por qué así: retirar una función viva porque un informe dijo «sobra» rompe Finanzas o Proveedores en las tiendas, y una alarma que se equivoca a menudo enseña a ignorarla. Un escáner a mano volvió a equivocarse dos veces (regex con comilla, `as never`): reconocer un comentario, un texto o una regex es trabajo del parser, que ya estaba en el repo. Qué se rompería sin esto: alguien retira `recibir_compras` (la llama `recibir_envio`, no una pantalla) y la recepción de mercadería deja de guardar.
Felipe se lleva: **«nadie la nombra» no es «sobra»** — una función de la base puede tener quien la llame dentro de la base (otra función, un disparador) o fuera del front (script, Dynamic). Verificado: `node --test` 74 pruebas (40 de la lectura, 34 de extremo a extremo) y `scripts/datos/comparar-mutaciones.mjs` (55 mutantes: 54 mueren, 1 equivalente en macOS); tres rondas de revisión independiente, la última encontró el titular mal etiquetado (313 llamadas, no 265), una llamada indirecta a `bajar_al_piso` que no se veía y 4 afirmaciones falsas en docs. Sin resolver: refrescar la foto de funciones (producción 578, foto 544, sin DEFAULTs); una sobrecarga en producción no cambia el código de salida; una llamada indirecta a una función que ni la foto ni las migraciones conocen sigue invisible.
