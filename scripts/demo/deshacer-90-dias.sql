...
begin
  ...
  if v_malas <> '' then
    raise exception E'[deshacer] Hay datos reales enganchados a la demo; no se borra nada. Resolverlos primero:%', v_malas;
  end if;
end $$;
...
-- Series de comprobantes: lo mismo con el correlativo (siguiente_numero previo = primer número sembrado).
...
from comprobantes c where c.id::text like '5eed%' group by 1, 2;
