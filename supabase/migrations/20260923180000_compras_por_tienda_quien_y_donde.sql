-- ============================================================================
-- 20260923180000_compras_por_tienda_quien_y_donde.sql — CAYLA V2 · ADR-0184 (F0 reescrita sobre ADR-0161)
--
...
comment on table retail.compradores_de_tienda is
  'ADR-0184. Tiendas EXTRA cuyas Compras ve y paga una persona, además de la suya (la persona de Compras que atiende varias, R-10). No da acceso por sí sola: hace falta que su rol vea un módulo de Compras (ADR-0161). Se escribe solo por agregar_/quitar_comprador_de_tienda.';
...
comment on function retail.fn_compras_ubicaciones() is
  'ADR-0184. Tiendas cuyas Compras ve, registra y paga quien consulta: todas si es líder; si su rol ve Facturas de compra, Por pagar o Notas de crédito, SU tienda (fn_ubicacion_actual_persona) más las extra de compradores_de_tienda; si no, ninguna.';
...
comment on function retail.fn_puede_comprar_en(uuid) is
  'ADR-0184. ¿Gestiona quien consulta las Compras de esta tienda? fn_compras_ubicaciones() para una sola. NULL → false.';
...
comment on function retail.quitar_comprador_de_tienda(uuid, uuid) is
  'ADR-0184. Solo líder. Quita esa tienda extra. Las compras ya hechas siguen siendo de su tienda.';
