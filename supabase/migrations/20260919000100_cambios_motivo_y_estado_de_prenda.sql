-- ============================================================================
-- 20260919000100_cambios_motivo_y_estado_de_prenda.sql — CAYLA V2
--
...
-- Re-ejecutable (2026-09-19): el archivo cambió de versión al fusionar con `main` (la
-- 20260918150000 la ocupa otra migración) y en la base local ya estaba aplicado con el nombre
-- viejo. Por eso todo es `if not exists` / `create or replace`: aplicarlo dos veces no rompe.

set search_path = retail, public, extensions;

-- ---------- 1-3. cambios: motivo, estado de la prenda que vuelve, y su candado ----------
alter table retail.cambios add column if not exists motivo text
  check (motivo in ('talla_chica', 'talla_grande', 'otro_color', 'defecto', 'otro'));
comment on column retail.cambios.motivo is
  'Por qué la clienta cambia la prenda (lista cerrada, R-45). Null solo en cambios anteriores a 20260919000100.';

alter table retail.cambios add column if not exists condicion text not null default 'vendible'
...
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'cambios_defecto_no_vuelve_al_piso') then
...
alter table retail.prendas_danadas add column if not exists cambio_id uuid unique references retail.cambios (id);
...
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'prendas_danadas_un_origen') then
...
drop function if exists retail.registrar_cambio(uuid, uuid, uuid, integer, text, uuid);
