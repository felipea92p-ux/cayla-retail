-- ============================================================================
-- Orden de producción: una sola forma de producir, con etapas, maquila,
-- costo estimado al abrir y costo REAL (con mermas) al cerrar.
--
-- Antes había dos mecanismos que se pisaban (la corrida con costo y el viejo
-- rastreador de etapas). Se unifican en `producciones`:
--   · costo_maquila  → lo que mandas afuera (planchado, corte tercerizado…).
--   · etapas jsonb   → {corte, confeccion, acabado} = pendiente|hecho|tercerizado.
--   · fecha_entrega  → "para cuándo".
--   · costo_unitario → ahora incluye la maquila.
--
-- Cerrar la orden (cerrar_produccion) confirma cuántas salieron BUENAS y el
-- costo REAL: el costo se reparte entre las buenas (la merma normal queda
-- absorbida en el costo, no como pérdida aparte) y esas prendas entran al stock.
-- ============================================================================

alter table producciones add column if not exists costo_maquila numeric(12, 2) not null default 0;
alter table producciones add column if not exists etapas jsonb not null default '{}'::jsonb;
alter table producciones add column if not exists fecha_entrega date;

-- El costo por prenda ahora incluye la maquila. Se recrea la columna generada.
alter table producciones drop column if exists costo_unitario;
alter table producciones add column costo_unitario numeric(12, 2)
  generated always as (round((costo_tela + costo_avios + costo_maquila) / nullif(cantidad, 0), 2)) stored;

-- Un solo camino de cierre: se elimina el marcado simple anterior.
drop function if exists marcar_produccion_terminada(uuid);

-- ----------------------------------------------------------------------------
-- Marcar el estado de una etapa (pendiente / hecho / tercerizado).
-- ----------------------------------------------------------------------------
create or replace function set_etapa_produccion(p_produccion_id uuid, p_etapa text, p_estado text)
returns void
language plpgsql
security definer
...
begin
  ...
  if not fn_puede_operar_sede(v_unidad) then
    raise exception 'No tienes permiso sobre esta orden';
  end if;
  if p_etapa not in ('corte', 'confeccion', 'acabado') then
...
-- ----------------------------------------------------------------------------
-- Cerrar la orden: confirma buenas + costo real, y manda al inventario.
-- p_buenas = [{ "variante_id": uuid, "cantidad": n }]  (las que salieron bien)
-- ----------------------------------------------------------------------------
create or replace function cerrar_produccion(
...
begin
  ...
  if v_prod.inventariado_at is not null then
    raise exception 'Esta orden ya está cerrada en el inventario';
  end if;

  select id into v_persona_id from personas where auth_user_id = auth.uid();

  -- Una muestra no entra al inventario: solo se cierra con su costo real.
  if v_prod.es_muestra then
    update producciones
      set costo_tela = coalesce(p_costo_tela, 0),
          costo_avios = coalesce(p_costo_avios, 0),
          costo_maquila = coalesce(p_costo_maquila, 0),
          estado = 'terminado'
      where id = p_produccion_id;
    return;
  end if;

  -- Ajusta el desglose a lo que salió BUENO (borra las que quedaron en 0).
  v_total := 0;
  for v_item in select * from jsonb_array_elements(p_buenas)
  loop
    ...
    if v_cant <= 0 then
      ...
      update produccion_lineas set cantidad = v_cant
        where produccion_id = p_produccion_id and variante_id = v_variante_id;
      v_total := v_total + v_cant;
    end if;
  end loop;

  if v_total <= 0 then
    raise exception 'No hay prendas buenas para mandar al inventario';
  end if;

  -- Costo y cantidad reales → el costo_unitario (generado) se recalcula solo.
  update producciones
    set costo_tela = coalesce(p_costo_tela, 0),
        costo_avios = coalesce(p_costo_avios, 0),
        costo_maquila = coalesce(p_costo_maquila, 0),
...
-- ----------------------------------------------------------------------------
-- Abrir una orden. Reemplaza al registrar_produccion anterior (agrega maquila,
-- fecha de entrega; el cierre inmediato usa cerrar_produccion con lo planeado).
-- ----------------------------------------------------------------------------
drop function if exists registrar_produccion(uuid, integer, numeric, numeric, numeric, jsonb, uuid, text, text, text, boolean, text);
...
begin
  ...
  -- Cierre inmediato (foto de apertura / algo ya terminado): planeado = real.
  if coalesce(p_marcar_terminado, false) then
    perform cerrar_produccion(v_prod_id, coalesce(p_costo_tela, 0), coalesce(p_costo_avios, 0),
                              coalesce(p_costo_maquila, 0), v_buenas);
  end if;
