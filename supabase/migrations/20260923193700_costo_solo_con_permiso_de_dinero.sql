...
-- catálogo, y Conteo le mandaba los 1.295 costos al navegador. Además cuatro funciones con permisos propios lo
-- devolvían sin preguntar (`fn_productos`, `fn_resumen_comparacion`, `fn_costo_historial`, `censo_crear_variante`).
...
--      no puede leer `costo`.
--   4. Las cuatro funciones devuelven el costo vacío a quien no tiene el permiso (`case when … then … end`; el
...
CREATE OR REPLACE FUNCTION retail.fn_resumen_comparacion(p_ubicacion_id uuid, p_a_desde date, p_a_hasta date, p_b_desde date, p_b_hasta date)
 RETURNS TABLE(variante_id uuid, producto_id uuid, referencia text, categoria_id uuid, categoria_nombre text, producto_estado text, producto_codigo text, sku text, codigo text, codigos_barras text[], talla text, color_codigo text, color_nombre text, color_hex text, costo numeric, estado_costo text, ledger_consistente boolean, a_ventas integer, a_devoluciones integer, a_importe numeric, a_costo_ventas numeric, a_costo_devoluciones numeric, a_uds_sin_costo integer, a_entradas integer, a_stock_inicio integer, a_stock_cierre integer, a_dias_con_stock numeric, b_ventas integer, b_devoluciones integer, b_importe numeric, b_costo_ventas numeric, b_costo_devoluciones numeric, b_uds_sin_costo integer, b_entradas integer, b_stock_inicio integer, b_stock_cierre integer, b_dias_con_stock numeric)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'retail', 'public', 'extensions'
AS $function$
...
select
  ...
  case when (select retail.fn_puede_ver_dinero_de_compras()) then b.costo_valor end, b.costo_estado,
  (b.dias_consistente and least(b.a_ini_raw, b.a_fin_raw, b.b_ini_raw, b.b_fin_raw) >= 0),
  coalesce(b.a_ventas, 0), coalesce(b.a_devoluciones, 0), round(coalesce(b.a_importe, 0), 2), case when (select retail.fn_puede_ver_dinero_de_compras()) then round(coalesce(b.a_costo_ventas, 0), 2) end, case when (select retail.fn_puede_ver_dinero_de_compras()) then round(coalesce(b.a_costo_devoluciones, 0), 2) end, coalesce(b.a_uds_sin_costo, 0), coalesce(b.a_entradas, 0),
  greatest(b.a_ini_raw, 0)::integer, greatest(b.a_fin_raw, 0)::integer, b.a_dias,
  coalesce(b.b_ventas, 0), coalesce(b.b_devoluciones, 0), round(coalesce(b.b_importe, 0), 2), case when (select retail.fn_puede_ver_dinero_de_compras()) then round(coalesce(b.b_costo_ventas, 0), 2) end, case when (select retail.fn_puede_ver_dinero_de_compras()) then round(coalesce(b.b_costo_devoluciones, 0), 2) end, coalesce(b.b_uds_sin_costo, 0), coalesce(b.b_entradas, 0),
