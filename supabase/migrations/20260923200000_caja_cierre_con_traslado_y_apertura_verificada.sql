-- Cierre con traslado y apertura verificada (ADR-0186, 2026-09-23 — spike docs/maquetas/caja-cierre-spike-2026-09/).
--
...
comment on column retail.cajas.monto_fondo is
  'Efectivo que quedó en el cajón al cerrar: contado − trasladado (ADR-0186). La apertura siguiente de la sede lo usa como «lo que debería haber». NULL en cierres anteriores a ADR-0186.';
comment on column retail.cajas.monto_apertura_esperado is
  'monto_fondo del último cierre de la sede al momento de abrir (ADR-0186). NULL si no había con qué comparar.';
...
comment on table retail.caja_traslados is
  'A dónde fue el efectivo contado al cerrar una caja (ADR-0186). Solo se escribe desde cerrar_caja; no se edita ni borra.';
...
-- Pública: solo quien puede cerrar esa caja ve cuánto espera el sistema (ADR-0186 lo muestra desde el inicio del cierre).
...
begin
  ...
  -- Lo que quedó en el cajón en el último cierre real de la sede. Los cierres de antes de ADR-0186 no lo tienen
