-- ============================================================================
-- 0048 — Sesiones de conteo: el censo y el control permanente son lo mismo
--
-- LA IDEA
...
--   que se use una vez y se abandone: la misma pantalla que trae las 900 prendas
--   de una vez es la que mantiene el inventario honesto para siempre.
--
-- POR QUÉ SESIÓN Y NO APLICAR AL INSTANTE
--   Aplicar cada línea en el momento habría sido más simple y también correcto.
--   Se eligió la sesión porque Felipe pidió (2026-09-09) **aprobar antes de que
--   entre nada**: las Encargadas cuentan, y hasta que él no cierra, el `stock` no
--   se mueve ni un gramo. El cierre ES la aprobación.
--   Precio de la decisión: dos tablas más y un estado que alguien puede dejar
--   abierto. Se mitiga con `conteos_un_abierto_por_sede`.
--
-- LA DECISIÓN MÁS FINA: `cantidad_sistema` SE CONGELA AL CONTAR, NO AL CERRAR
--   Sistema dice 10. A las 15:00 la Encargada cuenta y hay 8. A las 15:30 se
--   vende 1 → sistema 9. A las 16:00 el Líder cierra.
--     · Con el delta calculado al CONTAR (8 − 10 = −2): 9 + (−2) = **7**. ✔
--       (contó 8, se vendió 1, quedan 7)
--     · Leyendo el sistema al CERRAR (8 − 9 = −1): 9 + (−1) = 8. ✘ La venta
--       desaparece.
--   Un conteo es una afirmación sobre un INSTANTE, no sobre el presente. Un
--   delta compone con lo que pasó después; un absoluto lo pisa. Por eso
--   `conteo_lineas.cantidad_sistema` guarda lo que el sistema decía cuando la
--   prenda se contó, y el `do update` de un re-conteo NO lo toca.
--
-- POR QUÉ `ajuste` CON SIGNO Y NO UN `tipo='conteo'` NUEVO
...
--      la red de seguridad para arreglar otra cosa, el censo entero se borraría.
--      Bug latente que estalla meses después, cuando nadie recuerda por qué.
...
--   Esto es posible gracias a `0045_ajuste_con_signo.sql` (ADR-0023): antes de
--   esa migración un ajuste negativo era imposible.
--
-- EL ALCANCE ES DESCRIPTIVO, NO RESTRICTIVO
--   No limita qué variantes se pueden contar. Si alguien encuentra un jean
--   mientras cuenta blusas, tiene que poder registrarlo — bloquearlo garantiza
--   que ese jean nunca entre al sistema. El alcance alimenta la hoja de trabajo
--   de la pantalla y define el universo de `poner_en_cero`. Una ruta menos de
--   enforcement, una forma menos de frenar a alguien con una prenda en la mano
--   (principio 10).
--
-- SE ROMPE SI: el negocio necesitara dos equipos contando la misma sede a la vez
--   con hojas separadas. El índice único lo impide a propósito: dos conteos
--   abiertos calcularían cada uno su varianza contra un sistema que el otro está
--   por cambiar, que es la definición de estado inconsistente (principio 2).
--
-- CÓMO SE REVIERTE: `drop table conteo_lineas, conteos;` más el drop de las 7
--   funciones. Nada de lo anterior depende de esto. Los `movimientos` que un
--   cierre haya emitido NO se revierten — son historia (principio 4); se corrigen
--   con otro conteo.
-- ============================================================================
...
create table conteos (
  ...
  alcance_categoria_id uuid references categorias (id),
  alcance_contenedor_id uuid references contenedores (id),
  -- Qué hacer con lo que el alcance cubre pero nadie contó. 'ignorar' por
  -- defecto: es la única pieza del diseño que puede destruir datos en masa, y en
  -- el censo inicial (con `stock` casi vacío) no hay nada que poner en cero.
  ...
  nombre text,
  abierto_por uuid references personas (id),
...
create index conteos_sede_idx on conteos (sede_id, estado);

-- LA restricción más importante del diseño: un solo conteo abierto por
-- sede+ubicación. Dos conteos abiertos calcularían cada uno su varianza contra
-- un sistema que el otro está por cambiar.
create unique index conteos_un_abierto_por_sede
  on conteos (sede_id, ubicacion) where estado = 'abierto';
...
create table conteo_lineas (
  ...
  -- Lo que el sistema decía CUANDO SE CONTÓ. Nunca se recalcula (ver cabecera).
  cantidad_sistema integer not null,
  contenedor_id uuid references contenedores (id),
  contado_por uuid references personas (id),
  contado_en timestamptz not null default now(),
  actualizado_en timestamptz not null default now(),
  diferencia integer,                    -- se sella al cerrar
  movimiento_id uuid references movimientos (id),
  nota text,
  unique (conteo_id, variante_id)
);
create index conteo_lineas_conteo_idx on conteo_lineas (conteo_id);

alter table conteos enable row level security;
alter table conteo_lineas enable row level security;
create policy conteos_select on conteos for select using (fn_puede_operar_sede(sede_id));
create policy conteo_lineas_select on conteo_lineas for select
  using (exists (select 1 from conteos c where c.id = conteo_id and fn_puede_operar_sede(c.sede_id)));
-- Sin policies de escritura: se escribe SOLO vía las RPC de abajo, igual que
-- `stock` y `movimientos`. Es lo que hace imposible contar sin dejar rastro.

-- ---------- 3. una sola forma de componer el código de variante ----------
-- Se extrae de `fn_asignar_codigo_variante` (0047) para que `conteo_crear_variante`
-- pueda calcular el código ANTES de insertar (necesita el `sku`, que es not null)
-- sin duplicar la regla en dos lugares. La función de 0047 se reemplaza abajo
-- para que las dos usen la misma.
create or replace function fn_componer_codigo_variante(
  p_base text, p_color_id text, p_talla text
) returns text language sql immutable as $$
  select p_base || case when p_color_id is null then '' else '-' || p_color_id end
                || '-' || fn_token_talla(p_talla);
$$;
...
begin
  ...
  v_codigo := fn_componer_codigo_variante(v_base, v_color_id, v_talla);
...
-- ---------- 4. abrir ----------
create or replace function abrir_conteo(
...
-- ---------- 5. contar una prenda ----------
-- `p_modo`:
--   'sumar' (por defecto) — cada disparo de la pistola es UNA prenda que
--     acabas de levantar de la pila. Si el modo fuera absoluto, el segundo
--     escaneo de la segunda blusa idéntica pisaría al primero y el conteo
--     siempre daría 1.
--   'fijar' — para teclear un número corregido, o para una importación.
create or replace function conteo_contar(
...
begin
  ...
  if not fn_puede_operar_sede(c.sede_id) then
    raise exception 'No tienes permiso para contar en esa sede';
  end if;
  ...
  -- Lo que el sistema dice AHORA, que es el instante que esta línea afirma.
  ...
  insert into conteo_lineas (conteo_id, variante_id, cantidad_contada, cantidad_sistema,
    ...
    on conflict (conteo_id, variante_id) do update
      set cantidad_contada = case when p_modo = 'sumar'
          ...
          actualizado_en = now()
      -- OJO: `cantidad_sistema` NO se toca en el do update. Se conserva el del
      -- primer escaneo, que es el instante que la línea afirma (ver cabecera).
    returning id into v_linea_id;
  return v_linea_id;
end $$;

-- ---------- 6. contar escaneando (el camino de la pistola Zebra) ----------
-- Resuelve contra `codigos_barras` (código corto, sku viejo y códigos de
-- fábrica), y como red por si algo quedó fuera de esa tabla, contra
-- `variantes.codigo` y `variantes.sku`. Devuelve null si no conoce el código —
-- la pantalla usa eso para ofrecer "crear esta prenda", que es lo que convierte
-- el conteo en censo.
create or replace function conteo_contar_por_codigo(
...
begin
  ...
  return conteo_contar(p_conteo_id, v_variante_id, p_cantidad, p_modo);
end $$;

-- ---------- 7. crear una prenda al vuelo (esto es lo que hace del conteo un censo) ----------
-- Gate: `fn_puede_operar_sede`, NO `fn_es_lider`. Es el cambio de permiso
-- deliberado que hace posible el censo: `crear_producto_con_variantes` (0035)
-- rechaza a quien no es Líder, y si cada ficha tiene que crearla Felipe, no hay
-- censo. El control lo da el cierre, que sí es solo de Líder (ver `cerrar_conteo`).
-- NO toca stock: el stock aparece al cerrar, como un ajuste, con auditoría.
create or replace function conteo_crear_variante(
  p_conteo_id uuid,
  p_referencia text,
  p_talla text,
  p_color_codigo text,
...
begin
  ...
  -- El modelo: el que mandan, o uno nuevo.
  v_producto_id := p_producto_id;
  if v_producto_id is null then
    if coalesce(trim(p_referencia), '') = '' then
      raise exception 'Falta el nombre de la prenda';
    end if;
    -- `sku_padre` sigue siendo not null unique; se le da un valor provisional y
    -- el nombre real de cara a la gente es `codigo`.
    ...
    v_base := fn_asignar_codigo_producto(v_producto_id);
    update productos set sku_padre = v_base where id = v_producto_id;
  else
    v_base := fn_asignar_codigo_producto(v_producto_id);
  end if;

  -- ¿Ya existe esa combinación? `variantes_identidad_unica` (0047) garantiza que
  -- haya como mucho una, así que dos personas creando la misma prenda a la vez
  -- terminan contando sobre la misma fila en vez de duplicarla.
  ...
  if v_variante_id is null then
    v_codigo := fn_componer_codigo_variante(v_base, p_color_codigo, p_talla);
    -- El sku por defecto es el código escaneado de fábrica si lo hay, y si no el
    -- código corto: así una prenda nueva nace ya escaneable por lo que trae puesto.
  ...
  -- El código de fábrica que se escaneó: la prenda lo adopta y desde ahora la
  -- pistola la encuentra sin imprimir ni pegar nada.
  if nullif(trim(p_codigo_barras), '') is not null then
    perform registrar_codigo_barras(v_variante_id, trim(p_codigo_barras), 'proveedor',
                                    'adoptado durante el conteo');
  end if;

  return conteo_contar(p_conteo_id, v_variante_id, p_cantidad, 'sumar', p_contenedor_id);
end $$;

-- ---------- 8. previsualizar el cierre ----------
-- Es lo que hace que cerrar no dé miedo: se ve exactamente qué va a pasar antes
-- de que pase. Read-only.
create or replace function previsualizar_cierre_conteo(p_conteo_id uuid)
returns table (
...
as $$
  ...
  -- Lo que el alcance cubre y nadie contó, solo si el conteo pide ponerlo en 0.
...
-- SOLO Líder. Es el momento —y el único— en que el conteo toca el stock.
-- Idempotente: cerrar dos veces devuelve el resumen guardado en vez de duplicar
-- movimientos. Dos cierres concurrentes: el segundo se bloquea en el `for
-- update`, luego ve 'cerrado' y devuelve sin emitir nada. Sin tabla de
-- idempotency-keys, sin nada extra.
...
begin
  ...
  if c.estado <> 'abierto' then
    -- Idempotencia: no es un error volver a cerrar, es un no-op informativo.
    return query
  ...
  -- Si el conteo es del almacén, los ajustes tienen que enrutarse a la bolsa del
  -- almacén, y eso se hace pasando su contenedor (ver fn_aplicar_movimiento).
  if c.ubicacion = 'almacen' then
  ...
  -- Si `poner_en_cero`, las no contadas se materializan como líneas con 0 para
  -- que queden auditadas, en vez de ser movimientos huérfanos que nadie explica.
  if c.tratar_no_contado = 'poner_en_cero' then
    insert into conteo_lineas (conteo_id, variante_id, cantidad_contada, cantidad_sistema, contado_por, nota)
      ...
      from previsualizar_cierre_conteo(p_conteo_id) pv
...
create or replace function anular_conteo(p_conteo_id uuid, p_motivo text)
...
begin
  ...
  if not found then raise exception 'El conteo % no existe', p_conteo_id; end if;
  if c.estado <> 'abierto' then raise exception 'Ese conteo ya está %', c.estado; end if;
  if not fn_es_lider() then raise exception 'Solo un líder puede anular un conteo'; end if;
