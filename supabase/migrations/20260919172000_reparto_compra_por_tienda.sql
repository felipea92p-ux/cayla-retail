-- ============================================================================
-- 20260919172000_reparto_compra_por_tienda.sql — CAYLA V2 · ADR-0139 (parte 1 de 2)
--
...
begin
  ...
  if v_def not like '%compra_item_destinos%'
    ...
    raise exception 'registrar_compra cambió desde que se escribió esta migración (ADR-0139): reescríbela sobre la definición viva';
  end if;
  if (select count(*) from information_schema.columns where table_schema = 'retail' and table_name = 'compras_resumen')
     not in (32, 33) then
    raise exception 'compras_resumen cambió de forma desde que se escribió esta migración (ADR-0139): revisa sus columnas';
  end if;
end $$;
...
comment on table compra_item_destinos is 'ADR-0139: el reparto de una línea de comprobante entre tiendas (cuánto de la línea le toca a cada una). La suma por línea es igual a la cantidad facturada; solo lo escriben las RPC.';
...
comment on table compra_reasignaciones is 'ADR-0139: bitácora append-only de lo que un líder movió, sin recibir aún, de una tienda a otra dentro de una línea de comprobante.';
...
comment on column compra_item_cierres.ubicacion_id is 'ADR-0139: de qué tienda es lo que faltó (la parte de la línea que le tocaba a esa tienda).';
...
comment on function fn_puede_ver_compra(uuid) is 'ADR-0139: ¿el comprobante tiene algo repartido a mi tienda (o soy líder)? Reemplaza al candado por el destino único de la factura.';
...
comment on view compra_item_reparto_resumen is 'ADR-0139: por línea de comprobante y tienda — asignado, recibido, cerrado y pendiente. Lo recibido sale de movimientos.';
...
begin
  ...
  v_validacion := $b$-- Reparto por tienda (ADR-0139): sin `destinos`, la línea va entera a `p_ubicacion_destino_id`.
    v_n := v_n + 1;
    v_destinos := case
  ...
  if v_nuevo = v_def or (length(v_def) - length(replace(v_def, 'v_constraint text;', ''))) / length('v_constraint text;') <> 1 then
    raise exception 'registrar_compra: no encontré (o no es única) la declaración de v_constraint (ADR-0139)';
  end if;
  v_def := v_nuevo;
  ...
  if (length(v_def) - length(replace(v_def, 'v_subtotal := v_subtotal + (v_item ->> ''cantidad'')::integer * (v_item ->> ''costo_unitario'')::numeric;', ''))) > 0
    ...
    raise exception 'registrar_compra: no encontré (o no es única) la línea que acumula el subtotal (ADR-0139)';
  end if;
  v_def := v_nuevo;
  ...
  if v_nuevo = v_def then raise exception 'registrar_compra: no encontré la lista de columnas del insert en compras (ADR-0139)'; end if;
  v_def := v_nuevo;
  ...
  if v_nuevo = v_def then raise exception 'registrar_compra: no encontré los valores del insert en compras (ADR-0139)'; end if;
  v_def := v_nuevo;
  ...
  v_nuevo := replace(v_def,
    ...
    'v_n2 := v_n2 + 1;' || E'\n    ' || 'insert into compra_items (compra_id, producto_id, variante_id, descripcion, cantidad, costo_unitario)');
  if v_nuevo = v_def then raise exception 'registrar_compra: no encontré el insert en compra_items (ADR-0139)'; end if;
  v_def := v_nuevo;
  select count(*) into v_n from regexp_matches(v_def, '\(v_item ->> ''costo_unitario''\)::numeric\s*\)\s*;', 'g');
  if v_n <> 1 then raise exception 'registrar_compra: esperaba UN cierre del insert en compra_items y encontré % (ADR-0139)', v_n; end if;
  v_nuevo := regexp_replace(v_def, '\(v_item ->> ''costo_unitario''\)::numeric\s*\)\s*;',
                            replace('(v_item ->> ''costo_unitario'')::numeric', '\', '\\') || v_reparto);
  if v_nuevo = v_def then raise exception 'registrar_compra: no pude cerrar el insert en compra_items con el reparto (ADR-0139)'; end if;
...
begin
  ...
  if v_nuevo = v_def then raise exception 'recibir_compras: no encontré dónde declarar las variables del reparto (ADR-0139)'; end if;
  v_def := v_nuevo;

  v_nuevo := replace(v_def,
    'if v_recibido + v_cerrado + v_cantidad > v_linea.cantidad then',
    E'-- Reparto por tienda (ADR-0139): cada tienda recibe lo suyo. Además del tope de la línea, esta\n'
  ...
  if v_nuevo = v_def then raise exception 'recibir_compras: no encontré dónde poner el tope por tienda (ADR-0139)'; end if;
...
begin
  ...
  if v_nuevo = v_def then raise exception 'recibir_envio: no encontré la validación por destino de la cabecera (ADR-0139)'; end if;
  v_def := v_nuevo;
  v_nuevo := replace(v_def,
  ...
  if v_nuevo = v_def then raise exception 'recibir_envio: no encontré el mensaje de la validación por destino (ADR-0139)'; end if;
  v_def := v_nuevo;
  ...
  if (length(v_def) - length(replace(v_def, 'v_cierre ->> ''nota''', ''))) / length('v_cierre ->> ''nota''') <> 1 then
    raise exception 'recibir_envio: esperaba UNA llamada a cerrar_linea_compra con v_cierre ->> nota (ADR-0139)';
  end if;
...
comment on function reasignar_reparto_compra(uuid, uuid, uuid, integer, text, text) is 'ADR-0139: mueve, de una tienda a otra, lo que aún no recibió ni cerró dentro de una línea de comprobante. Solo líder; deja rastro en compra_reasignaciones.';
