-- ============================================================================
-- Compras (ADR-0104, D3): pago por lote — UNA transferencia, VARIOS comprobantes
--
...
comment on function retail.registrar_pago_compras(uuid, text, jsonb, text, date, uuid) is
  'Pago por lote (ADR-0104 D3): una transferencia aplicada a varios comprobantes del mismo proveedor, todo o nada, con candado por comprobante e idempotente por token (el token es el pago_grupo_id). Devuelve el pago_grupo_id. Solo quien puede registrar pagos (líder).';
