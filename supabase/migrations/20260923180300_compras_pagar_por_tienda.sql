-- ============================================================================
-- 20260923180300_compras_pagar_por_tienda.sql — CAYLA V2 · ADR-0151 (F4 reescrita sobre ADR-0161)
--
...
comment on column retail.compra_pagos.ubicacion_id is
  'ADR-0151. Qué tienda hizo este pago: cuenta contra SU parte de la factura. NULA en pagos de antes y en un pago del líder no atribuido a ninguna tienda.';
...
comment on function retail.fn_saldo_de_tienda(uuid, uuid) is
  'ADR-0151. Cuánto le falta pagar a esta tienda en esta factura: su parte (compra_parte_por_tienda) menos lo que pagó ELLA, sin pasar del saldo real de la factura (que ya resta notas de crédito y pagos sin tienda). 0 si no tiene parte o la factura está anulada.';
...
comment on function retail.fn_pago_no_supera_tienda() is
  'ADR-0151. Disparador diferido: lo pagado por una tienda en una factura no puede superar su parte (compra_parte_por_tienda). Se salta si el pago no lleva tienda.';
...
language sql
as $$ select
  E'  -- ADR-0151: quien no es líder paga SIEMPRE desde una de sus tiendas, y solo la parte de esa tienda.\n'
...
language sql
as $$ select
  p_sangria || E'-- ADR-0151: con tienda, esa tienda tiene parte y el monto no pasa de lo que le queda a ella.\n'
...
begin
  ...
  v := pg_temp.exigir(v, E'          v_persona\n        )\n        returning id into v_pago_id;',
    E'          v_persona,\n          case when retail.fn_es_lider() then null else p_ubicacion_destino_id end -- ADR-0151: la gestora\n        )\n        returning id into v_pago_id;',
