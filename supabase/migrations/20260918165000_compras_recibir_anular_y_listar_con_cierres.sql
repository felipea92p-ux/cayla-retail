-- ============================================================================
-- Compras (ADR-0104, D2): lo que ya existía ahora respeta cierres y notas de crédito
--
...
begin
  ...
  loop
    ...
    -- Lo cerrado («no va a llegar», ADR-0104 D2) ya no se puede recibir.
...
comment on function retail.recibir_compras(uuid, jsonb, text, text) is
  'Recibe mercadería contra una o varias facturas del mismo proveedor (ADR-0035). Un ítem con compra_item_id = null es fuera de factura (ADR-0076): mismo lote, sin tope ni deuda, costo opcional. El tope por línea es cantidad - recibido - cerrado (ADR-0104 D2).';
...
begin
  ...
  -- ADR-0104: una nota de crédito o un cierre de línea cuelgan del comprobante;
