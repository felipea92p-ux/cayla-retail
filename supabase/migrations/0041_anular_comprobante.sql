-- Anular un comprobante ya aceptado por SUNAT — el paso (c) del plan de
-- facturación. Hasta hoy el sistema podía EMITIR y TRANSMITIR pero no
-- deshacer: `anularDocumentoLucode` existía en el adaptador sin ruta ni botón,
-- y `actualizar_transmision_comprobante` rechaza 'anulado' a propósito. Anular
-- quedaba como trámite manual en el panel de Lucode (le pasó a B004-000001 el
-- 05-09: un documento legal por una venta que no existió).
--
-- QUÉ PROMETE: registra la anulación de UN comprobante aceptado, con motivo
-- obligatorio y trazabilidad de quién la pidió, distinguiendo "SUNAT ya la
-- confirmó" de "está en trámite" — el resumen diario de boletas se procesa de
-- forma diferida y decir 'anulado' antes de tiempo sería la misma mentira que
-- cerró ADR-0015.
-- QUÉ ASUME: la llamada al proveedor YA ocurrió y esto solo guarda su
-- resultado (mismo reparto que `actualizar_transmision_comprobante`: Postgres
-- no habla con Lucode).
-- QUÉ NO HACE: no toca stock ni ventas — un comprobante en este esquema no
-- mueve inventario (`venta_id` es opcional y Facturación todavía no está
-- conectada a `ventas`, ADR-0009). Tampoco emite la nota de crédito de una
-- devolución: anular un error de emisión y aceptar una devolución son dos
-- hechos distintos del negocio (`emitir_nota` cubre el segundo).
--
-- CÓMO SE REVIERTE:
--   drop function anular_comprobante(uuid, text, boolean, jsonb);
--   alter table comprobantes drop constraint comprobantes_anulado_tiene_motivo;
--   alter table comprobantes drop column motivo_anulacion, drop column
--     anulacion_solicitada_at, drop column anulado_at, drop column
--     respuesta_anulacion, drop column anulado_por;
...
comment on column comprobantes.anulacion_solicitada_at is
...
alter table comprobantes drop constraint if exists comprobantes_anulado_tiene_motivo;
alter table comprobantes add constraint comprobantes_anulado_tiene_motivo
  check (estado <> 'anulado' or motivo_anulacion is not null) not valid;

do $$
begin
  alter table comprobantes validate constraint comprobantes_anulado_tiene_motivo;
...
begin
  ...
  if not fn_puede_operar_sede(v_comprobante.sede_id) then
