-- Cierre de caja (ADR-0186): el n.º de operación del depósito bancario pasa a ser OPCIONAL (Felipe, 2026-09-23).
-- Quien deposita no siempre tiene el voucher a mano al cerrar; bloquear el cierre por eso era peor que no tenerlo.
-- Solo «Entregado al líder de equipo» sigue exigiendo a quién se entregó: ahí no hay otro rastro del dinero.
--
-- YA ESTÁ EN PRODUCCIÓN: se pegó en el SQL Editor el 2026-09-23 sin dejar este archivo en git. Este archivo copia lo
-- que hay allá (pg_get_functiondef y pg_get_constraintdef leídos ese mismo día) para que repo, local y producción
-- digan lo mismo. Volver a pegarlo es inofensivo: el check se reemplaza y la función se reescribe igual.

-- ── 1. El candado de la tabla: solo el líder exige referencia ────────────────────────────────────────────────────
alter table retail.caja_traslados drop constraint if exists caja_traslados_referencia;
alter table retail.caja_traslados add constraint caja_traslados_referencia
  check (destino <> 'lider' or length(btrim(coalesce(referencia, ''))) >= 2);

-- ── 2. cerrar_caja: misma firma, sin el `raise` del banco ────────────────────────────────────────────────────────
