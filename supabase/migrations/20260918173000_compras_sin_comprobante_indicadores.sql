...
comment on function retail.resumen_sin_comprobante(uuid) is
  'Una fila: unidades y recepciones sin comprobante del mes (Lima), unidades sin costo registrado, y la última recepción (no limitada al mes). Sin comprobante = lote sin ningún movimiento contra una línea de comprobante; sin costo = movimiento sin fila en costo_historial. Acotada por sede. ADR-0104.';
...
comment on function retail.recepciones_sin_comprobante(uuid, integer) is
  'Recepciones sin comprobante, una fila por lote, más recientes primero (máx. 200): unidades, costo unitario promedio ponderado de lo que tiene costo y sin_costo = algún movimiento sin costo registrado. Acotada por sede. ADR-0104.';
