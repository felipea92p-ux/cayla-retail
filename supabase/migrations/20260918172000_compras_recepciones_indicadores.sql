-- ============================================================================
-- Recibir mercadería: quién cumple y cuánto tarda (ADR-0104, sección «Lectura»)
--
...
comment on function retail.listar_recepciones_compras(uuid, date, date, text, integer) is
  'Recepciones contra comprobante: una fila por (lote, comprobante) con lo que llegó, lo facturado, lo que falta hoy y los días desde la emisión. Busca por documento, proveedor o guía. Más recientes primero, máx. 200. Acotada por sede. ADR-0104.';
...
comment on function retail.resumen_recepciones(date) is
  'Una fila: unidades recibidas, guías (lotes), días de entrega promedio (emisión → llegada), comprobantes recibidos y cuántos llegaron completos, más el faltante abierto de hoy (recepción parcial). p_desde null = 90 días atrás. Acotada por sede. ADR-0104.';
