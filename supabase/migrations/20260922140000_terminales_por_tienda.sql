-- ============================================================================
-- 20260922140000_terminales_por_tienda.sql — CAYLA V2
--
-- ADR-0152 (número provisional: numerar al subir). Cuentas TERMINAL por tienda: dos por tienda,
-- compartidas por quien trabaja ahí — una de VENTAS y una ADMINISTRATIVA.
--
...
comment on function retail.fn_puede_dar_descuento_por_etiqueta() is
  'Solo el líder: asignar una etiqueta con descuento a una prenda al crearla (ADR-0152). Punto único para abrirlo el día que se decida.';
comment on function retail.fn_puede_gestionar_caja() is
  'Líder o terminal de ventas: cerrar caja y mover caja (ADR-0152; refina ADR-0143).';
comment on function retail.fn_puede_ajustar_inventario() is
  'Líder o terminal administrativa: ajustar stock, cerrar conteo y cerrar un traslado con diferencia (ADR-0152; refina ADR-0143).';
comment on function retail.fn_puede_editar_catalogo() is
  'Líder o terminal administrativa: escribir en el Catálogo (ADR-0152). Las etiquetas NO entran: llevan descuentos.';
comment on function retail.fn_puede_editar_cuentas_proveedor() is
  'Líder o terminal administrativa: cuentas bancarias de proveedores (ADR-0152).';
