...
-- ⚠ SUPERSEDIDO (2026-09-11). Este archivo CORRIÓ en producción y por eso se
--   conserva como registro — pero el cuerpo de `registrar_venta` que deja NO es el
--   que vive hoy. Otra sesión, en paralelo, aplicó después `37_registrar_venta_p_nota.sql`
--   con la misma firma y una guarda más segura (`is not true` en el candado de sede,
--   que no deja pasar un NULL). Verificado contra la base el 2026-09-11: producción
--   tiene `is not true`. NO volver a pegar este archivo: dejaría el candado con el
--   agujero. El cuerpo vivo está en la 37.
--
